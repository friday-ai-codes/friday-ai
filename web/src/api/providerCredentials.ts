...
export interface ListCredentialsParams {
   ...
   * 的 `?scope=any`（system ∪ 当前 project_id 全集，
...
export const providerCredentialsApi = {
  ...
  /** POST /api/providers/credentials/<id>/test-connection/（ 既有端点）。 */
   ...
   *
   * 把该凭证设为同 (scope, scope_id, provider_type) 维度的默认凭证（默认凭证路径）。
   */
