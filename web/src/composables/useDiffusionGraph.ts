/**
 * GraphRAG 二跳扩散画布数据转换 composable（ + ）
 *
 * 输入：(hop1Neighbors, hop2Neighbors, sourceChunks) 三个 reactive ref；
 * 输出：Vue Flow Node[] / Edge[] + 折叠 / 截断统计 + expandFolded 控制函数。
 *
 * 落地：节点 / 边构造 + 父节点推断 + 边样式 + 去重；折叠 / 截断为静态占位。
 * 接力：FOLD_THRESHOLD=50 / HARD_LIMIT=200 / BATCH_SIZE=50 完整折叠 / 截断 /
 ...
 * composable 决定）， 仅扩展 composable 即激活，**模板零改**。
 */
...
/**
 * ME-07：边样式 magic number 提到顶部命名常量，避免 1.5 / 2 / 0.9 / 0.5
...
/**
 * 折叠 / 截断阈值（per UI-SPEC §5.6 + ROADMAP §SC3）：
...
function buildEdge(
  ...
  // LO-02: neighbor.edge_type 已是 DiffusionEdgeType（=EdgeType 同义），删冗余 as 断言
  ...
  // HI-04: weight 防御 —— 后端 partial mock 路径下 weight 可能为 null/undefined/NaN，
...
export function useDiffusionGraph(
   ...
   * Array.prototype.sort 在现代 JS 引擎中是稳定排序（ECMA-262 2019+），
   ...
   * （T-257-04-04 spoofing mitigation）。
   */
   ...
   * （T-257-04-02 tampering mitigation：阻止用户通过反复点击突破 HARD_LIMIT）。
   */
