...
/**
 * + — useDiffusionGraph 单测
 * 验证：
 * - 节点 / 边构造 + 去重 + 父节点推断 + 边样式公式 + clamp（ a-i 9 条）
 * - 折叠 / 截断 / 排序 / expandFolded / watch reset（ j-q 8 条）
 */
...
describe('useDiffusionGraph', () => {
  ...
  it('e: strokeWidth 公式 + clamp [1.5, 4]', () => {
    ...
    const hop1 = [
      ...
      makeNeighbor({ chunk_id: 'h-275', file_path: 'src/a.ts', hop: 1, weight: 0.5 }),
      ...
      // hop=2 / weight=0 → 1.5
      makeNeighbor({ chunk_id: 'h-15', file_path: 'src/a.ts', hop: 2, weight: 0 }),
    ]
    const { flowEdges } = useDiffusionGraph(
      ref([hop1[0], hop1[1]]),
      ref([hop1[2], hop1[3]]),
    ...
    expect(byTarget['h-275']?.strokeWidth).toBeCloseTo(2.75, 5)
    expect(byTarget['h-clampHigh']?.strokeWidth).toBe(4)
    expect(byTarget['h-clampLow']?.strokeWidth).toBe(1.5)
    expect(byTarget['h-15']?.strokeWidth).toBe(1.5)
  })
  ...
  /**
   * 折叠 / 截断 / 排序 / expandFolded / watch reset 8 条单测。
