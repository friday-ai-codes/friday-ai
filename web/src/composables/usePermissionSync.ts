...
export function usePermissionSync {
 ...
 /**
 * 检测权限变化并通知用户
 * - 被移除出项目：toast 警告 + 跳转首页
 * - 角色降低：toast 提示
 * - 角色提升：静默更新，不打断用户
 */
 function detectPermissionChanges(
 oldMemberships: ProjectMembershipBrief,
 newMemberships: ProjectMembershipBrief,
 ) {
 // 无旧数据时不做对比（首次加载）
 if (oldMemberships.length === 0)
 return
 for (const oldM of oldMemberships) {
 const newM = newMemberships.find(m => m.project_id === oldM.project_id)
 if (!newM) {
 // 被移除出项目
 warning('您已被移除出项目，正在返回首页')
 router.push('/')
 return
 }
 ...
 return
 // 保存旧权限快照（深拷贝）
 const oldMemberships = authStore.projectMemberships.map(m => ({ ...m }))
 // 调用 fetchMe 获取最新权限
 await authStore.fetchMe
 // 对比权限变化
 detectPermissionChanges(oldMemberships, authStore.projectMemberships)
