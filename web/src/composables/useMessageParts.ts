/**
 * legacy legacy message → parts 数组 hydrate adapter。
 *
 * 纯函数 / 无外部依赖 / 仅前端运行时合成 —— 不写库。
 *
 * 算法（确定性，三分支兜底）：
 *   规则 A：`msg.parts` 非空 → 直接返回（idempotent；coding-plan workflow+ 新消息）
 *   规则 B：`metadata.timeline` 非空 → 按 timeline 顺序合成（legacy 末期 timeline-based 消息）
 *   规则 C：`metadata.narrations` 非空 → narrations 串 text part + tool_calls 串 tool_use part
 *           （legacy era 典型 chat 答复）
 *   规则 D：兜底 —— 单 text part = `msg.content`
 *
 * 设计契约见 覆盖见 `__tests__/fixtures/legacy-legacy-messages.json`。
 */
...
/**
 * legacy legacy message → parts 数组合成。
 *
...
export function hydrateLegacyMessage(msg: ConversationMessage): MessagePart[] {
  ...
  //   legacy era 典型：narration = 中间过渡文本，最终 content = 正式答复
  //   关键 case：F5 deep_analysis 长 markdown → 永远是顶层 text part，不再被
  // narration-block 折叠容器吃掉（ Goal 根治目标）
