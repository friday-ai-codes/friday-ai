...
<template>
  <PageContainer>
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        <Button variant="ghost" size="icon" @click="router.push('/runners')">
          <span class="icon-[lucide--arrow-left] w-5 h-5" />
        </Button>
        <h1 class="text-2xl font-bold">
          {{ runner?.name || '加载中...' }}
        </h1>
        <div v-if="runner" class="relative flex-shrink-0 w-3 h-3 transition-all duration-300">
    ...
    <ConfirmDialog
      ...
      title="删除 Runner"
      :description="`确定要删除 Runner「${runner?.name}」吗？此操作不可撤销。`"
      confirm-text="删除"
      variant="destructive"
      :loading="deleting"
