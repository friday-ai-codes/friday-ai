...
/**
 * /projects/:id/providers — 项目级 Provider 凭证管理页（Phase / / ）
 *
 * 路由：由 unplugin-vue-router 文件系统扫描自动注册 `/projects/:id/providers`
 *
 * 职责：
 * - useRoute 读取:id 参数作为 projectId
 * - 复用 Plan 交付的 scope-aware ProviderSettings 容器（scope='project' + projectId）
 * - 后端纵深防御（Plan Permission + queryset 双层）才是权限权威；
 * 前端仅 UX 层渲染，非项目成员 API 会返空列表/404（T- / T-）
 *
 * 与 /admin/providers 的差异：scope 由 'system' 切到 'project'，
...
const route = useRoute('/projects/[id]/providers')
...
<template>
 ...
 <ProviderSettings scope="project":project-id="projectId" />
 </main>
</template>
