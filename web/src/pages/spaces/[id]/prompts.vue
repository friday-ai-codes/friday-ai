 ...
 *   - DataTable 渲染 mergedSpaceList computed（D-06 三态合并）
 ...
 *   - 双层防御（Threat T-215-03）：onClick 内部 if (!canEdit.value) return + 后端 RBAC 兜底
 *
 * 与 -04 PromptEditor 的契约：
 *   - v-model:open 双向绑定 sheetOpen
 *   - :mode 由 editorMode ref 决定
 *   - :space-id 显式透传 spaceId（防 Threat T-215-04 跨租户）
 */
