...
<template>
  <div class="space-y-8">
    ...
    <div class="flex items-center justify-between">
      <div class="space-y-1">
        <div class="flex items-center gap-3">
          <div class="p-2 rounded-xl bg-primary/10 flex items-center justify-center">
    ...
    <EmptyState
      ...
      title="暂无关联仓库"
      description="将仓库关联到此空间以开始使用"
      :action-label="isSpaceAdmin ? '关联仓库' : undefined"
                  ...
                  <Tooltip>
                    <TooltipTrigger as-child>
                      <button @click="togglePermission(link)">
                        <Badge
                          :variant="link.permission_level === 'read_write' ? 'default' : 'secondary'"
    ...
    <Dialog v-model:open="dialogOpen">
      <DialogContent class="sm:max-w-lg">
        ...
        <div class="space-y-4">
          ...
          <!-- 仓库列表 -->
          <div class="max-h-72 overflow-y-auto space-y-1 border border-border/50 rounded-xl p-2 bg-muted/20">
            <div
            ...
            <div
              ...
              :class="selectedRepoIds.has(repo.id) ? 'bg-primary/10 border border-primary/30' : 'hover:bg-muted/50 border border-transparent'"
              @click="toggleRepo(repo.id)"
            >
              <Checkbox
                :model-value="selectedRepoIds.has(repo.id)"
                @click.stop
        ...
        <DialogFooter>
          ...
          <Button
            :disabled="selectedRepoIds.size === 0 || linking"
            class="group relative overflow-hidden"
            @click="handleLink"
          >
            <span class="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-700" />
