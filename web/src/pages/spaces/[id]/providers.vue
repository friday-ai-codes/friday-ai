...
/**
 * /spaces/:id/providers — 空间级 Provider 凭证管理页（ / / ）
 *
 * 路由：由 unplugin-vue-router 文件系统扫描自动注册 `/spaces/:id/providers`
 *
 * 职责：
 *   - useRoute 读取 :id 参数作为 spaceId
 * - 复用 交付的 scope-aware ProviderSettings 容器（scope='space' + spaceId）
 * - 后端纵深防御（ Permission + queryset 双层）才是权限权威；
 *     前端仅 UX 层渲染，非空间成员 API 会返空列表/404（T-229-07-01 / T-229-07-02）
 *
