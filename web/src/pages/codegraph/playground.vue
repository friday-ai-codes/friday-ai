...
// ：Tabs 容器（per UI-SPEC 硬约束 16，默认 layers）
const activeTab = ref<'layers' | 'graphrag'>('layers')

// ：Drawer state（CodePreviewDrawer 占位， 接力）
const drawerOpen = ref(false)
const selectedChunkId = ref<string | null>(null)

// ME-06: 切 Tab 时自动关闭 Drawer，避免 Drawer 与新 tab 内容无关联导致 UX 紊乱
watch(activeTab, () => {
...
function handleChatPrefill(params: { query: string, repositoryIds: string[] }) {
  // LO-04: 用 router.push({ path, query }) 让 Vue Router 自动 encode 每个 ID，
...
<template>
  <div class="max-w-7xl mx-auto p-6 space-y-6">
    ...
    <div class="flex gap-4 items-start">
      ...
      <!-- 右侧 Tabs 容器（：layers + graphrag 双 tab） -->
      <div class="card flex-1 min-w-0">
        <Tabs v-model="activeTab" class="w-full">
          <TabsList class="grid grid-cols-2 m-3">
          ...
          <TabsContent value="graphrag" class="mt-0">
            <GraphRAGDiffusionTab
              :hop1-neighbors="searchResult?.hop1_neighbors ?? []"
              :hop2-neighbors="searchResult?.hop2_neighbors ?? []"
              :source-chunks="extractSourceChunks(searchResult)"
              :loading="isLoading"
