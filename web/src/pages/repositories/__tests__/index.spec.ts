...
function makeRepository(overrides: Partial<Repository> = {}): Repository {
  return {
    ...
    git_url: overrides.git_url ?? 'https://gitlab.example.com/frontend/study-app.git',
  ...
  it('renders indexed repositories as quiet management cards', async () => {
    listMock.mockResolvedValue([
      makeRepository({
        id: 'repo-1',
        name: 'study-app',
        git_url: 'https://gitlab.example.com/frontend/study-app.git',
      }),
    ])
    ...
    expect(urlChip.text()).toContain('https://gitlab.example.com/frontend/study-app.git')
