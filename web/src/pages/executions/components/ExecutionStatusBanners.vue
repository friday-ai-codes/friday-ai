...
<template>
  ...
  >
    ...
    >
      <span class="icon-[lucide--wifi-off] w-4 h-4" />
  ...
  >
    ...
    >
      <div class="flex items-center gap-2 text-sm">
        <span class="icon-[lucide--bug] w-4 h-4" />
        ...
        <div class="flex items-center gap-1.5 ml-3 pl-3 border-l border-white/30">
          ...
          <Switch
            :checked="isBreakpointMode"
            class="data-[state=checked]:bg-white/30 data-[state=unchecked]:bg-white/20 h-4 w-7"
      ...
      <Button
        variant="ghost"
        size="sm"
        class="text-white hover:bg-white/20 h-7 text-xs"
        @click="emit('cancelDebug')"
      >
        <span class="icon-[lucide--square] w-3.5 h-3.5 mr-1" />
        终止调试
      </Button>
    </div>
  </Transition>
