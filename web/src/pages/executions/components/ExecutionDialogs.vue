...
<template>
  ...
  <Dialog :open="approvalDialogOpen" @update:open="emit('update:approvalDialogOpen', $event)">
    <DialogContent>
      ...
      <div class="space-y-4">
        <div v-if="selectedNodeExecution?.approval_data?.display_data" class="p-3 rounded-lg bg-muted">
    ...
    <DialogContent>
      ...
      <DialogFooter>
        ...
        <Button :disabled="triggering" @click="emit('trigger')">
          <span class="icon-[lucide--zap] w-4 h-4 mr-2" />
          触发
        </Button>
      </DialogFooter>
    </DialogContent>
    ...
    <DialogContent class="max-w-md">
      ...
      <div v-else class="space-y-3">
        ...
        <div v-if="resumeSkipNodes.length > 0">
          <p class="text-xs font-medium text-muted-foreground mb-1.5">
            <span class="icon-[lucide--skip-forward] w-3.5 h-3.5 inline-block mr-1 align-text-bottom" />
        ...
        <div v-if="resumeRerunNodes.length > 0">
          <p class="text-xs font-medium text-muted-foreground mb-1.5">
            <span class="icon-[lucide--play] w-3.5 h-3.5 inline-block mr-1 align-text-bottom" />
      ...
      <DialogFooter>
        ...
        <Button :disabled="resuming || resumePreviewLoading" @click="emit('resumeFromFailed')">
          <span v-if="resuming" class="icon-[lucide--loader-2] w-4 h-4 mr-2 animate-spin" />
          <span v-else class="icon-[lucide--play-circle] w-4 h-4 mr-2" />
          确认继续
        </Button>
      </DialogFooter>
    </DialogContent>
