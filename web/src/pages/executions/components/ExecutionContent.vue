...
<template>
  ...
  <div v-if="loading && !currentExecution" class="flex-1 flex items-center justify-center">
    <span class="icon-[lucide--loader-2] w-8 h-8 animate-spin text-primary" />
  </div>

  <!-- 错误状态 -->
  <div v-else-if="error" class="flex-1 flex items-center justify-center p-6">
    <div class="card border-destructive max-w-md w-full">
      <div class="p-5 space-y-4 py-6 text-center text-destructive">
        <span class="icon-[lucide--x-circle] w-12 h-12 mx-auto mb-4" />
        <p>{{ error }}</p>
      </div>
    </div>
  </div>
  ...
  <!-- DAG 画布 -->
  <div v-else-if="currentExecution" class="flex-1 min-h-0 relative">
    ...
    >
      <div class="text-center space-y-2">
        <span class="icon-[lucide--layout-grid] w-12 h-12 mx-auto opacity-30" />
        <p class="text-sm">
    ...
    <div class="absolute top-3 right-3 z-10 space-y-2">
      <CostSummaryBar
        :cost-summary="costData?.summary ?? null"
        :loading="costLoading"
      />
      <ProviderCostTable
        v-if="costData && isTerminalStatus"
        :cost-data="costData"
      />
    </div>
    ...
    >
      <div class="max-w-md w-full mx-4">
        <div class="bg-red-50 dark:bg-red-900/20 border border-red-200/60 dark:border-red-800/50 rounded-2xl p-6 shadow-xl space-y-3">
          <div class="flex items-center gap-3">
            <div class="shrink-0 w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/40 flex items-center justify-center">
              <span class="icon-[lucide--alert-triangle] w-5 h-5 text-red-500" />
            </div>
            <div>
          ...
          <div class="flex justify-end">
            <Button
              ...
              @click="emit('retry')"
            >
              <span class="icon-[lucide--rotate-ccw] w-3.5 h-3.5 mr-1.5" />
              重新执行
            </Button>
          </div>
        </div>
      ...
      >
        <div class="bg-red-50 dark:bg-red-900/30 backdrop-blur-sm border border-red-200/50 dark:border-red-800/50 rounded-2xl px-4 py-3 shadow-lg">
          <div class="flex items-start gap-2">
            <span class="icon-[lucide--alert-circle] w-4 h-4 text-red-500 mt-0.5 shrink-0" />
