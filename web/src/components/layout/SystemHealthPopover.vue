...
<template>
 ...
 <PopoverContent align="end":side-offset="8" class="w-80 overflow-hidden">
 ...
 <button
 type="button"
 class="flex items-center justify-center w-7 rounded-md hover:bg-muted transition-colors disabled:opacity-50":disabled="loading":title="loading ? '检测中...': '立即刷新'"
 @click="refresh"
 >
 <span
 class="text-base":class="loading ? 'icon-[lucide--loader-2] animate-spin': 'icon-[lucide--refresh-cw]'"
 />
 </button>
 </div>
 ...
 >
 <span
 class="text-lg shrink-0 mt-0.5":class="[toneIcon(svc.tone), toneClass(svc.tone)]"
 />
 <div class="flex-1 min-w-0">
 <div class="flex items-center justify-between gap-2">
 <span class="text-sm font-medium truncate">{{ svc.label }}</span>
 <span
 class="text-xs shrink-0 px-1.5 py-0.5 rounded":class="{
