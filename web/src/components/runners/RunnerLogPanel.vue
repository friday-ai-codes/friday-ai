...
<template>
  <div class="card">
    ...
    >
      <div class="flex items-center justify-between">
        ...
        <span
          ...
          :class="collapsed ? 'icon-[lucide--chevron-down]' : 'icon-[lucide--chevron-up]'"
        />
      </div>
      <!-- 过滤器 -->
      <div v-show="!collapsed" class="flex flex-wrap items-center gap-2 text-xs mt-3" @click.stop>
        <label
          ...
          :key="option.key"
          :for="`runner-log-filter-${option.key}`"
        ...
        >
          <Checkbox
            :id="`runner-log-filter-${option.key}`"
            :checked="filters[option.key]"
            class="h-3.5 w-3.5 rounded-[4px] ring-offset-0 focus-visible:ring-1 focus-visible:ring-offset-0"
    ...
    <div v-show="!collapsed" class="p-5 space-y-4 p-0">
      <div class="relative">
        <div
          ref="scrollContainer"
          class="bg-gray-950 rounded-b-2xl p-4 font-mono text-sm max-h-80 overflow-y-auto"
          @scroll="onScroll"
        >
          <!-- 空状态 -->
          ...
          >
            ...
            <span class="shrink-0 w-4 h-4 mt-0.5" :class="[getLogIcon(log), getLogColor(log)]" />
