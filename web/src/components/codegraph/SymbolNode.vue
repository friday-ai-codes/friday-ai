...
<template>
 ...
 props.selected
 ? 'ring-2 ring-primary/50 shadow-lg border-primary/30': 'border-border/50 hover:shadow-md',
 ]":aria-label="`${data.symbol_type}: ${data.name}`"
 >
 <!-- 类型徽章：右上角 absolute -->
 <Badge:variant="symbolTypeBadgeVariant(data.symbol_type)"
 class="absolute top-2 right-2 text-xs px-1.5 leading-none"
 >
 {{ data.symbol_type }}
 </Badge>
 <!-- 头部：图标 + 名称 -->
 ...
 <span:class="`icon-[lucide--${symbolTypeIcon(data.symbol_type)}] w-3.5 .5 text-primary`" />
 </div>
