...
/**
 * 代码片段预览 Drawer（，UI-SPEC / / 硬约束 9 + 10）
 *
 ...
 * "代码片段未在当前查询命中范围内，无法预览"（UI-SPEC §6 + §10 硬约束 10）。
 *
 ...
 * （per ESLint vue/no-v-html + 威胁模型 T-257-03-01）。
 */
...
<template>
  <Sheet :open="open" @update:open="onOpenUpdate">
    <SheetContent
      side="right"
      class="w-[560px] max-w-[80vw] p-0 flex flex-col"
    ...
    >
      ...
      <div class="flex-1 overflow-hidden p-5">
        ...
        <ScrollArea v-else class="h-full">
          <pre
            class="font-mono text-xs p-3 rounded-lg bg-muted/30 whitespace-pre-wrap break-words"
          >{{ chunk.content }}</pre>
        </ScrollArea>
      </div>
    </SheetContent>
