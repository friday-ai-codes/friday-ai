...
/**
 * GraphRAG 二跳扩散画布主入口（，UI-SPEC ）
 *
 * 复用 已 freeze 的 props/emit 契约：
 ...
 *
 * 折叠 / 截断模板已挂 v-if 占位，但本 plan composable 永远返回 false（ 接力
 * 扩展 useDiffusionGraph 即可激活，**不再改本组件**，保证 Wave 3 真并行）。
 */
import type {
  Edge,
  EdgeComponent,
...
// LO-03：使用 Vue Flow 原生 NodeMouseEvent 替代 Pick 自造类型
function onVueFlowNodeClick(evt: NodeMouseEvent) {
  emit('node-click', evt.node.id)
}

// HI-01：键盘激活路径 —— DiffusionNode keydown.enter/space 会 emit('activate', chunk_id)，
...
// HI-03：新查询返回不同图尺寸时主动 fitView 复位视口；
...
<template>
  <!-- HI-07：TooltipProvider 上提到画布根，节点 / 边共享单实例（避免 200×2 provider 状态机） -->
  <TooltipProvider :delay-duration="200">
    <div class="h-[520px] relative">
      <!-- 截断警告 banner（>200 节点；UI-SPEC ）—— 激活 -->
      <div
        v-if="truncated"
      ...
      >
        ...
        >
          <span
            v-for="type in EDGE_TYPES_LEGEND"
            :key="type"
        ...
        >
          <button
            type="button"
            class="bg-card/90 backdrop-blur-sm text-xs h-8 inline-flex items-center px-3 rounded-md border border-border/50 hover:bg-muted/40"
            @click="expandFolded"
          >
            <span class="icon-[lucide--chevron-down] mr-1.5 w-3.5 h-3.5" />
            显示更多 ({{ foldedCount }}+)
          </button>
        </Panel>
      </VueFlow>

      <!-- 加载遮罩 (ME-02：尊重 prefers-reduced-motion) -->
      <div
        v-if="loading"
        class="absolute inset-0 flex items-center justify-center bg-background/50 z-10"
      >
        <span class="icon-[lucide--loader-circle] animate-spin motion-reduce:animate-none w-8 h-8 text-primary" />
      </div>
    </div>
  </TooltipProvider>
</template>
