...
<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    >
      <ToggleGroupItem
        v-for="type in SYMBOL_TYPES"
        :key="type.value"
        :value="type.value"
        class="min-h-[36px] text-xs gap-1.5 px-2.5"
      >
        <span class="w-3.5 h-3.5" :class="[type.icon]" />
        {{ type.label }}
      </ToggleGroupItem>
    </ToggleGroup>

    <Input
      :model-value="props.nameQuery ?? ''"
      placeholder="搜索名称..."
    ...
    <Input
      :model-value="props.filePathQuery ?? ''"
      placeholder="所有文件"
      class="w-48 h-9 text-sm font-mono"
      @update:model-value="emit('update:filePathQuery', String($event))"
    >
      <template #prefix>
        <span class="icon-[lucide--folder] text-muted-foreground w-3.5 h-3.5" />
      </template>
    </Input>
  </div>
</template>
