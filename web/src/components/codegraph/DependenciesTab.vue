...
<template>
 ...
 <div class="flex rounded-lg border border-border/50 .5">
 <button
 type="button"
 class="rounded-md px-2.5 py-1 text-xs transition-colors":class="focusType === 'file' ? 'bg-primary/15 text-primary': 'text-muted-foreground'"
 ...
 <button
 type="button"
 class="rounded-md px-2.5 py-1 text-xs transition-colors":class="focusType === 'component' ? 'bg-primary/15 text-primary': 'text-muted-foreground'"
 ...
 @click="drillDown"
 >
 <span class="icon-[lucide--git-fork] mr-1.5 .5 w-3.5" />
 下钻符号级 DAG
 </Button>
 </div>
 ...
 <!-- Vue Flow 双向依赖画布 -->
 <VueFlow
 v-else:nodes="flowNodes":edges="flowEdges":min-zoom="0.2":max-zoom="2.0":fit-view-on-init="true":pan-on-scroll="false":prevent-scrolling="true":nodes-draggable="true"
 ...
 <span class=".5 w-2.5 rounded-sm border border-sky-500/50 bg-sky-500/10" />
 上游（被谁用）
 </span>
 <span class="flex items-center gap-1.5 text-xs text-muted-foreground">
 <span class=".5 w-2.5 rounded-sm border-2 border-primary bg-primary/10" />
 焦点
 </span>
 <span class="flex items-center gap-1.5 text-xs text-muted-foreground">
 <span class=".5 w-2.5 rounded-sm border border-emerald-500/50 bg-emerald-500/10" />
 下游（用了谁）
 </span>
 </Panel>
 </VueFlow>
