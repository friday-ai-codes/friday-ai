...
<template>
  <div class="card flex flex-col gap-4 p-5 w-[300px] shrink-0 self-start sticky top-4">
    <!-- 查询 -->
    <div>
      ...
      <div class="relative">
        ...
        >
          <div class="max-h-48 overflow-y-auto p-1">
            ...
            >
              <span
                class="w-4 h-4 border rounded-sm flex items-center justify-center shrink-0"
                :class="selectedRepoIds.includes(repo.id) ? 'bg-primary border-primary' : 'border-input'"
              >
                <span v-if="selectedRepoIds.includes(repo.id)" class="icon-[lucide--check] w-3 h-3 text-primary-foreground" />
              </span>
    ...
    <Button
      ...
      @click="handleSearch"
    >
      <span v-if="props.loading" class="icon-[lucide--loader-circle] animate-spin mr-2 w-4 h-4" />
      执行检索
    </Button>
    ...
    >
      <span class="icon-[lucide--message-circle] mr-2 w-4 h-4" />
      在 Chat 中提问
    </Button>
  </div>
</template>
