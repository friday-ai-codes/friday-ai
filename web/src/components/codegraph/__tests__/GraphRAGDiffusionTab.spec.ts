...
// HI-03：fitView 模块级共享 spy，watch hop1/hop2 props 变化时被 watch 调用
...
describe('graphRAGDiffusionTab', () => {
  ...
  it('f: VueFlow @node-click → emit node-click(chunkId)（LO-01：通过事件触发非内部方法）', async () => {
    const wrapper = mount(GraphRAGDiffusionTab, {
      props: {
