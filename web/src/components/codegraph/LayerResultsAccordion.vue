  ...
  <div class="card flex-1 min-w-0">
    ...
    <div class="p-5 space-y-2">
      ...
      >
        ...
        >
          <span
            class="icon-[lucide--chevron-right] text-muted-foreground transition-transform shrink-0"
            :class="openStates[layer.id] ? 'rotate-90' : ''"
          />
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger as-child>
                <span
                  class="icon-[lucide--info] w-3.5 h-3.5 text-muted-foreground cursor-help shrink-0"
                  aria-label="功能说明"
                />
              </TooltipTrigger>
        ...
        <div v-show="openStates[layer.id]" class="border-t border-border/50">
          <!-- Loading 骨架屏 -->
          <div v-if="props.loading" class="p-4 space-y-2">
          ...
          >
            <div class="relative">
              <div
                class="font-mono text-xs bg-muted/30 rounded-lg p-3 overflow-y-auto transition-all"
                :class="l5Expanded ? 'max-h-none' : 'max-h-80'"
              >
              ...
              <div
                v-if="!l5Expanded"
                class="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-background/80 to-transparent rounded-b-lg flex items-end justify-center pb-2"
              >
                <Button variant="ghost" size="sm" class="text-xs h-7" @click="l5Expanded = true">
                  <span class="icon-[lucide--expand] mr-1.5 w-3.5 h-3.5" />
                  展开全文
                </Button>
              </div>
              <div v-else class="mt-2 flex justify-center">
                <Button variant="ghost" size="sm" class="text-xs h-7" @click="l5Expanded = false">
                  收起
                </Button>
              </div>
            </div>
          ...
          <div v-else class="p-4">
            ...
            <div v-else class="space-y-2">
              <div
                v-for="(item, idx) in (getLayerData(layer.layerKey)?.items ?? [])"
                :key="idx"
