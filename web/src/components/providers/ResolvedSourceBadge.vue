...
/**
 * ResolvedSourceBadge — / 四层 Provider 解析 Inspector。
 *
 * Props:
 ...
 *   - size?: 'sm' | 'md'（默认 md）
 *
 * 视觉契约（UI-SPEC §Source-hue token + §Dimension 3）:
 ...
 *     value 不存在：灰色 —
 *   - 禁用 backdrop-blur（Clean Card 纠偏 UI-SPEC L33）
 *
 * Analog: ProviderHealthBadge.vue（同目录， 四态模板）+ PATTERNS 2
 */
 ...
 *
 * UI-SPEC §Source-hue token L155-167：Badge outline + 内部 icon 着色，
...
/** 四层中文标签（UI-SPEC §Copywriting L287 node → 节点配置 映射）。 */
...
/** 链表每行的 value 文本：provider_type [+ / model]（UI-SPEC §Copywriting + Typography mono）。 */
...
<template>
  <span class="inline-flex items-center relative">
      ...
      <Tooltip>
        <TooltipTrigger as-child>
          <Badge
            variant="outline"
            role="button"
            :aria-label="`当前生效：${labelMap[props.source]} · 点击展开优先级链`"
            class="inline-flex items-center gap-1 px-2 py-0.5 cursor-pointer select-none"
            :class="{ 'text-xs': props.size === 'sm' }"
          >
            <span
              :class="hueIconMap[props.source]"
        ...
        <TooltipContent class="max-w-sm text-xs font-normal">
          ...
          >
            ...
            <span
              v-if="entry.active"
              class="icon-[lucide--check] w-3 h-3 text-primary shrink-0"
              aria-hidden="true"
              title="当前生效层"
            />
          </div>
    ...
    <!--
      A11y sr-only 链表 — 屏幕阅读器 + 测试双用（UI-SPEC §I-5 契约）。
      Tooltip Portal 在 happy-dom 下不挂 DOM；sr-only 镜像保证 chain 契约可被
      wrapper.html() 断言（沿用 title / aria / DOM 三重承载模式）。
    -->
