 ...
 *   - 若旧 modelValue 不在新 list 中,自动 emit('update:modelValue', null) 清空(D-14)
 *   - capability 过滤:
 *     requiresTools=true  且 m.supports_tools===false → 排除
 *     requiresVision=true 且 m.supports_vision!==true → 排除
 *   - option 标签:`${display_name} [${Math.round(ctx/1000)}K]`(UI-SPEC §Placeholder)
...
watch(
  ...
  async (id) => {
    ...
    try {
      ...
      // D-14:旧 model 在新 Provider 下不可用 → 清空,由父组件负责提示文案
    ...
    <SelectContent>
      <template v-if="loading">
        <div class="px-3 py-2 text-sm text-muted-foreground inline-flex items-center gap-1">
          <span class="icon-[lucide--loader-2] animate-spin w-3 h-3" />
          加载模型清单中…
        </div>
      </template>
