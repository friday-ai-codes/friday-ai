 ...
 *
 * Typography: SelectLabel 显式 font-normal 覆盖默认字重(UI-SPEC §Typography)。
 */
...
function onChange(id: unknown) {
  ...
  const cred = idStr
    ? (availableCredentials.value.find(c => c.id === idStr) ?? null)
    : null
    ...
    <SelectContent>
      <template v-if="availableCredentials.length === 0">
        <div class="px-3 py-4 flex flex-col items-center gap-2">
          <span class="icon-[lucide--alert-triangle] w-4 h-4 text-muted-foreground" />
      ...
      <template v-else>
        <SelectGroup
          v-for="(creds, providerType) in groupedByProvider"
          :key="providerType"
        >
