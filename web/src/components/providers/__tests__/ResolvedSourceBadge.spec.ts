/**
 * — ResolvedSourceBadge 组件测试。
 *
 * 覆盖 UX-05 D-13/D-14 契约：
 ...
 *   F: Badge 本身 variant="outline"（UI-SPEC §Dimension 3 硬锁 — hue 仅 icon，Badge 不叠色）
 *
 * Analog: ProviderHealthBadge.spec（同目录，）+ 三重承载模式（title / aria-label）。
 */
...
describe('resolvedSourceBadge', () => {
  ...
  it('f: Badge 本身使用 variant="outline"（UI-SPEC §Dimension 3 锁定 hue 仅 icon）', () => {
    const wrapper = mount(ResolvedSourceBadge, {
      props: { source: 'node', chain: makeChain('node') },
    })
    const html = wrapper.html()
