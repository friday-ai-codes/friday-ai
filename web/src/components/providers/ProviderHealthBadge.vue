 ...
 *
 * 四态映射（work item §Color Badge variant 表严格对齐）:
 ...
 *
 * Typography: work item §Typography 要求 Label 使用 font-normal，覆盖 Badge 默认 font-semibold。
...
const tooltipText = computed( => {
 ...
 ? `连接正常（最后测试于 ${relative}，延迟 ${props.latencyMs}ms）`: relative
 ? `连接正常（最后测试于 ${relative}）`: '连接正常'
 case 'error':
 return props.lastError
 ? `连接失败：${props.lastError}${relative ? `（最后测试于 ${relative}）`: ''}`: '连接失败'
 case 'testing':
 return '正在测试连接，请稍候…'
 default:
...
<template>
 ...
 <Badge:variant="variantByStatus"
 role="status"
 aria-live="polite":data-locked="locked ? 'true': 'false'"
