...
<template>
 ...
 class="group transition-colors hover:bg-muted/40":class="{ 'opacity-55': !c.is_active }"
 >
 <!-- Provider：品牌图标芯片 + 显示名 -->
 <td class="px-4 py-3.5">
 <div class="flex items-center gap-2.5">
 <span
 class="flex w-8 shrink-0 items-center justify-center rounded-lg":class="getProviderBrandColor(c.provider_type).bg"
 aria-hidden="true"
 >
 <span
 class=" w-4":class="[iconFor(c.provider_type), getProviderBrandColor(c.provider_type).text]"
 />
 </span>
 ...
 title="该维度默认凭证"
 >
 <span class="icon-[lucide--star] .5 w-2.5" aria-hidden="true" />
 默认
 </span>
 </div>
 ...
 >
 <span
 class=" w-3":class="c.scope === 'system' ? 'icon-[lucide--globe]': 'icon-[lucide--folder-lock]'"
 aria-hidden="true"
 />
 ...
 >
 <SelectTrigger class=" min-w-44 max-w-56 text-xs">
 ...
 <span class="icon-[lucide--box] w-3 opacity-70" aria-hidden="true" />
 {{ c.default_model || '未配置' }}
 </span>
 </td>
 <!-- 健康 -->
 ...
 variant="ghost"
 size="icon"
 class=" w-8 text-muted-foreground opacity-60 transition-opacity group-hover:opacity-100":aria-label="`${c.name} 操作菜单`"
 >
 ...
 <span class="icon-[lucide--pencil] mr-2 .5 w-3.5" aria-hidden="true" />
 编辑
 </DropdownMenuItem>
 <DropdownMenuItem
 v-if="!c.is_default"
 ...
 >
 <span class="icon-[lucide--star] mr-2 .5 w-3.5" aria-hidden="true" />
 设为默认
 </DropdownMenuItem>
 <DropdownMenuItem @click="emit('refreshModels', c)">
 <span class="icon-[lucide--refresh-cw] mr-2 .5 w-3.5" aria-hidden="true" />
 刷新模型清单
 </DropdownMenuItem>
 <DropdownMenuItem
 ...
 @click="emit('delete', c)"
 >
 <span class="icon-[lucide--trash-2] mr-2 .5 w-3.5" aria-hidden="true" />
 删除凭证
 </DropdownMenuItem>
 </DropdownMenuContent>
 </DropdownMenu>
