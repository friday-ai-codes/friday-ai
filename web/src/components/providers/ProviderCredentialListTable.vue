 ...
 * - ProviderHealthBadge @test 转发为 emit('test-connection', c)
 *
...
const emit = defineEmits<{
 ...
 (e: 'toggle-active', c: ProviderCredentialDto): void
 (e: 'test-connection', c: ProviderCredentialDto): void
 (e: 'refresh-models', c: ProviderCredentialDto): void
}>
...
<template>
 ...
 <span:class="[iconFor(c.provider_type), 'w-4 ']" aria-hidden="true" />
 ...
 @test="emit('test-connection', c)"
 />
 </td>
 <td class="px-4 py-3">
 <Switch:model-value="c.is_active":aria-label="`启用 ${c.name}`"
 @update:model-value="emit('toggle-active', c)"
 />
 </td>
 ...
 <DropdownMenuItem @click="emit('refresh-models', c)">
 刷新模型清单
 </DropdownMenuItem>
 <DropdownMenuItem
