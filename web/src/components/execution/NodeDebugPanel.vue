...
<template>
 ...
 <Button variant="ghost" size="icon" class="ml-auto w-6" @click="fetchInteractions">
 <span class="icon-[lucide--refresh-cw] w-3 " />
 </Button>
 </div>
 ...
 class=" rounded-xl border border-amber-500/30 bg-amber-500/5 space-y-2"
 >
 ...
 <Button
 size="sm"
 class="shrink-0 self-end":disabled="submitting || !customAnswer.trim"
 @click="submitAgentAnswer(customAnswer)"
 >
 <span v-if="submitting" class="icon-[lucide--loader-2] w-3 animate-spin mr-1" />
 提交
 </Button>
 </div>
 </div>
 ...
 <div
 v-if="answered"
 class=" rounded-xl border border-border/50 bg-muted/30 space-y-1 opacity-70"
 >
 <div class="text-sm">
 ...
 <!-- InteractionLog 待回答 -->
 <div
 v-for="item in pendingInteractions":key="item.id"
 class=" rounded-xl border border-amber-500/30 bg-amber-500/5 space-y-2"
 >
 ...
 <pre v-if="item.code_snippet" class=" rounded-lg bg-muted text-xs overflow-auto max- font-mono">{{ item.code_snippet }}</pre>
 ...
 <!-- InteractionLog 已回答 -->
 <div
 v-for="item in answeredInteractions":key="item.id"
 class=" rounded-xl border border-border/50 bg-muted/30 space-y-1 opacity-70"
 >
 <div class="text-sm">
