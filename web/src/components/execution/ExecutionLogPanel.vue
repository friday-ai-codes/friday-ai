...
<template>
 ...
 class="border-b last:border-b-0 py-2":class="getLogBorderClass(log.level)"
 >
 <Collapsible>
 ...
 <!-- 级别图标 -->
 <span
 class="shrink-0 w-4 mt-0.5":class="[getLogIconClass(log.level), getLogColorClass(log.level)]"
 />
 <!-- 时间戳 -->
 ...
 class="icon-[lucide--chevron-down] w-4 text-muted-foreground shrink-0 transition-transform"
 />
 </div>
 </CollapsibleTrigger>
 <!-- 上下文详情（可展开） -->
 <CollapsibleContent v-if="log.context && Object.keys(log.context).length > 0">
 <div class="mt-2 ml-10 bg-muted/40 rounded-lg ">
