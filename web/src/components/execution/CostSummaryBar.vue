...
<template>
  ...
  <div
    v-if="loading"
    class="flex items-center gap-3 px-3 py-1.5 card shadow-sm"
  >
  ...
  <div
    ...
    class="flex items-center gap-3 px-3 py-1.5 card shadow-sm text-xs"
  >
    <!-- 总成本 -->
    <div class="flex items-center gap-1 text-foreground">
      <span class="icon-[lucide--coins] w-3.5 h-3.5 text-amber-500" />
      <span class="font-medium tabular-nums">{{ formatCost(costSummary.total_cost_usd) }}</span>
    </div>

    <div class="w-px h-3.5 bg-border/50" />

    <!-- 总 Token -->
    <div class="flex items-center gap-1 text-muted-foreground">
      <span class="icon-[lucide--hash] w-3.5 h-3.5" />
