...
<template>
  <div class="space-y-2">
    ...
    <div
      v-for="step in steps"
      :key="step.id"
      class="rounded-xl border border-border/50 overflow-hidden transition-colors"
      :class="step.status === 'failed' ? 'border-red-400/30' : ''"
    >
      <!-- 步骤头部（可点击展开） -->
      <button
      ...
      >
        <!-- 状态圆点 -->
        <div class="w-2.5 h-2.5 rounded-full shrink-0" :class="statusColor(step.status)" />
        <!-- 名称 -->
        ...
        <!-- 展开指示 -->
        <span
          class="icon-[lucide--chevron-down] w-3.5 h-3.5 text-muted-foreground transition-transform duration-200"
          :class="{ 'rotate-180': expandedStepId === step.id }"
        />
      </button>
      ...
      <div v-if="expandedStepId === step.id" class="px-3 pb-3 space-y-3 border-t border-border/30">
        ...
        <div v-if="Object.keys(step.input_data).length > 0">
          ...
          <pre class="text-[11px] bg-muted/30 rounded-lg p-2 overflow-x-auto max-h-40 overflow-y-auto font-mono">{{ JSON.stringify(step.input_data, null, 2) }}</pre>
        </div>
        ...
        <div v-if="Object.keys(step.output_data).length > 0">
          ...
          <pre
            class="text-[11px] bg-muted/30 rounded-lg p-2 overflow-x-auto max-h-40 overflow-y-auto font-mono"
            :class="step.status === 'failed' ? 'text-red-400' : ''"
