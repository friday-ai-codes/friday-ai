...
<template>
 ...
 <Button
 v-for="opt in statusOptions":key="opt.value":variant="selectedStatus === opt.value ? 'secondary': 'ghost'"
 size="sm"
 class=" text-xs"
 ...
 <div v-for="i in 5":key="i" class=" rounded-xl bg-muted/30 animate-pulse" />
 </div>
 <!-- 空状态 -->
 <div v-else-if="executions.length === 0" class="flex flex-col items-center justify-center py-12 gap-3">
 <span class="icon-[lucide--history] w-10 text-muted-foreground/30" />
 <p class="text-sm text-muted-foreground">
 还没有执行记录
 </p>
 <Button size="sm" @click="emit('execute')">
 <span class="icon-[lucide--play] w-3.5 .5 mr-1.5" />
 立即执行
 </Button>
 </div>
 <!-- 执行列表 -->
 ...
 <Pagination
 v-model:page="currentPage":total="totalCount":items-per-page="pageSize":sibling-count="1"
 />
 </div>
 </div>
</template>
