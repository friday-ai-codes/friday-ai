...
<template>
 ...
 >
 <span class="icon-[lucide--alert-triangle] w-3 mr-1" />
 部分完成
 </Badge>
 <Badge
 ...
 <span class="icon-[lucide--check-circle] w-4 text-emerald-500" />
 成功仓库
 ...
 <span
 class="icon-[lucide--chevron-down] w-4 transition-transform duration-200":class="{ 'rotate-180': succeededOpen }"
 />
 </CollapsibleTrigger>
 <CollapsibleContent class="space-y-2 pt-2">
 <div
 v-for="(repo, index) in succeededRepos":key="index"
 ...
 <a
 v-if="repo.mr_url || repo.merge_request_url":href="repo.mr_url || repo.merge_request_url"
 target="_blank"
 ...
 <div
 v-for="(task, tIdx) in repo.tasks_completed":key="tIdx"
 class="flex items-center gap-1.5 text-xs text-muted-foreground"
 >
 <span class="icon-[lucide--check] w-3 text-emerald-500 shrink-0" />
 <span>{{ task }}</span>
 </div>
 </div>
 <!-- Changes Stats -->
 ...
 <span
 class="icon-[lucide--chevron-down] w-4 transition-transform duration-200":class="{ 'rotate-180': failedOpen }"
 />
 </CollapsibleTrigger>
 <CollapsibleContent class="space-y-2 pt-2">
 <div
 v-for="(fail, index) in failedDetails":key="index"
 class="rounded-xl bg-red-500/5 border border-red-500/10 "
 >
