...
<template>
  <div class="flex items-center gap-3 px-4 py-2">
    ...
    <button
      type="button"
      class="flex items-center justify-center w-9 h-9 rounded-full bg-primary text-primary-foreground hover:bg-primary/90 transition-colors"
      :aria-label="isPlaying ? '暂停' : '播放'"
      @click="togglePlay"
    >
      <span
        class="w-5 h-5"
        :class="isPlaying ? 'icon-[lucide--pause]' : 'icon-[lucide--play]'"
      />
    </button>
    ...
    <button
      type="button"
      class="flex items-center justify-center w-9 h-9 rounded-lg hover:bg-secondary transition-colors disabled:opacity-30"
      :disabled="!canStepBackward"
      aria-label="上一步"
      @click="emit('stepBackward')"
    >
      <span class="w-4 h-4 icon-[lucide--skip-back]" />
    </button>
    <button
      type="button"
      class="flex items-center justify-center w-9 h-9 rounded-lg hover:bg-secondary transition-colors disabled:opacity-30"
    ...
    >
      <span class="w-4 h-4 icon-[lucide--skip-forward]" />
    </button>
