  ...
  <div class="space-y-6">
    ...
    <div>
      <h3 class="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
        <span class="icon-[lucide--brain] w-4 h-4 text-violet-500" />
        ReAct 步骤流
      </h3>
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  :class="getStepMeta(step.action_type).bgClass"
                >
                  <span
                    class="w-3.5 h-3.5"
                    :class="[getStepMeta(step.action_type).iconClass, getStepMeta(step.action_type).colorClass]"
                  />
                </div>
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-1.5">
                    <span
                      ...
                      :class="getStepMeta(step.action_type).colorClass"
                    >
                ...
                <div class="flex items-center gap-1 shrink-0">
                  <span
                    ...
                    class="icon-[lucide--alert-triangle] w-3.5 h-3.5 text-red-500"
                  />
                  <span
                    class="text-[11px] tabular-nums"
                    :class="step.duration_ms != null && step.duration_ms > 30000 ? 'text-red-500 font-medium' : 'text-muted-foreground'"
                  >
                ...
                <!-- 展开箭头 -->
                <span class="icon-[lucide--chevron-down] w-3.5 h-3.5 text-muted-foreground transition-transform" />
              </CollapsibleTrigger>
              ...
              <CollapsibleContent>
                <div class="px-3 pb-3">
                  ...
                  <div v-else-if="loadedDetails.has(step.id)" class="mt-1">
                    <JsonHighlighter
                      :json="(loadedDetails.get(step.id)?.payload as Record<string, unknown>) ?? null"
                      :show-copy="true"
                    />
                  </div>
                </div>
    ...
    <div>
      <h3 class="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
        <span class="icon-[lucide--coins] w-4 h-4 text-amber-500" />
        Token 消耗与成本
      </h3>
      ...
      <div v-else class="card overflow-hidden">
        <div
          v-for="node in allNodeCosts"
          :key="node.node_id"
          class="divide-y divide-border/30"
        >
          <div
            v-for="(modelData, modelName) in node.models"
            :key="modelName"
