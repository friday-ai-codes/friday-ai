...
<template>
  <div class="card card overflow-hidden">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="bg-primary/10 rounded-lg p-2">
            <span class="icon-[lucide--search-code] w-5 h-5 text-amber-500" />
          </div>
        ...
        >
          <span class="icon-[lucide--check-circle] w-3 h-3 mr-1" />
          审查通过
        </Badge>
        <Badge
        ...
        >
          <span class="icon-[lucide--x-circle] w-3 h-3 mr-1" />
          审查未通过
        </Badge>
        <Badge
          v-else-if="isFailed"
    ...
    <div class="p-5 space-y-4">
      ...
      >
        <div class="text-center p-2 rounded-xl bg-red-500/5 border border-red-500/10">
        ...
        <div class="text-center p-2 rounded-xl bg-amber-500/5 border border-amber-500/10">
        ...
        <div class="text-center p-2 rounded-xl bg-primary/5 border border-primary/10">
      ...
      <!-- Summary -->
      <div v-if="reviewReport.summary" class="p-3 rounded-xl bg-muted/30 border border-border/30">
      ...
      <div v-if="mrReviews.length > 0" class="space-y-2">
        <Collapsible
          v-for="(mr, mrIdx) in mrReviews"
          :key="mrIdx"
          :open="expandedMRs[mrIdx] !== false"
        ...
        >
          <CollapsibleTrigger class="flex items-center justify-between w-full py-1.5 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors group">
            <span class="flex items-center gap-2">
              <span class="icon-[lucide--git-pull-request] w-4 h-4 text-amber-500" />
            ...
            <span
              ...
              :class="{ 'rotate-180': expandedMRs[mrIdx] !== false }"
            />
          </CollapsibleTrigger>
          <CollapsibleContent class="space-y-3 pt-2">
            <!-- Group by dimension -->
            <div
              v-for="(issues, dimension) in groupIssuesByDimension(mr.issues || [])"
              :key="dimension"
              class="space-y-1.5"
            >
              <!-- Dimension header -->
              <div class="flex items-center gap-1.5 text-xs font-medium text-muted-foreground py-1">
                <span class="w-3.5 h-3.5 text-amber-500" :class="[getDimensionMeta(dimension as string).icon]" />
              ...
              <!-- Issue cards -->
              <div
                v-for="(issue, issueIdx) in issues"
                :key="issueIdx"
              ...
              >
                ...
                >
                  ...
                  <div v-if="issue.file_path || issue.file" class="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <span class="icon-[lucide--file] w-3 h-3" />
                  ...
                  <!-- Suggestion -->
                  <div v-if="issue.suggestion || issue.fix" class="p-2 rounded-lg bg-emerald-500/5 border border-emerald-500/10">
                    <div class="flex items-center gap-1 text-xs font-medium text-emerald-600 mb-1">
                      <span class="icon-[lucide--lightbulb] w-3 h-3" />
                      建议修改
                    </div>
      ...
      <!-- Conclusion -->
      <div v-if="reviewReport.conclusion" class="p-3 rounded-xl bg-muted/30 border border-border/30">
        <div class="text-xs font-medium text-muted-foreground mb-1.5">
          结论
        </div>
        <p class="text-sm leading-relaxed whitespace-pre-wrap">
