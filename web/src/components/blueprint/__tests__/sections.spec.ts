...
describe('blueprintAssociationsSection —— ⭐ SC-4 反查（116-04 交付）', () => {
  ...
  it('9c. projectId 为空 ⇒ 关联项目块不渲染；非空 ⇒ 渲染站内跳转（正反并列）', () => {
    ...
    const withProject = mountWith(BlueprintAssociationsSection, { artifactId: 'a1', citations: {}, projectId: 'proj-1', projectName: '示例练习' })
    ...
    expect(withProject.text()).toContain('示例练习')
  })
})
