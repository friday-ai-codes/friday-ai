/**
 * GraphSearchModal — 仓库级 GraphRAG 关联搜索弹窗 component 测试（Phase）
 *
 * 覆盖：
 * 1. 输入查询触发搜索并展示命中片段（graphSearch 被调 + results 列表渲染）
 * 2. 切分支后搜索带正确 branch 参数（branch 经 prop 透传给 graphSearch， 红线）
...
const InputStub = defineComponent({
 ...
 template: `<input
 data-testid="query-input":value="modelValue"
...
describe('graphSearchModal', => {
 ...
 it('切分支后搜索带正确 branch 参数（ 红线）', async => {
