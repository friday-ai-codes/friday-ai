/**
 * — RepoHashFreshnessCard 单测（UI-SPEC ）
 *
 * 测试三态视觉契约：FRESH/STALE/UNKNOWN 文案、border-l-4 accent bar、
 * 按钮 loading 状态、behindCommits 显示、错误状态。
 *
 * 扩展（UI-SPEC GraphRAG 状态区段）：
...
// ：构造 GraphRAG IndexHistory fixture
...
describe('repoHashFreshnessCard', () => {
  ...
  it('b: STALE 态 — 显示"索引已过期"，alert-triangle 图标存在（UI-SPEC §11 border-l-4 由 grep 验证）', async () => {
    vi.mocked(repositoriesApi.get).mockResolvedValue(makeRepo({
    ...
    // STALE 态必须显示 alert-triangle 图标（UI-SPEC §5.2）
  ...
  it('e: STALE + behind_commits=null — 副文案为"本地与远端 HEAD 不一致"（D-17 降级）', async () => {
    vi.mocked(repositoriesApi.get).mockResolvedValue(makeRepo({
      remote_head_sha: 'def5678def5678',
      last_indexed_commit_sha: 'abc1234abc1234',
      behind_commits: null,
  ...
  // ==================== ：GraphRAG 状态区段 ====================
  // UI-SPEC §5.7：5 态 Badge / edge_count / payload_synced_at relative time
  describe('graphRAG 状态区段', () => {
    beforeEach(() => {
