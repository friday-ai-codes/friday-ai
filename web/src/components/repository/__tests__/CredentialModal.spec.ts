/**
 * CredentialModal — 凭证管理弹窗 component 测试（/02/03）
 *
 * 覆盖：
 *  1. 打开弹窗（无凭证）渲染 Access Token 表单 → 提交调用现有 API（setAccessToken mock）
 *     + emit('saved') + 关闭弹窗（CRED-01/02）
 *  2. 已有凭证时脱敏展示（••••）+ 认证类型徽标，不回显明文；点击「更新凭证」切到表单
 *  3. 保存失败：错误提示不含明文 Token（脱敏红线，T-297-01-01）
 *  4. 旧路由 redirect 守卫：credential.vue 声明 redirect → `#credential`（CRED-03，静态断言）
 *
...
const PLAINTEXT_TOKEN = 'ghp_super_secret_plaintext_0123456789'
...
describe('credentialModal', () => {
  beforeEach(() => {
    // 弹窗走 repositoriesStore.setAccessToken（同步 has_credential，code-review M-1）；
  ...
  it('旧路由 credential.vue 声明 redirect → #credential（CRED-03）', () => {
