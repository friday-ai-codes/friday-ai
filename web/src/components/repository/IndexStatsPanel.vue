...
const props = defineProps<{
  repositoryId: string
  // : 当前检索分支，切分支即时重拉统计（edge/图谱维度 branch-aware）
  branch?: string | null
}>()
  ...
  <div class="card">
    ...
    <div class="p-5">
      ...
      <div v-else-if="error" class="text-center py-6">
        <div class="inline-flex p-3 rounded-full bg-destructive/10 mb-3">
      ...
      <div v-else class="space-y-6">
        ...
        <div
          ...
          class="flex items-start gap-2 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30 text-xs text-amber-700 dark:text-amber-300"
        >
        ...
        <div v-if="stats.coverage_percent !== null" class="space-y-2">
          ...
          <Progress :model-value="stats.coverage_percent" class="h-2" />
        </div>
        ...
        <div v-if="Object.keys(stats.language_distribution).length > 0" class="space-y-3">
          ...
          <div class="flex items-center gap-6">
            <!-- 饼图 -->
            <svg viewBox="0 0 100 100" class="w-24 h-24 shrink-0">
              <path
                v-for="slice in getPieSlices(stats.language_distribution)"
                :key="slice.lang"
                :d="slice.path"
                :fill="slice.color"
                stroke="hsl(var(--card))"
            ...
            <div class="flex-1 space-y-1.5 min-w-0">
              <div
                v-for="slice in getPieSlices(stats.language_distribution).slice(0, 6)"
                :key="slice.lang"
