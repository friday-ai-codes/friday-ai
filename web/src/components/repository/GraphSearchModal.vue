...
/**
 * GraphSearchModal — 仓库级 GraphRAG 关联搜索弹窗（Phase）
 *
 * 消费 端点 `POST /repositories/{id}/graph-search/`：输入查询 → 展示 L3
...
<template>
 ...
 <span:class="isLoading
 ? 'icon-[lucide--loader-circle] animate-spin mr-1.5': 'icon-[lucide--search] mr-1.5'"
 />
 {{ isLoading ? '搜索中...': '搜索' }}
 </Button>
 </div>
 ...
 class="rounded-xl border border-border/50 bg-muted/20 text-center text-sm text-muted-foreground"
 >
 输入查询并搜索，查看命中片段与关联扩散图
 </div>
 <ul v-else class="space-y-2 max-h-[460px] overflow-y-auto pr-1">
 <li
 v-for="(item, idx) in result?.results ?? ":key="`${item.chunk_id}-${idx}`"
 class="rounded-xl border bg-card transition-colors":class="selectedChunkId === item.chunk_id
 ? 'border-primary/50 ring-1 ring-primary/30': 'border-border/50'"
 >
 ...
 >
 L{{ item.line_start }}<template v-if="item.line_end != null">–{{ item.line_end }}</template>
 </p>
 ...
 <GraphRAGDiffusionTab:hop1-neighbors="result?.hop1_neighbors ?? ":hop2-neighbors="result?.hop2_neighbors ?? ":source-chunks="extractSourceChunks(result)":loading="isLoading"
