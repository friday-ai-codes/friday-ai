      ...
      <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div class="relative w-full sm:max-w-md">
          ...
          <input
            ...
            class="w-full pl-8 pr-8 h-7 rounded-md border border-border/60 bg-background text-xs focus:outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/30 transition-colors"
          >
          <button
            v-if="searchInput"
            class="absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-muted/50 transition-colors"
            @click="clearSearch"
          >
          ...
          >
            ...
            <div class="font-mono">
              <template v-if="item.last_commit_sha">
                <a
                  v-if="gitUrl"
                  :href="commitUrl(item.last_commit_sha)"
                  target="_blank"
                  rel="noopener noreferrer"
                  class="text-primary hover:underline inline-flex items-center gap-1"
                  :title="item.last_commit_sha"
                >
