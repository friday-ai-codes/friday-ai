...
<template>
 <div class="glass-card rounded-2xl w-64 space-y-4">
 <!-- 标题 -->
 ...
 <!-- FPS -->
 <span
 class="font-mono text-xs font-bold":class="fpsBadgeClass(fps)"
 aria-live="polite":aria-label="`当前帧率 ${fps} FPS`"
 >
 {{ fps }} FPS
 </span>
 </div>
 ...
 <button
 class="text-xs px-2 py-0.5 rounded transition-colors":class="renderMode === 'force3d' ? 'bg-primary text-primary-foreground': 'text-muted-foreground hover:text-foreground'"
 @click="emit('update:render-mode', 'force3d')"
 >
 3D
 </button>
 <button
 class="text-xs px-2 py-0.5 rounded transition-colors":class="renderMode === 'echarts' ? 'bg-primary text-primary-foreground': 'text-muted-foreground hover:text-foreground'"
 @click="emit('update:render-mode', 'echarts')"
 >
 ECharts
 </button>
 ...
 <input
 id="galaxy-max-nodes"
 type="range":value="maxNodes"
 min="50"
 max="5000"
 step="50"
 class="w-full .5 accent-primary cursor-pointer":aria-valuemin="50":aria-valuemax="5000":aria-valuenow="maxNodes"
 aria-label="最大节点数量"
 ...
 <label
 v-for="n in NODE_TYPES":key="n.type"
 class="flex items-center gap-2 text-xs cursor-pointer hover:text-foreground transition-colors":class="activeNodeTypes.has(n.type) ? 'text-foreground/80': 'text-muted-foreground/40'"
 >
 <input
 type="checkbox":checked="activeNodeTypes.has(n.type)"
 class="accent-primary w-3 "
 ...
 <div class="grid grid-cols-1 gap-0.5">
 <label
 v-for="e in EDGE_TYPES":key="e.type"
 class="flex items-center gap-2 text-xs cursor-pointer hover:text-foreground transition-colors":class="activeEdgeTypes.has(e.type) ? 'text-foreground/80': 'text-muted-foreground/40'"
 >
 <input
 type="checkbox":checked="activeEdgeTypes.has(e.type)"
 class="accent-primary w-3 "
