...
// ============================================================================
// Galaxy → DiffusionEdgeType 映射（DRAWER-04）
// ============================================================================
...
<template>
  ...
  >
    <SheetContent
      side="right"
      class="min-w-[480px] max-w-[640px] p-0 bg-[#0a0a1f]/95 backdrop-blur-xl border-l border-white/10 flex flex-col"
    >
      <!-- Header -->
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="mt-0.5 shrink-0">
            <span
              ...
              :class="nodeDetail ? typeColor(nodeDetail.node.type) : 'bg-white/10 text-white/30 border-white/10'"
            >
      ...
      >
          ...
          <ScrollArea class="h-full">
            <div class="px-6 py-4 space-y-6">
              ...
              <dl class="grid grid-cols-2 gap-x-4 gap-y-3">
                ...
                <div>
                  ...
                  <dd class="mt-1">
                    <span
                      class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border"
                      :class="typeColor(nodeDetail.node.type)"
                    >
              ...
              <div v-if="nodeDetail.node.metadata && Object.keys(nodeDetail.node.metadata).length > 0">
                ...
                >
                  <span
                    class="icon-[lucide--chevron-right] text-sm transition-transform"
                    :class="metadataExpanded ? 'rotate-90' : ''"
                  />
                  元数据
                </button>
                <div
                  v-if="metadataExpanded"
                  class="mt-2 p-3 rounded-lg bg-white/5 border border-white/10"
                >
          ...
          <div class="h-[440px] relative">
            <Suspense v-if="activeTab === 'graph'">
              <GraphRAGDiffusionTab
                :hop1-neighbors="toNeighborMetadata(nodeDetail.neighbors)"
                :hop2-neighbors="[]"
                :source-chunks="toSourceChunks(nodeDetail.node)"
                :loading="loading"
        ...
        >
          <ReferencesList
            :called-by="nodeDetail.called_by"
            :calls="nodeDetail.references"
