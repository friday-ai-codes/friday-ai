...
watch( => props.nodes, (nodes) => {
 if (nodes && nodes.length > 0) setCorpus(nodes)
}, { immediate: true })
watch(localQuery, (q) => {
...
const clampedIndex = computed( =>
 ...
 ? Math.min(selectedIndex.value, results.value.length - 1): 0,
)
...
<template>
 ...
 role="option":aria-selected="idx === clampedIndex"
 class="flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors":class="idx === clampedIndex ? 'bg-white/10': 'hover:bg-white/5'"
 @click="selectNode(item)"
 @mouseenter="selectedIndex = idx"
 >
 <!-- 类型 badge -->
 <span
 class="shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border":class="typeColor(item.type)"
 >
