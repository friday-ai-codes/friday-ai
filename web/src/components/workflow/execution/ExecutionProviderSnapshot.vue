...
/**
 * ExecutionProviderSnapshot — UX-02 D-09 Execution 详情页 AI 节点 Provider 快照
 *
 * Props:
 ...
 *
 * 设计要点：
 *   - Clean Card（`.card p-6`）风格，严禁玻璃拟态光晕层（UI-SPEC L33 Clean Card 纠偏）
 *   - 折叠态（默认）：显示 provider_type 中文名 + model；展开态：source + Replay 按钮
 *   - snapshot=null：渲染 warning Badge + 说明文案 + Replay disabled + tooltip 三重承载
 * （title / aria-label / TooltipContent；happy-dom 下 Portal 不挂 DOM，沿用 模式）
 *
...
/** Provider 中文名映射（UI-SPEC §Copywriting L285） */
...
/** source layer 中文映射（UI-SPEC §Copywriting L287） */
...
<template>
  <section class="card p-6 space-y-4" role="region" aria-label="Provider 快照">
    <!-- 标题 + 折叠按钮 -->
    <header class="flex items-center justify-between">
      ...
      <Button
        ...
        :aria-controls="`snap-detail-${nodeId}`"
    ...
    <div
      v-show="expanded && snapshot"
      :id="`snap-detail-${nodeId}`"
      class="pt-2 border-t border-border/40"
    >
      <!-- ：四层 Provider 解析 Inspector（snapshot 带 chain 时渲染） -->
      <div
        ...
        <Tooltip>
          <TooltipTrigger as-child>
            <Button
              ...
              :title="replayTooltipText"
              :aria-label="`使用此快照重放（${replayTooltipText}）`"
              @click="onReplay"
            >
              使用此快照重放
            </Button>
