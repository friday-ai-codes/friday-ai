...
<template>
  <div class="agent-execution-log space-y-4">
    ...
    <div v-if="hasIterations" class="space-y-3">
      ...
      <div
        v-for="(iteration, index) in parsedOutput.iterations"
        :key="index"
          ...
          <div v-if="iteration.think" class="flex gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <Collapsible>
                ...
                <CollapsibleContent>
                  <div class="mt-2 text-sm text-foreground whitespace-pre-wrap bg-muted/30 rounded-lg p-3">
                    {{ iteration.think }}
                  </div>
                </CollapsibleContent>
              </Collapsible>
          ...
          <div v-if="iteration.act" class="flex gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                ...
                <component
                  :is="getStatusIcon(iteration.act.status)"
                  class="w-3.5 h-3.5" :class="[getStatusColor(iteration.act.status), iteration.act.status === 'running' && 'animate-spin']"
                />
              ...
              <Collapsible>
                ...
                <CollapsibleContent>
                  <div class="mt-2 space-y-2">
                    ...
                    <div v-if="iteration.act.output !== undefined" class="bg-muted/30 rounded-lg p-3">
                      <div class="text-xs font-medium text-muted-foreground mb-1">
                        输出
                      </div>
                    ...
                    <div v-if="iteration.act.error" class="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
          ...
          <div v-if="iteration.observe" class="flex gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <Collapsible>
                ...
                <CollapsibleContent>
                  <div class="mt-2 text-sm text-foreground whitespace-pre-wrap bg-muted/30 rounded-lg p-3 overflow-x-auto">
                    {{ iteration.observe }}
                  </div>
                </CollapsibleContent>
              </Collapsible>
      ...
      <div class="relative">
        ...
        <div class="space-y-3">
          ...
          >
            <!-- Timeline Dot -->
            <div
              class="relative z-10 shrink-0 w-8 h-8 rounded-full flex items-center justify-center border-2" :class="[tool.status === 'success' && 'bg-emerald-500/20 border-emerald-500', tool.status === 'error' && 'bg-red-500/20 border-red-500', tool.status === 'running' && 'bg-primary/20 border-primary']"
            >
              <component
                :is="getStatusIcon(tool.status)"
                class="w-4 h-4" :class="[getStatusColor(tool.status), tool.status === 'running' && 'animate-spin']"
              />
            </div>
            ...
            <div class="flex-1 bg-muted/30 rounded-lg p-3">
              ...
              <Collapsible :open="expandedTools.has(tool.id)" @update:open="toggleTool(tool.id)">
                <CollapsibleTrigger class="group flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors">
                  <component
                    :is="expandedTools.has(tool.id) ? ChevronDown : ChevronRight"
                    class="w-3 h-3"
                  />
                  查看详情
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <div class="mt-2 space-y-2">
                    <div class="bg-card/50 rounded-lg p-2">
                      <div class="text-xs font-medium text-muted-foreground mb-1">
                        输入
                      </div>
                      <pre class="text-xs overflow-x-auto">{{ JSON.stringify(tool.input, null, 2) }}</pre>
                    </div>
                    <div v-if="tool.output !== undefined" class="bg-card/50 rounded-lg p-2">
                      <div class="text-xs font-medium text-muted-foreground mb-1">
                        输出
                      </div>
                    ...
                    <div v-if="tool.error" class="bg-red-500/10 border border-red-500/30 rounded-lg p-2">
                      <div class="text-xs font-medium text-red-500 mb-1">
                        错误
                      </div>
    ...
    <div
      ...
      class="bg-card/80 backdrop-blur-sm border border-emerald-500/30 rounded-xl p-4"
    >
      <div class="flex items-start gap-3">
