      ...
      <ScrollArea class="h-64">
        <div class="p-2 space-y-1">
          <div v-for="category in displayVariables" :key="category.category">
            <button
              class="flex items-center gap-2 w-full p-2 hover:bg-accent rounded-md text-left"
              @click="toggleCategory(category.category)"
            >
              <component
                :is="expandedCategories[category.category] ? ChevronDown : ChevronRight"
                class="w-4 h-4"
              />
            ...
            <div v-if="expandedCategories[category.category]" class="ml-6 space-y-0.5">
              <button
                ...
                :key="item.path"
                class="flex items-center justify-between w-full p-1.5 hover:bg-accent rounded text-left group"
          ...
          <div v-if="displayVariables.length === 0" class="py-8 text-center">
            <Variable class="w-8 h-8 mx-auto text-muted-foreground/30" />
