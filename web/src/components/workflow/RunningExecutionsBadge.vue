  ...
  <Popover v-model:open="popoverOpen">
    <PopoverTrigger as-child>
      <Button
        ...
        class="h-8 relative"
      >
        <span class="icon-[lucide--activity] w-4 h-4 mr-1 text-emerald-500" />
        <span class="text-xs">运行中</span>
        <Badge
          class="absolute -top-1 -right-1 h-4 min-w-4 px-1 text-[10px] bg-emerald-500 text-white border-0 animate-pulse"
        >
          {{ runningCount }}
        </Badge>
      </Button>
    ...
    <PopoverContent align="end" class="w-72 p-0 rounded-xl bg-card/95 backdrop-blur-md border border-border/50">
        ...
        >
          <div class="flex items-center gap-2 min-w-0">
            <span
              ...
              :class="{ 'bg-primary animate-pulse': exec.status === 'running', 'bg-amber-500': exec.status === 'pending' || exec.status === 'queued' }"
            />
            <span class="text-xs text-muted-foreground truncate">
              {{ formatTime(exec.created_at) }}
            </span>
          </div>
          <span class="icon-[lucide--chevron-right] w-3.5 h-3.5 text-muted-foreground/40 group-hover:text-muted-foreground transition-colors" />
        </div>
