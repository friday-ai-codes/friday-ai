      ...
      <div class="p-5 space-y-4">
        ...
        <div v-if="task.pr_url || task.status === 'partial_success'" class="space-y-3">
          <h4 class="text-sm font-medium flex items-center gap-2">
            <span class="icon-[lucide--git-pull-request] w-4 h-4 text-primary" />
            Merge Request
          </h4>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            class="block p-4 rounded-xl border border-emerald-200 dark:border-emerald-800 hover:from-emerald-500/20 hover:to-teal-500/10 transition-all duration-300 group"
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                <div class="p-2 rounded-lg bg-emerald-500/20">
                  <span class="icon-[lucide--git-pull-request] w-5 h-5 text-emerald-600" />
                </div>
                <div>
              ...
              <span
                class="icon-[lucide--external-link] w-4 h-4 text-emerald-500 group-hover:translate-x-1 transition-transform"
              />
            </div>
            ...
            <div
              ...
              class="mt-3 p-2 rounded-lg bg-amber-100 dark:bg-amber-900/30 flex items-center gap-2"
            >
          ...
          >
            <div class="flex items-start gap-3">
              <div class="p-2 rounded-lg bg-amber-500/20">
                <span class="icon-[lucide--alert-triangle] w-5 h-5 text-amber-500" />
              </div>
              <div class="flex-1">
                ...
                <div v-if="task.branch_name" class="mt-3 p-2 rounded-lg bg-card/50">
        ...
        <div class="space-y-2">
          ...
          <pre class="text-xs bg-secondary p-3 rounded-md overflow-x-auto whitespace-pre-wrap">{{ task.prompt }}</pre>
        </div>
        ...
        <div v-if="task.plan_output" class="space-y-2">
          ...
          <pre class="text-xs bg-secondary p-3 rounded-md overflow-x-auto whitespace-pre-wrap max-h-60">{{ task.plan_output }}</pre>
        </div>
        ...
        <div v-if="task.error_message" class="space-y-2">
          ...
          <pre class="text-xs bg-destructive/10 text-destructive p-3 rounded-md overflow-x-auto whitespace-pre-wrap">{{ task.error_message }}</pre>
        </div>
        ...
        <div v-if="task.human_feedback" class="space-y-2">
          ...
          <pre class="text-xs bg-secondary p-3 rounded-md overflow-x-auto whitespace-pre-wrap">{{ task.human_feedback }}</pre>
        </div>
