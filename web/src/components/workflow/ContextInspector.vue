    ...
    <div class="px-5 py-3.5 border-b border-border/50 flex items-center gap-2">
      ...
      <div v-if="context" class="flex items-center gap-2 mt-2">
        <span
          ...
          :class="{ 'bg-primary/10 text-primary': context.status === 'running', 'bg-emerald-500/10 text-emerald-700': context.status === 'completed', 'bg-red-500/10 text-red-700': context.status === 'failed', 'bg-muted text-muted-foreground': !['running', 'completed', 'failed'].includes(context.status) }"
        >
          {{ context.status }}
        </span>
      ...
      <div class="p-5 space-y-4">
        ...
        <template v-else>
          ...
          <Collapsible v-if="getEntries(context?.trigger_data || {}).length > 0" :open="expandedSections.trigger">
            <CollapsibleTrigger class="flex items-center gap-2 w-full p-2 hover:bg-accent rounded-md" @click="toggleSection('trigger')">
              <component :is="expandedSections.trigger ? ChevronDown : ChevronRight" class="w-4 h-4" />
            ...
            <CollapsibleContent>
              <div class="ml-6 pl-4 border-l space-y-1 py-2">
                <div
                  v-for="[key, value] in getEntries(context?.trigger_data || {})"
                  :key="key"
                  class="group flex items-start gap-2 text-xs hover:bg-accent/50 rounded p-1"
                >
          ...
          <Collapsible v-if="getEntries(context?.global_params || {}).length > 0" :open="expandedSections.global">
            <CollapsibleTrigger class="flex items-center gap-2 w-full p-2 hover:bg-accent rounded-md" @click="toggleSection('global')">
              <component :is="expandedSections.global ? ChevronDown : ChevronRight" class="w-4 h-4" />
            ...
            <CollapsibleContent>
              <div class="ml-6 pl-4 border-l space-y-1 py-2">
                <div
                  v-for="[key, value] in getEntries(context?.global_params || {})"
                  :key="key"
                  class="group flex items-start gap-2 text-xs hover:bg-accent/50 rounded p-1"
                >
          ...
          <Collapsible v-if="nodeOutputs.length > 0" :open="expandedSections.nodes">
            <CollapsibleTrigger class="flex items-center gap-2 w-full p-2 hover:bg-accent rounded-md" @click="toggleSection('nodes')">
              <component :is="expandedSections.nodes ? ChevronDown : ChevronRight" class="w-4 h-4" />
              ...
              <div class="ml-6 pl-4 border-l space-y-3 py-2">
                <div v-for="[nodeId, outputs] in nodeOutputs" :key="nodeId" class="space-y-1">
                  ...
                  <div class="ml-2 space-y-1">
                    <div
                      v-for="[key, value] in getEntries(outputs)"
                      :key="key"
                      class="group flex items-start gap-2 text-xs hover:bg-accent/50 rounded p-1"
                    >
          ...
          <Collapsible v-if="getEntries(context?.input_data || {}).length > 0" :open="expandedSections.input">
            <CollapsibleTrigger class="flex items-center gap-2 w-full p-2 hover:bg-accent rounded-md" @click="toggleSection('input')">
              <component :is="expandedSections.input ? ChevronDown : ChevronRight" class="w-4 h-4" />
            ...
            <CollapsibleContent>
              <div class="ml-6 pl-4 border-l space-y-1 py-2">
                <div
                  v-for="[key, value] in getEntries(context?.input_data || {})"
                  :key="key"
                  class="group flex items-start gap-2 text-xs hover:bg-accent/50 rounded p-1"
                >
