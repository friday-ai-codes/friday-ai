...
<template>
  ...
  <Collapsible v-if="hasPredecessor" :open="inputSchemaOpen" class="mt-4" @update:open="emit('update:inputSchemaOpen', $event)">
    ...
    <CollapsibleTrigger class="flex items-center justify-between w-full group">
      ...
      <span
        class="icon-[lucide--chevron-down] text-muted-foreground transition-transform duration-200"
        :class="{ 'rotate-180': inputSchemaOpen }"
      />
    </CollapsibleTrigger>
    <CollapsibleContent class="mt-3">
      <div class="rounded-xl bg-muted/30 border border-border/50 overflow-hidden">
        <div
          v-for="(input, idx) in directPredecessorOutputs"
          :key="`${input.nodeId}-${input.fieldName}`"
          class="px-3 py-2.5 text-xs"
          :class="{ 'border-t border-border/50': Number(idx) > 0 }"
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <span
                ...
                :class="getPortTypeColor(input.type)"
              >
                {{ input.type }}
              </span>
            </div>
  ...
  <Collapsible v-if="nodeTypeInfo?.outputs?.length" :open="outputSchemaOpen" class="mt-4" @update:open="emit('update:outputSchemaOpen', $event)">
    ...
    <CollapsibleTrigger class="flex items-center justify-between w-full group">
      ...
      <span
        class="icon-[lucide--chevron-down] text-muted-foreground transition-transform duration-200"
        :class="{ 'rotate-180': outputSchemaOpen }"
      />
    </CollapsibleTrigger>
    <CollapsibleContent class="mt-3">
      <div class="rounded-xl bg-muted/30 border border-border/50 overflow-hidden">
        <template v-for="(output, idx) in nodeTypeInfo.outputs" :key="output.name">
          ...
          <template v-if="output.schema?.properties">
            <div
              v-for="(propSchema, propKey, propIdx) in output.schema.properties"
              :key="`${output.name}-${propKey}`"
              class="px-3 py-2 text-xs"
              :class="{ 'border-t border-border/50': Number(idx) > 0 || Number(propIdx) > 0 }"
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  ...
                  <span
                    class="text-[10px] px-1.5 py-0.5 rounded-full bg-muted"
                    :class="getPortTypeColor(propSchema.type)"
                  >
                    {{ propSchema.type }}
                  </span>
                </div>
            ...
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  ...
                  <span
                    class="text-[10px] px-1.5 py-0.5 rounded-full bg-muted"
                    :class="getPortTypeColor(output.type)"
                  >
                    {{ output.type }}
                  </span>
                </div>
