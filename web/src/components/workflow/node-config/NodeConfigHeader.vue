    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-1">
        <Button
          ...
          variant="ghost"
          size="sm"
          class="h-8 hover:bg-muted/50 text-muted-foreground hover:text-foreground"
          @click="emit('autoFill')"
        >
          <Wand2 class="w-4 h-4 mr-1.5" />
          自动填充
        </Button>
        <Button variant="ghost" size="icon" class="h-8 w-8 hover:bg-muted/50" @click="emit('close')">
          <X class="w-4 h-4" />
        </Button>
      </div>
    </div>
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger as-child>
            <button
              type="button"
              class="inline-flex items-center justify-center w-5 h-5 rounded hover:bg-muted/50 transition-colors"
              @click="copyNodeId"
            >
              <Check v-if="idCopied" class="w-3 h-3 text-green-500" />
