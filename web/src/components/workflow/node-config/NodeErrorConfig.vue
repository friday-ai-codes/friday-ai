...
<template>
 ...
 <AlertTriangle class=" w-4 text-muted-foreground" />
 <span>错误处理</span>
 </div>
 <ChevronDown
 class=" w-4 text-muted-foreground transition-transform duration-200":class="{ 'rotate-180': isOpen }"
 />
 </CollapsibleTrigger>
 ...
 <SelectContent>
 <SelectItem
 v-for="opt in strategyOptions":key="opt.value":value="opt.value"
 >
 <div class="flex items-center gap-2">
 <component:is="opt.icon" class=".5 w-3.5" />
 <span>{{ opt.label }}</span>
 </div>
 </SelectItem>
 </SelectContent>
 ...
 <Input
 type="number":model-value="retryTimes"
 min="0"
 max="10"
 placeholder="0"
 class=""
 ...
 <Input
 type="number":model-value="retryDelay"
 min="1"
 max="300"
 placeholder="5"
 class=""
 ...
 <Input
 type="number":model-value="nodeTimeoutSeconds ?? ''"
 min="1"
 placeholder="不限制"
 class=""
