...
<template>
  ...
  >
    ...
    <div class="px-6 pt-4 shrink-0">
      <div class="flex gap-1 p-1 rounded-xl bg-muted/50 border border-border/30">
        <button
          type="button"
          class="flex-1 px-3 py-2 text-sm font-medium rounded-lg transition-all"
          :class="activeTab === 'blank' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'"
        ...
        <button
          type="button"
          class="flex-1 px-3 py-2 text-sm font-medium rounded-lg transition-all"
          :class="activeTab === 'import' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'"
      ...
      <template v-if="activeTab === 'blank'">
        ...
        <div v-if="templates.length > 0 || templatesLoading" class="space-y-2">
          ...
          <div class="grid grid-cols-2 gap-2">
            ...
            <button
              type="button"
              class="flex flex-col items-start gap-1.5 p-3 rounded-xl border text-left transition-all duration-150"
              :class="[selectedTemplateId === null ? 'border-primary bg-primary/5 ring-1 ring-primary/30' : 'border-border/50 hover:border-border hover:bg-muted/30']"
              @click="selectTemplate(null)"
            >
              <div class="flex items-center gap-2">
                <span
                  class="icon-[lucide--file-plus] text-base"
                  :class="selectedTemplateId === null ? 'text-primary' : 'text-muted-foreground'"
                />
            ...
            <button
              ...
              type="button"
              class="flex flex-col items-start gap-1.5 p-3 rounded-xl border text-left transition-all duration-150"
              :class="[selectedTemplateId === tpl.template_id ? 'border-primary bg-primary/5 ring-1 ring-primary/30' : 'border-border/50 hover:border-border hover:bg-muted/30']"
              @click="selectTemplate(tpl.template_id)"
            >
              <div class="flex items-center gap-2">
                <span
                  class="icon-[lucide--workflow] text-base"
                  :class="selectedTemplateId === tpl.template_id ? 'text-primary' : 'text-muted-foreground'"
                />
      ...
      <template v-if="activeTab === 'import'">
        <div class="space-y-3">
          ...
          >
            ...
            >
            <span
              class="icon-[lucide--upload] text-2xl mx-auto mb-2 block"
              :class="importErrors.length > 0 ? 'text-destructive' : importPreview ? 'text-green-500' : 'text-muted-foreground'"
            />
          ...
          <div v-if="importWarnings.length > 0" class="space-y-1">
            <div
              v-for="(warn, idx) in importWarnings"
              :key="idx"
          ...
          <!-- Preview -->
          <div v-if="importPreview" class="rounded-xl border border-border/50 bg-muted/20 p-3 space-y-2">
            ...
            <div v-if="importPreview.nodes.length > 0" class="flex flex-wrap gap-1">
              <span
                v-for="nodeType in [...new Set(importPreview.nodes.map((n: any) => n.node_type).filter(Boolean))]"
                :key="nodeType"
