/**
 * — ExecutionProviderSnapshot 组件测试。
 *
 * 覆盖 UX-02 D-09 详情页快照展示契约：
 ...
 *
 * Analog: ProviderCredentialMissingCard.spec.ts+
 *   Pattern 2 L242-246（ExecutionProviderSnapshot analog）。
 */
...
describe('executionProviderSnapshot', () => {
  ...
  it('d: snapshot = null → Replay disabled + tooltip 文案 "历史 Execution 未记录快照"（title/aria-label 承载）', () => {
    ...
    // title / aria-label / wrapper.html 承载（与 一致的 模式）
