  ...
  <div :class="cn('flex flex-col gap-4', compact ? 'text-sm' : '')">
    ...
    <div class="space-y-3">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <div
            :class="cn('p-1.5 rounded-md border', getExecutionStatus(execution.status).bg, getExecutionStatus(execution.status).border)"
          >
            <span :class="cn('w-4 h-4', `icon-[${getExecutionStatus(execution.status).icon}]`, getExecutionStatus(execution.status).color, getExecutionStatus(execution.status).animate && 'animate-spin')" />
          </div>
      ...
      <div class="space-y-1.5">
        ...
        <Progress
          ...
          class="h-2"
          :class="cn(execution.status === 'failed' && '[&>div]:bg-red-500', execution.status === 'completed' && '[&>div]:bg-green-500', execution.status === 'partial_success' && '[&>div]:bg-amber-500')"
        />
      </div>
    </div>
    ...
    <div class="flex flex-col gap-2 overflow-y-auto max-h-[calc(100vh-12rem)] pr-1">
      ...
      >
        ...
        <div
          class="group flex items-center gap-3 p-2 cursor-pointer hover:bg-accent rounded-lg"
          @click="emit('selectNode', node)"
        >
          <!-- Node Status Icon -->
          <div
            :class="cn('flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center border', getExecutionStatus(node.status).bg, getExecutionStatus(node.status).border)"
          >
            <span :class="cn('w-3.5 h-3.5', `icon-[${getExecutionStatus(node.status).icon}]`, getExecutionStatus(node.status).color, getExecutionStatus(node.status).animate && 'animate-spin')" />
          </div>
