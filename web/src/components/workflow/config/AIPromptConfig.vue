...
// 若未来需要,可参考 action 步骤 2(5) 扩展,读 currentModelMeta.supports_reasoning
...
<template>
  <div class="space-y-4">
    ...
    <!-- 模型(按 credential 维度 + capability 过滤;UI-06) -->
    <div class="space-y-2">
      <Label class="font-normal">
        模型
      </Label>
    ...
    <div class="space-y-2">
      ...
      <Select v-model="outputFormat">
        ...
        <SelectContent>
          <SelectItem
            v-for="option in OUTPUT_FORMATS"
            :key="option.value"
            :value="option.value"
          >
            {{ option.label }}
    ...
    <Collapsible v-model:open="advancedOpen">
      <CollapsibleTrigger class="flex items-center gap-2 w-full py-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
        <span
          class="icon-[lucide--chevron-right] h-4 w-4 transition-transform duration-200"
          :class="{ 'rotate-90': advancedOpen }"
        />
        高级设置
      </CollapsibleTrigger>
      <CollapsibleContent class="space-y-4 pt-2">
        <!-- 最大思考 Token 数(D-14:非 Anthropic 禁用 + tooltip) -->
        <TooltipProvider :delay-duration="100">
          <Tooltip>
            <TooltipTrigger as-child>
              <div
                class="space-y-2"
                :class="{ 'opacity-60': !thinkingSupported }"
              >
    ...
    <MarkdownEditorModal
      ...
      v-model="systemPrompt"
      title="编辑系统提示词"
      description="定义 AI 的角色、能力范围和输出要求"
      :workflow-nodes="workflowNodes"
      :workflow-edges="workflowEdges"
      :current-node-id="currentNodeId"
      placeholder="设定 AI 的角色和行为规范..."
