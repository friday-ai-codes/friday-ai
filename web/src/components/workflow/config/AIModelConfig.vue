...
<template>
 ...
 <span class="icon-[lucide--info] w-3.5 .5" />
 <span>
 ...
 <Button
 variant="outline"
 size="sm":disabled="loadingModels || !localApiBaseUrl"
 @click="fetchCustomModels"
 >
 <span
 v-if="loadingModels"
 class="icon-[lucide--loader-2] w-4 mr-1 animate-spin"
 />
 <span v-else class="icon-[lucide--refresh-cw] w-4 mr-1" />
 查询模型
 </Button>
 </div>
 <p class="text-xs text-muted-foreground">
 ...
 <SelectContent>
 <SelectItem
 v-for="option in modelOptions":key="option.value":value="option.value"
 >
 {{ option.label }}
 </SelectItem>
 </SelectContent>
 ...
 <Button
 variant="outline"
 size="icon":disabled="loadingModels || (localUseCustomApi && !localApiBaseUrl) || (!localUseCustomApi && !systemConfig?.has_api_key)":title="localUseCustomApi ? '从自定义 API 获取模型': '从系统 API 获取模型'"
 @click="localUseCustomApi ? fetchCustomModels: fetchSystemModels"
 >
 <span
 v-if="loadingModels"
 class="icon-[lucide--loader-2] w-4 animate-spin"
 />
 <span v-else class="icon-[lucide--refresh-cw] w-4 " />
 </Button>
 </div>
