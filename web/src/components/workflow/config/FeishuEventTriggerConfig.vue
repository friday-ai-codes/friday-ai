...
<template>
 ...
 <div class="space-y-1 rounded-xl border border-border/50 bg-background/30">
 <label
 v-for="option in FEISHU_EVENT_TYPE_OPTIONS":key="option.value":for="`event-${option.value}`"
 class="flex items-start gap-3 rounded-lg border transition-colors cursor-pointer":class="eventType === option.value ? 'bg-primary/5 border-primary/20': 'border-transparent hover:bg-muted/50'"
 >
 <input:id="`event-${option.value}`"
 v-model="eventType"
 ...
 <div class=" rounded-xl border border-border/50 bg-background/30">
 <label
 for="work-item-story"
 class="flex items-center gap-3 rounded-lg border transition-colors cursor-pointer":class="workItemType === 'story' ? 'bg-primary/5 border-primary/20': 'border-transparent hover:bg-muted/50'"
 >
 <input
 id="work-item-story"
 ...
 <!-- 其他类型 - 即将支持 -->
 <div class="flex items-center gap-3 rounded-lg opacity-50 cursor-not-allowed">
 ...
 <Badge
 v-for="status in (filterStatus.value.value as string)":key="status"
 variant="secondary"
 ...
 <button
 type="button"
 class="hover:bg-muted rounded-full .5"
 ...
 type="button"
 class="w-full flex items-center gap-2 rounded-lg text-left text-sm hover:bg-muted/50 transition-colors":class="{ 'bg-primary/10': filterStatus.includes(status.value) }"
 @click="filterStatus.toggle(status.value, !filterStatus.includes(status.value))"
 >
 <span
 class="w-4 rounded border flex items-center justify-center transition-colors":class="filterStatus.includes(status.value) ? 'bg-primary border-primary text-primary-foreground': 'border-border'"
 >
 ...
 <button
 type="button"
 class="w-full flex items-center gap-2 rounded-lg text-left text-sm hover:bg-muted/50 transition-colors"
 ...
 <!-- 显示前2个标签 -->
 <Badge
 v-for="project in selectedProjects.slice(0, 2)":key="project.id"
 variant="secondary"
 class="text-xs gap-1 pr-1"
 >
 {{ project.name }}
 ...
 type="button"
 class="w-full flex items-center gap-2 rounded-lg text-left text-sm hover:bg-muted/50 transition-colors":class="{ 'bg-primary/10': projectIds.includes(project.id) }"
 @click="toggleProject(project.id)"
 >
 <span
 class="w-4 rounded border flex items-center justify-center transition-colors":class="projectIds.includes(project.id) ? 'bg-primary border-primary text-primary-foreground': 'border-border'"
 >
 ...
 <template v-else>
 <Badge
 v-for="project in excludedProjects.slice(0, 2)":key="project.id"
 variant="destructive"
 class="text-xs gap-1 pr-1"
 >
 {{ project.name }}
 <button
 type="button"
 class="hover:bg-red-600 rounded-full .5"
 ...
 <template v-else>
 <button
 v-for="project in filteredExcludeProjects":key="project.id"
 type="button"
 class="w-full flex items-center gap-2 rounded-lg text-left text-sm hover:bg-muted/50 transition-colors":class="{ 'bg-red-500/10': excludeProjectIds.includes(project.id) }"
 @click="toggleExcludeProject(project.id)"
 >
 <span
 class="w-4 rounded border flex items-center justify-center transition-colors":class="excludeProjectIds.includes(project.id) ? 'bg-red-500 border-red-500 text-white': 'border-border'"
 >
