 ...
 * 增强展示 (D-13/D-14/D-15)：详细文件列表 Collapsible + 工具调用摘要 + 向后兼容旧版 payload。
 */
import { ref } from 'vue'
...
<template>
  <div class="card mt-2 animate-fade-in">
    ...
    <div class="p-4 space-y-2">
      <TransitionGroup name="step">
        ...
        >
          ...
          <!-- 步骤文字 -->
          <span
            :class="{ 'text-muted-foreground': step.status === 'pending', 'text-foreground font-semibold': step.status === 'running', 'text-muted-foreground line-through': step.status === 'done' }"
          >
            {{ step.name }}
          </span>
        </div>
    ...
    <!-- 底部：摘要统计 + 详细文件列表 + 工具调用 (D-13/D-14/D-15) -->
