 ...
 *
 * 嵌入冲突警告（D-16: Warning Alert，建议性质不阻断操作 D-17）。
 * 顶部展示已完成确认步骤折叠摘要（D-04）。
 */
...
// commit message 字符数（D-09: 最大 5000）
  ...
  <div class="card mt-2 animate-fade-in">
    ...
    <div class="p-4 space-y-3">
      ...
      <div v-if="completedSteps.length > 0" class="space-y-1 mb-3">
        <div
          v-for="(step, idx) in completedSteps"
          :key="idx"
      ...
      <div
        ...
        class="p-3 rounded-lg border border-amber-500/30 bg-amber-500/5 mb-3"
      >
          ...
          <div class="space-y-1 min-w-0">
            <p class="text-sm font-medium text-foreground">
              {{ conflictData.behind_by
                ? `检测到潜在合并冲突（落后 ${conflictData.behind_by} 个提交）`
                : '检测到潜在合并冲突'
              }}
            </p>
            ...
            >
              <Badge
                v-for="file in conflictData.conflicting_files"
                :key="file"
                variant="warning"
                class="text-xs"
              >
      ...
      <div class="space-y-3">
        ...
        <p
          ...
          :class="charCount > 5000 ? 'text-destructive' : 'text-muted-foreground'"
        >
          {{ charCount }}/5000
        </p>
      </div>
