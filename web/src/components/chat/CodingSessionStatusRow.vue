...
const props = defineProps<{
  ...
  repoGitUrl: string
  /**
   * ：可选 conversationId 用于在仓库名旁渲染 RelevanceBadge。
   * 不传则不渲染（向后兼容既有调用方）。
   */
...
<template>
  <div class="flex flex-wrap items-center gap-2 text-xs py-1.5 border-b border-border/30 last:border-b-0">
    ...
    <a
      ...
      :aria-label="`打开仓库 ${session.repository_name}`"
    >
    ...
    <!-- ：相关性徽章（store 无 trace 时优雅降级不渲染） -->
    <RelevanceBadge
      v-if="effectiveConversationId"
      :repository-id="session.repository_id"
    ...
    <a
      ...
      :aria-label="`打开分支 ${session.branch_name}`"
    >
    ...
    <span v-if="session.commit_sha" class="flex items-center gap-1 text-muted-foreground">
      ...
      <button
        ...
        :aria-label="`复制 commit sha ${session.commit_sha}`"
        @click="copyCommitSha"
      >
    ...
    <!-- PR 链（completed） -->
    <a
      v-if="isCompleted && session.pr_url"
      :href="session.pr_url"
      target="_blank"
      rel="noopener noreferrer"
    ...
    <span
      ...
      class="flex items-center gap-1 text-destructive truncate max-w-xs"
    ...
    <Button
      ...
      size="sm"
      class="ml-auto"
      :aria-label="`重试 ${session.repository_name} 的编码会话`"
      @click="handleRetry"
    >
