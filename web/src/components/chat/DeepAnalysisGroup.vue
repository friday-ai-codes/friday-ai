...
<template>
  <!-- 单个：直接渲染 -->
  <DeepAnalysisCard
    v-if="!isMulti && items.length === 1"
    :session="items[0].session"
    :task-label="items[0].taskLabel"
  ...
  >
    ...
    <div class="dag-tabs">
      <button
        ...
        :key="item.session.session_id || i"
        type="button"
        class="dag-tab"
        :class="{ 'is-active': i === activeIndex }"
        :title="item.taskLabel || item.session.task_description"
        @click="goTo(i)"
      >
        <span
          class="dag-tab-dot"
          :class="item.status === 'running' ? 'dag-tab-dot--running' : 'dag-tab-dot--done'"
        />
