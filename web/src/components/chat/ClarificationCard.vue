...
<template>
 ...
 <button
 v-for="opt in payload.options":key="opt.id"
 type="button"
 role="radio":aria-checked="selectedId === opt.id":disabled="isAnswered || submitting"
 class="w-full text-left rounded-lg border px-3 py-2 transition-colors disabled:cursor-not-allowed disabled:opacity-60":class="[
 selectedId === opt.id
 ...
 <span
 class="mt-0.5 size-4 shrink-0 rounded-full border-2 transition-colors":class="[
 selectedId === opt.id
 ...
 <Textarea
 v-model="freeformText":disabled="submitting"
 rows="2"
 ...
 <Button
 class="w-full":disabled="!canSubmit"
 @click="submit"
 >
