 ...
 * Phase：仓库多选器
 *
 * 用于 TechPlanCard 创建态内嵌 + 追加态 Dialog。受控的多选交互：
...
function toggle(id: string) {
 ...
 ? props.modelValue.filter(x => x !== id): [...props.modelValue, id]
 emit('update:modelValue', next)
}
...
<template>
 ...
 <CommandGroup>
 <CommandItem
 v-for="repo in filtered":key="repo.id":value="repo.id":disabled="isDisabled(repo.id)"
 class="flex items-center gap-2 cursor-pointer"
 @select="toggle(repo.id)"
 >
 <Checkbox:model-value="selectedSet.has(repo.id)":disabled="isDisabled(repo.id)"
 ...
 <Button:disabled="modelValue.length === 0 || submitting"
 @click="handleConfirm"
 >
