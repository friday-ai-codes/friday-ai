...
<template>
 ...
 <button
 type="button"
 class="sjv-switch-btn":class="{ 'is-active': mode === 'structured' }"
 role="tab":aria-selected="mode === 'structured'"
 ...
 <button
 type="button"
 class="sjv-switch-btn":class="{ 'is-active': mode === 'raw' }"
 ...
 >
 <li v-for="(it, i) in searchView.diagnosis.issues":key="`is-${i}`">{{ it }}</li>
 </ul>
 <ul
 ...
 >
 <li v-for="(it, i) in searchView.diagnosis.suggestions":key="`sg-${i}`">{{ it }}</li>
 </ul>
 </div>
 </div>
 <!-- 通用结构化树 -->
 ...
 <JsonNode
 v-for="[k, v] in rootEntries":key="String(k)":node-key="k":value="v":depth="0"
 />
 </div>
 </template>
 </div>
