/**
 * Quick Task：hydrate adapter 单元测试。
 *
 * 5 条真实 v25 fixture 全覆盖 D5 算法分支：
 * F1：纯 content（单 text part）
 * F2：content + narrations（v25 典型）
 * F3：content + tool_calls + narrations（含 routing trace）
 * F4：content + timeline + tool_calls（v25 末期 timeline-based）
...
describe('hydrateLegacyMessage (Quick Task )', => {
 it('F1: 纯 content → 单 text part', => {
 const parts = hydrateLegacyMessage(asMessage(v25Fixtures.F1))
 ...
 const parts = hydrateLegacyMessage(asMessage(v25Fixtures.F2))
 ...
 const parts = hydrateLegacyMessage(asMessage(v25Fixtures.F3))
 ...
 const parts = hydrateLegacyMessage(asMessage(v25Fixtures.F4))
 ...
 const parts = hydrateLegacyMessage(asMessage(v25Fixtures.F5))
