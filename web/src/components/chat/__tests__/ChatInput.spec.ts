...
describe('chatInput model-selector 折叠重构', => {
 ...
 await nextTick
 const items = wrapper.findAll('.model-menu-item')
 expect(items.length).toBe(3)
 const labels = items.map(i => i.text)
 expect(labels.some(l => l.includes('anthropic-prod / claude-sonnet-4'))).toBe(true)
 expect(labels.some(l => l.includes('anthropic-prod / claude-opus-4'))).toBe(true)
 expect(labels.some(l => l.includes('anthropic-dev / claude-sonnet-4'))).toBe(true)
 wrapper.unmount
 })
 ...
 await wrapper.find('.model-selector').trigger('click')
 await nextTick
 await wrapper.find('.model-menu-item').trigger('click')
 await nextTick
