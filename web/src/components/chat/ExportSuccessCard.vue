...
/**
 * 导出成功卡片 -- 在消息流中展示导出结果，包含飞书文档链接。
 * 复用 DocSummaryCard 的 .card.mt-2.animate-fade-in 结构 (per D-14)。
 */
...
const formattedTime = computed(() => {
  try {
    return new Date(props.exportedAt).toLocaleString('zh-CN')
  }
  catch {
    return props.exportedAt
  }
})
</script>
