...
<template>
 ...
 <span
 class="icon-[lucide--chevron-right] da-caret":class="expanded ? 'rotate-90': ''"
 />
 </button>
 ...
 <component:is="rowIsInteractive(row.view) ? 'button': 'div'"
 v-for="row in rows":key="row.idx"
 class="da-row":class="[
 ...
 <span
 class="da-row-text":class="{ 'da-row-text--clamp': isLongText(row.view.text) && !expandedRows.has(row.idx) }"
 >
 {{ isLongText(row.view.text) && !expandedRows.has(row.idx) ? previewText(row.view.text): row.view.text }}
 </span>
 <span
 v-if="rowIsInteractive(row.view)"
 class="icon-[lucide--chevron-right] da-row-caret":class="expandedRows.has(row.idx) ? 'rotate-90': ''"
 />
 </div>
 <div v-if="row.view.expandable && expandedRows.has(row.idx)" class="da-row-detail">
 <StructuredJsonView:value="row.view.detailValue ?? row.view.detail":tool-name="row.view.toolName"
 kind="input"
 />
 </div>
 </div>
