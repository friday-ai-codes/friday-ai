...
async function handleConfirmDelete(): Promise<void> {
  ...
  try {
    ...
    // ：末尾边界 — 选中 preview 列表最后一条时，拒绝提交
    // 避免 targetMsg fallback 到选中消息本身 → DELETE ?before_id=<self.id> 导致一条都删不掉
    // Pitfall 4：简化版（REVIEW WR-04 推荐方案）— 文案锁定自 UI-SPEC §Fix 3
  ...
  <Dialog :open="props.open" @update:open="emit('update:open', $event)">
    <DialogContent class="card rounded-2xl max-w-lg">
      ...
      <ScrollArea class="max-h-80">
        ...
        <div v-else-if="messages.length === 0" class="p-4 text-sm text-muted-foreground">
          暂无可清理的消息
        </div>
        <ul v-else class="space-y-2 p-1">
          <li
            ...
            :key="msg.id"
            class="flex items-start gap-2 p-2 rounded-md border border-border/50"
          >
            <Checkbox
              :id="`clean-msg-${msg.id}`"
              :model-value="selectedIds.has(msg.id)"
              :aria-label="`选择消息 ${roleLabel(msg.role)} ${msg.created_at}`"
        ...
        <AlertDialog>
          <AlertDialogTrigger as-child>
            <Button
              ...
              :aria-label="`永久删除选中的 ${selectedCount} 条消息`"
            >
              永久删除选中消息
            </Button>
          </AlertDialogTrigger>
