...
/**
 * PromptEditor — Prompt 编辑抽屉主容器（ 交付核心）
 *
 * 职责：
...
// ============================================================================
// 脏检测（与 currentPrompt.active_version 字节级比较，对齐 D-03 契约）
//
...
async function handleSave(): Promise<void> {
  ...
  // 二次确认（UI-SPEC §Copywriting "确认保存" AlertDialog）
  // TODO -05：使用专用 Dialog 承载 change_note Input（useConfirmDialog 当前不支持 Input slot）
  ...
  try {
    if (props.mode === 'create') {
      ...
      // 由页面层（-05）在点击 "+ 新建 Prompt" 时直接构造 payload 调 store.createPrompt。
...
<template>
  <Sheet :open="open" @update:open="handleOpenChange">
    <SheetContent
      side="right"
      class="sm:max-w-3xl! w-full flex flex-col gap-0 p-0 bg-background"
    >
      <!-- Header：图标 + 标题 + slug code 标签 + dirty 指示 -->
      <SheetHeader class="border-b border-border/60 bg-card px-6 py-4 gap-1.5">
        <div class="flex items-start gap-3">
          <div class="shrink-0 mt-0.5 w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center">
      ...
      >
        ...
        >
          ...
          <VariableSchemaEditor
            :model-value="editedVariablesSchema"
        ...
        >
          <div class="space-y-3">
            ...
            <div class="grid grid-cols-[1fr_18rem] gap-4">
              <PromptBodyEditor
                :model-value="editedBody"
                @update:model-value="onBodyChange"
              />
              <PromptVariablePanel
                :body="editedBody"
                :variables-schema="editedVariablesSchema"
              />
            </div>
          </div>
        </TabsContent>
      ...
      <SheetFooter class="mt-auto border-t border-border/60 bg-card px-6 py-3 flex-row items-center justify-between gap-2 sm:flex-row sm:justify-between sm:space-x-0">
        <span class="text-[11px] text-muted-foreground">
          <template v-if="isDirty">
            <span class="inline-block w-1.5 h-1.5 rounded-full bg-amber-500 mr-1.5 align-middle" />
            存在未保存修改
          </template>
          <template v-else-if="mode === 'edit' && currentPrompt">
            <span class="inline-block w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1.5 align-middle" />
            已与服务器同步
          </template>
        </span>
