 ...
 *
 * 上游依赖：
 * - ~/stores/prompts::previewPrompt（-01 Task 3 交付）
 * - ~/api/prompts::PromptVariableMissingError（-01 Task 2 交付）
  ...
  <div class="space-y-5">
    ...
    <div class="flex items-center justify-between gap-3">
      ...
      <Button
        ...
        :title="canPreview ? '' : '请先编辑 Prompt 正文'"
    ...
    <div
      v-if="renderError"
      role="alert"
      class="rounded-lg border border-destructive/40 bg-destructive/6 p-3 text-xs text-destructive flex items-start gap-2"
    >
    ...
    <div class="rounded-xl border border-border/60 bg-card p-4 shadow-sm">
      <PromptVariableForm
        :body="body"
        :variables-schema="prompt.active_version?.variables_schema ?? {}"
    ...
    <div class="space-y-2">
      ...
      >
        ...
        <pre class="font-mono text-sm leading-6 text-foreground whitespace-pre-wrap p-4 max-h-[400px] overflow-auto">{{ renderedResult }}</pre>
      </div>
