 ...
 *
 * 校验契约(与 215-01 交付的 PromptUpdateInputSchema 对齐):
 *   - title  非空 + max 200
 *   - description max 1000 + 默认空串
 *
 * 表单值变化通过 update:values 事件向上冒泡给 PromptEditor 容器(215-04 整合)。
 *
 * 只读字段 Label / 徽章文案严格对齐 215-UI-SPEC.md §Copywriting §Color 表。
 */
...
// ============================================================================
// 只读字段展示映射(与 215-UI-SPEC.md §Color 徽章表严格对齐)
// ============================================================================
...
<template>
  <form class="space-y-5">
    ...
    <!-- 只读元信息区 -->
    <div v-if="prompt" class="card p-4 space-y-3">
    ...
    <div v-if="isLocked && prompt" class="space-y-4">
      ...
      <div class="space-y-1.5">
        ...
        <div
          ...
          class="rounded-md border border-border/60 bg-muted/40 px-3 py-2 text-sm leading-relaxed whitespace-pre-wrap"
          :class="prompt.description ? 'text-foreground' : 'text-muted-foreground italic'"
        >
