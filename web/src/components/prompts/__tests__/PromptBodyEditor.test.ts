...
// Wave 1 的契约测试(variableHighlight.test.ts)使用 Node 全局 __dirname 成功,
...
describe('promptBodyEditor', () => {
  ...
  it('外层 wrapper 固定 480px 高度(UI-SPEC CodeMirror 视觉契约)', () => {
    const wrapper = mount(PromptBodyEditor, {
      props: { modelValue: '' },
    })
    const outer = wrapper.find('div.rounded-lg')
  ...
  it('源文件包含 :deep(.cm-prompt-variable) 与精确 HSL 值(UI-SPEC 契约)', () => {
    const source = readFileSync(sfcPath, 'utf-8')
