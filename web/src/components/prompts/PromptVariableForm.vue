...
<template>
 ...
 <div
 v-for="varName in detectedVariables":key="varName"
 class="space-y-1.5"
 >
 ...
 <Textarea
 v-if="useTextarea(varName)":id="`var-${varName}`":model-value="values[varName] ?? ''":placeholder="placeholderFor(varName)":class="{ 'border-destructive ring-destructive/50': effectiveError(varName) !== '' }"
 rows="3"
 @update:model-value="(v: string | number) => handleInput(varName, String(v))"
 />
 <Input
 v-else:id="`var-${varName}`":model-value="values[varName] ?? ''":placeholder="placeholderFor(varName)":class="{ 'border-destructive ring-destructive/50': effectiveError(varName) !== '' }"
 @update:model-value="(v: string | number) => handleInput(varName, String(v))"
 />
