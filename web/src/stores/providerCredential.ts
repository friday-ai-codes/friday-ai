/**
 * Provider 凭证管理 Pinia Store（ / ）
 *
 * 缓存策略（CONTEXT D-07 ~ D-09）：
 ...
 * 1. 满足 UX-06 秒级展开诉求(hard refresh / 路由跳转保留状态)
...
export const useProviderCredentialStore = defineStore('providerCredential', () => {
  ...
  /** UX-06：按 scope / projectId 拉取凭证列表,同上下文 30s TTL 内命中缓存。 */
  ...
  /** UI-02：拉取 5 Provider 元信息 + credential_schema JSON Schema（动态表单数据源）。 */
  ...
  /** UX-06：强制刷新 available_models 并写回 credential。 */
  ...
  /**
   * 把凭证设为同 (scope, provider_type) 维度默认（乐观更新 + 失败回滚）。
   *
  ...
  /** UX-06 核心路径：cache hit 直接返回；miss 触发 refreshModels。 */
