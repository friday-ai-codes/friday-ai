...
- **紧凑间距**：卡片内边距 `p-4`~`p-5`，卡片间距 `gap-4`，避免大面积空白
...
| 卡片内容         | `p-5`                                                             |
| 图标容器（小）   | `p-1.5 rounded-lg bg-primary/10`                                  |
| 图标容器（大）   | `p-2 rounded-lg bg-primary/10`                                    |
...
<RouterLink class="card card-interactive group flex flex-col">
  <div class="p-4 flex-1 space-y-3">...</div>
