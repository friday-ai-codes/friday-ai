...
for cand in \
  ...
  "/Users/example/Projects/open-source/friday-clean/.claude/gsd-core/bin/gsd-tools.cjs"; do
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`/Users/example/Projects/open-source/friday-clean/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. If `GSD_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.
...
global: `/Users/example/Projects/open-source/friday-clean/.claude/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
