...
Plan 列已映射到 `01`（评估/CAS/normalizer）或 `02`（durable/MCP/守卫/观测）。Wave 0 两边均为 `0`。不得删掉这些 automated 命令或把行为改成仅人工。
...
### 143-01（评估 / CAS / normalizer）

- [x] `server/tests/initiatives/test_session_capture_eval.py` — EVAL-01/02/05 RED（tracer 已建，实现前期望 RED）：
...
- [x] `server/tests/initiatives/test_session_capture_eval_tasks.py` — EVAL-03/04、OBS-04 RED（tracer 已建，实现前期望 RED）：
...
- [x] 扩展 `server/tests/initiatives/test_capture_inv6_guard.py` — 新模块禁止 Capture ORM 写、`MemoryService`、`record_hook_writeback`、`aschedule_ingestion`、`background_runner`；persist 仍禁 deferred sinks；enqueue 放独立模块
- [x] 扩展 `server/tests/mcp_tools/test_report_session_knowledge.py` — `test_accepted_enqueues_durable_eval`、`test_terminal_capture_does_not_reenqueue`、`test_enqueue_failure_still_accepted`；继续断言 ProjectMemory 零写入
- [x] 扩展 `server/tests/initiatives/test_capture_observability.py` — 用 Phase 143 lifecycle 白名单替换 `test_no_eval_sampling_events`（persist 路径仍禁止提前 eval sampling）
- [x] Framework install: 无 — 已有 pytest / pytest-django / pytest-asyncio
