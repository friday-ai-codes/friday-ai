...
| T2 K 裁剪 + 凸组合 score_ranked | 107-05 | 3 | RELY-05 | T-107-05, T-107-10 | LLM 编造 id 丢弃 + 位移预算裁剪（base 为子集内相对位次，含子集常态回归）+ violations 留痕 | unit + integration | `cd server && uv run pytest tests/codegraph/test_repo_router_v2_degraded.py tests/codegraph/test_repo_router_v2_meta.py -x -q` | ✅ 既有 | ⬜ pending |
