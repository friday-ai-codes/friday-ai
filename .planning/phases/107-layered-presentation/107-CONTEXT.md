...
- 生产事故锚点：会话 `sess-example-01` 两个 `ConvergenceSession` 都停在 `clarify/waiting_clarification`，agent 等不到就绕道 `create_coding_plan` 徒手编方案——RELY-02 的超时出口正是断掉这条绕行的前提（RELY-01/SPINE 在 Phase 109 收口）。
