...
> `数据环境: 开发库/CI（结构性结论）` 或 `数据环境: 生产实例 friday.example.com（分布实测）`。
...
| 生产实例 friday.example.com | PostgreSQL 17 | `--days 7`（建议再取 `--days 30` 对照） | **deferred** | **deferred**（须记录当时 `SettingKeys.LOG_*` 采样配置） | **否**——只含未命中缓存的调用 | **deferred**：待运维执行 |
...
**数据环境: 生产实例 friday.example.com（分布实测）——deferred，尚未执行。**
...
| 指标 | 值 | 数据环境 |
|------|----|---------|
| 样本量 n | deferred | 生产实例 friday.example.com（deferred） |
| p50_ms | deferred | 生产实例 friday.example.com（deferred） |
| p90_ms | deferred | 生产实例 friday.example.com（deferred） |
| p99_ms | deferred | 生产实例 friday.example.com（deferred） |
| 采样率 | deferred | 生产实例 friday.example.com（deferred） |
