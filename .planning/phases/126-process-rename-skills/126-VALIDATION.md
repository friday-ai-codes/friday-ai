...
- **After every task commit:** Run the task-scoped pytest file(s) listed in the Per-Task Verification Map
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 126-01-01 | 01 | 0/1 | EXEC-01 | T-126-04 | BFS 闸门防 DoS | unit | `cd server && uv run pytest tests/codegraph/test_process_trace_model.py -q` | ❌ W0 | ⬜ pending |
| 126-01-02 | 01 | 1 | EXEC-01 | T-126-04 | depth/branching/minSteps/conf | unit | `cd server && uv run pytest tests/services/code_graph/test_process_trace.py -q` | ❌ W0 | ⬜ pending |
| 126-01-03 | 01 | 1 | EXEC-01 | — | QUEUE_GRAPH + queueing_lock | unit | `cd server && uv run pytest tests/services/code_graph/test_process_enqueue.py -q` | ❌ W0 | ⬜ pending |
| 126-02-01 | 02 | 2 | EXEC-02 | T-126-01 | MCP PAT fail-closed | unit/api | `cd server && uv run pytest tests/mcp_tools/test_schema_snapshot.py -q` | ✅ 扩展 | ⬜ pending |
| 126-02-02 | 02 | 2 | EXEC-02 | — | intra/cross/unknown | unit | `cd server && uv run pytest tests/services/code_graph/test_process_trace.py -q` | ❌ W0 | ⬜ pending |
...
| 126-04-02 | 04 | 3 | RENAME-01 | T-126-03 | knowledge 白名单 | unit | `cd task && uv run pytest tests/ -k rename_preview -q` | ✅ 扩展 | ⬜ pending |
