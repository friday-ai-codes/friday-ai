...
| 项 | 值 | 数据环境 |
|----|----|---------|
| dense-only 查询耗时（`--verify-cosine` 的 `duration_ms`） | **待生产实例执行补录** | 生产实例 friday.example.com（deferred 人工步骤） |
| 返回 score 样例（应为 COSINE，自查询 top-1 ≈ 1.0） | **待生产实例执行补录** | 生产实例 friday.example.com（deferred 人工步骤） |
...
在有真实索引的部署实例（friday.example.com）上运行：
...
| 指标 | 值 | 数据环境 |
|------|----|---------|
| 总仓数（is_deleted=False） | 待生产实例执行补录 | 生产实例 friday.example.com |
| 有索引仓数（N_r > 0） | 待生产实例执行补录 | 生产实例 friday.example.com |
| p50 | 待生产实例执行补录 | 生产实例 friday.example.com |
| p90 | 待生产实例执行补录 | 生产实例 friday.example.com |
| p99 | 待生产实例执行补录 | 生产实例 friday.example.com |
| max | 待生产实例执行补录 | 生产实例 friday.example.com |
| mean | 待生产实例执行补录 | 生产实例 friday.example.com |
| median（N̄ 建议口径） | 待生产实例执行补录 | 生产实例 friday.example.com |
