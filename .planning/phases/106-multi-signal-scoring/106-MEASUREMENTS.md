...
> `数据环境: 开发库（结构性结论）` 或 `数据环境: 生产实例 friday.example.com（分布实测）`。
...
**数据环境: 生产实例 friday.example.com（分布实测）——deferred 人工步骤，尚未执行。**
本地开发库无真实仓数据，本地跑出的全 0 / 小样本结果不得写入本节占位表。

### 执行指引（人工步骤）

在有真实数据的部署实例（friday.example.com）上运行：
...
| 指标 | 值 | 数据环境 |
|------|----|---------|
| 总仓数（is_deleted=False） | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| last_commit_at 有值仓数 | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| 覆盖率 | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| 新鲜度 p50（距 now 天数） | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| 新鲜度 p90（距 now 天数） | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
...
| facet | 覆盖率 | 数据环境 |
|-------|--------|---------|
| 业务线/产品线（非空且非「未分类」） | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| 技术栈 | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| 团队归属 | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| 关键程度 | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| 活跃度 | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
...
|-------|-------------------|-------------------|-----------|--------------------|---------|
| 业务线/产品线 | 待生产实例执行补录 | 待生产实例执行补录 | — | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| 服务对象 | 待生产实例执行补录 | 待生产实例执行补录 | — | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| 技术形态 | 待生产实例执行补录 | 待生产实例执行补录 | — | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
| 技术栈 | 待生产实例执行补录 | 待生产实例执行补录 | — | 待生产实例执行补录 | 生产实例 friday.example.com（deferred） |
