...
<objective>
项目级对话技术方案第二批（接 260728-ppb）：用受约束 `task_category` + 服务端直驱 `FeatureSolutionService` 兜底路由，挡住「方案覆盖范围」类 `ask_clarification`，并提供可复用 `propose_project_repos` 命令补「示例功能专项」关联仓库（生产步骤只写 SUMMARY，本任务不写库）。
...
verify **不要**把未 migrate 的本地默认库 `manage.py` 裸跑退出码当绿灯——以 pytest `--reuse-db` 为准。
  ...
  <done>
D-03 落地：命令经 RepoAssociationService 提案（可选 confirm）；pytest 覆盖 dry-run/写路径/缺参；SUMMARY 含 示例功能专项生产补数步骤且本任务未碰生产库。
  </done>
</task>
