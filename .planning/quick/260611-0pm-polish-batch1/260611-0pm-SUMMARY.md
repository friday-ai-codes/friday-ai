...
- **个人路径泄漏清理**：10+ 处 `/home/user/Projects/acme/*` 硬编码默认值改走环境变量（`TS_SAMPLE_REPO` / `GO_GIN_SAMPLE_REPO` / `GO_SAMPLE_REPO` / `VUE_SAMPLE_REPO` / `SAMPLE_FRONTEND_REPO` / `VOLAR_TEST_REPO` / `GOPLS_TEST_REPO`），skipif 增加空值守卫（`Path("")` 即 `.` 会误判存在）；management commands 示例路径改为通用占位，`measure_go_call_completeness --repo-root` 改为必填
