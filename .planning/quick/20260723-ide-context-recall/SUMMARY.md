...
Friday 本地实例搭「思维进阶独立场景」演示项目（绑定 main/feat 分支 + RepoAssociation +
...
5. **分支解析边界（已裁决不做）**：study-app 真实分支 `feat/260618.m-7019711929.思维进阶独立场景` 的 `m-{id}` 写法不被宽松正则（`-m{id}`）命中。用户裁决：有显式分支绑定（ProjectBranch）+ 仓库关联兜底即可覆盖，不扩展 `branch_parsing`。

## 生产实测记录（2026-07-23，friday.example.com + study-app）

配置切换 `~/.friday/config.json` → example.com（本机 dev 配置留档 `config.json.localdev`）。生产已索引 261 仓（含 study-app `5bdc1da9`）。建真实项目「小学思维进阶-刷题入口」（`11b32791`）：挂真实飞书工作项 story/7019711929（upsert 自动回源拉到真实标题）+ 绑定 main/feat 分支。
