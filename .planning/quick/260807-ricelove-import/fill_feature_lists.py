#!/usr/bin/env python3
"""用 MiMo 为 ricelove-scheme 项目补齐 feature_list（格式对齐「示例功能专项」）。
...
def _load_reference_sample() -> str:
    a = Artifact.objects.filter(
        project__name="示例功能专项", type__key="feature_list"
    ).first()
...
async def main() -> None:
    ...
    if not reference:
        raise SystemExit("missing 示例功能专项 feature_list reference")
