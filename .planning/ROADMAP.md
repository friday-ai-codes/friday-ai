...
- 🚧 **v0.23.0 仓库路由增强（分阶段决策漏斗）** — Phases 128–132 (in progress) — 把「全库单段文本相似度选仓」升级为「画像 → 团队门禁 → 短名单 → 章程/历史 → 放置单元 → 门禁/反思」的可解释决策漏斗；验收锚点「示例功能专项」 — [requirements](./REQUIREMENTS.md) · [research](./research/ROUTING-RANKING.md)
