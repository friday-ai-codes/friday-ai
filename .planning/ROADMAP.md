...
- [ ] **Phase 56: compat 内部工具调用 → progress/trace 事件透出** (TBD plans) - OpenAI 兼容流式响应把内部工具调用经 §15 taxonomy 映射为 progress/`reasoning_summary`，不暴露 CoT、不误用 tool_calls（INV-5），缺事件优雅降级零回归 — TRACE-01, TRACE-02
