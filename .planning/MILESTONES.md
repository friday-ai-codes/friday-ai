...
**Timeline:** 2026-08-14 立项并当日收口（约 1 天）。立项动机是「示例功能专项」等 feature list 场景下，全库单段文本相似度选仓会漂到语义巧合仓；把选仓升级为可解释决策漏斗（画像→团队→短名单→章程/历史→放置单元→门禁/反思），在 `BlueprintRouteAdapter` / `RepoAssociationService` 上演进，**不推倒** `RepoRouterV2`。
