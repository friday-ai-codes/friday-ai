...
4. **PLAN-04 建分支绑项目**：方案确认后逐仓建分支并推送 + 绑 仓库↔分支↔项目（写 `ProjectBranch`，经 `ProjectBranchService.bind(source=plan)`，Phase 85 seam）。分支名 **固定格式**：`{type}/{yymmdd}.m-{项目跟踪id}.{项目名}[-{版本号}]`（type=conventional commits，yymmdd 日期，项目跟踪 id=飞书 work_item id，项目名=项目跟踪看板名一致，版本号有则填）。示例 `feat/260610.m-123456770019.示例功能专项-v1.0`。AI 生成分支名 + 用户卡片确认，回接 IDE 闭环。
