...
| 5 | golden set 离线可跑：示例功能专项首条 case，输出引用覆盖率/目标仓命中率/审查打回率等指标，同输入重复运行一致 | ✓ PASS | 实跑 `manage.py evaluate_blueprint_golden` → `sample_exam_boost: citation_coverage=1.0 target_repo_hit_rate=1.0 → PASS`，exit 0，caller 事件带 duration_ms；确定性双跑内建门槛（command:77-81 逐字节比对）+ `test_evaluate_blueprint_golden` 8 例（含连续两次输出一致、坏 case CommandError 非零退出）；fixture 含 sample_service_service/sample_course_service/sample_practice_service 三 direct 仓。打回率/人审修改量/澄清轮次为 CONTEXT 锁定的占位接口（数据 112–114 填充，见 deferred） |
...
| 04-2 | 首条 case 示例功能专项，机制级断言 | ✓ | expected: direct_repos 三仓/required fp/min 阈值（非逐仓全等） |
...
| `server/tests/fixtures/blueprint_golden/sample_exam_boost.json` | 634 | 过 validate_blueprint；含 sample_service_service/sample_course_service/sample_practice_service + study-plan indirect；目录独立仅 1 个 json | ✓ VERIFIED |
