...
provides:
  ...
  - "tests/fixtures/blueprint_golden/assessment_boost.json：三条 fp intent = brownfield / greenfield / greenfield（112-03/04 路由加权断言的期望依据）"
affects:
...
- **数据同步 2 文件**：`blueprint_samples.py` 两条 fp 取 `greenfield`（后端接口净新增）/ `brownfield`（前端入口改造）——工厂天然覆盖两个枚举分支；`assessment_boost.json` 三条 fp 取 `brownfield`（`:42` 自述改造）/ `greenfield`（`:55` 自述净新增）/ `greenfield`（新增组卷能力），即 112-03/04 路由加权断言的期望依据。
...
| Task | Commit | 内容 |
| ---- | ------ | ---- |
| 1 | `5d4c25c0` | feature_points 补必填枚举 intent + blueprint_samples / assessment_boost 字面数据同步 + 3 条正负向 schema 断言 |
...
- `server/tests/fixtures/blueprint_golden/assessment_boost.json`（修改：三条 fp 各补 intent，缩进沿用原文件）
...
- `uv run python manage.py evaluate_blueprint_golden`：退出码 0（assessment_boost 过 validate_blueprint 门，passed=1 / failed=0）
...
- artifacts contains 断言：`"greenfield"` ∈ blueprint_schema.py ✓；`blueprint.spec_gate.config` ∈ models.py ✓；`async def aget_json_setting` ∈ settings_service.py ✓；`blueprint.route.scored` ∈ event_taxonomy.py ✓；`"intent"` ×3 ∈ assessment_boost.json ✓
...
- **golden fixture**：`assessment_boost.json` 三条 intent 分布（brownfield / greenfield / greenfield）是 112-03/04 路由加权断言的期望依据——greenfield 功能点重章程，`onion-learning` 应凭 `owned_domains(status=planned)` 进候选。
