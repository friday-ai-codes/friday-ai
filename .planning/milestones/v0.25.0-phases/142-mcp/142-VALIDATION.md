...
Plan 01–04 已落地。Wave 0 文件均存在；下表为最终合并态证据。Nyquist 按 **Phase 142 自有覆盖** 判定，不以工作树中无关蓝图脏改动的全目录失败为否决。

| Task ID | Plan | Wave | Requirement | Threat Ref | Secure Behavior | Test Type | Automated Command | File Exists | Status |
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 142-01-01 | 01 | 0 | MCP-01 | T-142-01 / T-142-06 | 认证用户 POST 非空 Q/A（可选元数据全集）→ HTTP 200、`accepted=true`、DB 有 Capture 行可证 `capture_id`；成功不声称已挂钩或已入 RAG | integration | `cd server && uv run pytest tests/mcp_tools/test_report_session_knowledge.py::test_member_report_persists_capture -x` | ✅ | ✅ green |
| 142-01-02 | 01 | 0 | MCP-01 | T-142-01 | 缺 `question` 或 `answer`（含 blank）→ 400 `invalid_params`，无 Capture 行 | integration | `cd server && uv run pytest tests/mcp_tools/test_report_session_knowledge.py::test_missing_question_or_answer_400 -x` | ✅ | ✅ green |
| 142-01-03 | 01 | 0 | MCP-01 | T-142-01 | 匿名 POST `/api/mcp/tools/report_session_knowledge/` → 401 `authentication_failed` | integration | `cd server && uv run pytest tests/mcp_tools/test_report_session_knowledge.py::test_missing_token_401 -x` | ✅ | ✅ green |
| 142-01-04 | 01 | 0 | MCP-02 | T-142-03 / T-142-06 | 无 `project_id`/仓元数据 → 200 `accepted=true`，行存在，`reason=unanchored`；**不是** `branch_unresolved` | integration | `cd server && uv run pytest tests/mcp_tools/test_report_session_knowledge.py::test_unanchored_still_accepted -x` | ✅ | ✅ green |
| 142-01-05 | 01 | 0 | MCP-02 | T-142-03 | 无法解析的 `git_url` → 200 `accepted=true`，`reason=repo_unresolved`，有 Capture 行 | integration | `cd server && uv run pytest tests/mcp_tools/test_report_session_knowledge.py::test_unresolved_repo_still_accepted -x` | ✅ | ✅ green |
| 142-01-06 | 01 | 0 | MCP-02 | T-142-07 | 仅 `branch_name=main`（无 `project_id`）→ 200 `accepted=true`，`reason` 不是 `branch_unresolved` | integration | `cd server && uv run pytest tests/mcp_tools/test_report_session_knowledge.py::test_default_branch_does_not_mean_rejected -x` | ✅ | ✅ green |
| 142-01-07 | 01 | 0 | MCP-02 | T-142-03 | 响应 `reason` 原样透传 persist `link_reason`（含 `repo_unauthorized` 等成功挂钩码）；挂钩失败仍 `accepted=true` | integration | `cd server && uv run pytest tests/mcp_tools/test_report_session_knowledge.py::test_link_reason_passthrough -x` | ✅ | ✅ green |
| 142-01-08 | 01 | 0 | MCP-01, MCP-02 | T-142-05 | 同用户/session/question 重试 → 同一 `capture_id`，`idempotent_hit=true`，首次答案与挂钩原因不覆盖 | integration | `cd server && uv run pytest tests/mcp_tools/test_report_session_knowledge.py::test_idempotent_hit_keeps_first_write -x` | ✅ | ✅ green |
...
| 142-01-10 | 01 | 0 | MCP-04 | T-142-07 | 新工具不增加 `ProjectMemory`、不调用 `MemoryService.append`；旧 `branch_unresolved` 未收语义保持在旧工具测试 | integration | `cd server && uv run pytest tests/mcp_tools/test_report_session_knowledge.py::test_session_tool_does_not_write_project_memory tests/mcp_tools/test_report_project_knowledge.py::test_unresolvable_branch_fail_soft -x` | ✅ | ✅ green |
| 142-01-11 | 01 | 0 | OBS-02 | T-142-02 | MCP 路径 persist 后 Capture 行不含明文 `sk-` 等密钥（persist 已测；本路径再钉一次） | integration | `cd server && uv run pytest tests/mcp_tools/test_report_session_knowledge.py::test_redaction_on_mcp_path -x` | ✅ | ✅ green |
...
| 142-04-02 | 04 | 2 | MCP-01..04 / STORE-03 | T-142-07 | Phase 142 自有 MCP/Capture/Memory/`report_project_knowledge` 回归全绿；无 SessionCapture 旁路 create。全目录 `tests/mcp_tools/` 另有 2 项蓝图脏改动失败，范围外、非阻断 | regression | 自有门禁：`test_report_session_knowledge.py` + `test_report_project_knowledge.py` + `test_capture_service.py` + INV-6 + schema/alignment（56 passed）。全目录 361 项中 359 passed、2 failed（见 Unrelated Baseline Failures） | ✅ | ✅ green（phase-owned） |
...
全目录 `tests/mcp_tools/` 在 Plan 04 Task 2 跑出 **361 collected / 359 passed / 2 failed**。两项失败来自执行前已存在的蓝图/技术方案脏改动，**不属于 Phase 142 合同**，本计划未修改、未暂存、未提交对应文件。

| Test | Assertion | Dirty source | Disposition |
|------|-----------|--------------|-------------|
| `tests/mcp_tools/test_stage_runner_tools.py::test_route_blueprint_repos_dry_run` | `body["router_version"] == "v2"` 实际为 `"clarify"` | 未提交的 `process_runtime` / blueprint 路由改动 | 范围外、非阻断 |
| `tests/mcp_tools/test_blueprint_clarification_tools.py::test_response_assembly_splats_the_extras_so_the_off_state_is_byte_identical` | `'"blueprint_status":' not in src` | 未提交的 `mcp_tools/technical_plan_service.py` 新增 `blueprint_status` 字面量 | 范围外、非阻断 |

Phase 142 自有门禁（干净库，无 `--reuse-db`）：`test_report_session_knowledge.py` 16 + `test_report_project_knowledge.py` 15 + `test_capture_service.py` 14 + `test_schema_snapshot.py` 2 + `test_mcp_package_alignment.py` 4 + `test_capture_inv6_guard.py` 3 + `test_memory_inv6_guard.py` 2 = **56 passed**。npm `tests/server.test.ts` **12 passed**。`ruff check` 通过。`SessionCapture` / `CaptureService` / `test_report_project_knowledge.py` **无 diff**。

Nyquist / Sign-Off 以该自有覆盖为准，不以全目录蓝图基线为否决。
...
- [x] `nyquist_compliant: true` set in frontmatter（按 Phase 142 自有覆盖，非外部脏套件）

**Approval:** Plan 04 executor 2026-08-28 — validated / Nyquist complete on phase-owned evidence

**Counts:** phase-owned 56 pytest + 12 vitest；full `tests/mcp_tools/` 359/361 with 2 documented unrelated failures.
