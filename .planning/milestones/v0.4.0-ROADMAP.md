...
- [x] **Phase 21: 触发模型与执行可观测** - 飞书 event_type 断裂修复、schedule 假功能移除、dispatch 失败可查，执行详情错误展示 + WS 断线轮询兜底 + 状态枚举对齐 (7 plans, 3 waves) (completed 2026-06-13)
...
- [x] 21-01-PLAN.md — 后端测试脚手架：trigger_sync(TRIG-01) + trigger_type_choices(TRIG-02) + test_hooks 广播(OBS-01) + dispatch 失败持久化(TRIG-03)
- [x] 21-02-PLAN.md — 前端测试脚手架：status 全覆盖(OBS-03) + useExecutionsStore(OBS-01/03) + useExecutionState 降级(OBS-02) + NodeOverviewTab 结构化错误(OBS-01)

**Wave 1** *(依赖 Wave 0；后端与独立前端并行)*

- [x] 21-03-PLAN.md — TRIG-01 async_sync 单数+filter_config + TRIG-03 飞书/webhook dispatch 失败可查
- [x] 21-04-PLAN.md — TRIG-02 TriggerType 移除 schedule + 0027 migration + OBS-01 WS 广播 error 字段
- [x] 21-05-PLAN.md — TRIG-02 前端 schedule 残留清除 + OBS-03 列表 statusOptions/stats 对齐

**Wave 2** *(依赖 Wave 0 + 后端 WS 广播契约 21-04)*

- [x] 21-06-PLAN.md — OBS-03 status.ts 补 suspended + OBS-01 useExecutionsStore node_failed 写 error + stats 纠偏
- [x] 21-07-PLAN.md — OBS-01 NodeOverviewTab error_code/结构化解析 + OBS-03 DAG 色 + OBS-02 WS 断线降级轮询
...
| 21. 触发模型与执行可观测 | 7/7 | Complete    | 2026-06-13 |
