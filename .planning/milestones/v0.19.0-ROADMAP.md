...
### ✅ v0.19.0 技术方案可信度 (Phases 105–110，其中 108 已移交 v0.20.0) — COMPLETE 2026-08-02（审计 tech_debt，未打 tag）
...
- [x] **Phase 105: 编排解锁与评估标尺** - 置信度由分数 margin 确定性推导 + 分数可拆解落 trace + 幂等与快照回放 + golden set 回归门禁（RELY-04, ROUTE-07/08/09） (completed 2026-07-30)
- [x] **Phase 106: 多信号打分函数重构** - 消除尺寸偏置 + 元数据信号真正入分 + 活跃度连续化 + 权重外置不发版可调（ROUTE-03/04/05/06） (completed 2026-07-30)
- [x] **Phase 107: 分层呈现与链路韧性** - 本项目/全局两组呈现与跨组标注 + 降级可见 + 澄清必达有出口 + Stage 1 重试与延迟上界（ROUTE-01/02, RELY-02/03/05） (completed 2026-07-31)
...
**Success Criteria** (what must be TRUE):
  ...
  1. 「示例功能专项」用例中前端候选 `sample_service_service` 排在 `sample_web` 之前、后端 `sample_course_service` 与 `sample_user_service` 进入 Top-5；断言锁定的是机制（`sample_web` 的广度加成不高于 `sample_service_service`）而非某组权重下的偶然名次；golden set 门禁通过（Recall@5 不低于基线、误自动选中率 ≤ 10%）。
...
**Coverage (v0.19.0):** 19/19 需求全部映射，无孤儿、无重复（DEPTH-01~05 已移交 v0.20.0，不再计入本里程碑）。**收口 17 Complete / 2 Partial（ROUTE-03、RELY-02）/ 0 未达**，里程碑判定 `tech_debt`。
