...
**Success Criteria** (what must be TRUE):
  ...
  2. `blueprint_route` 输出候选带 `charter_match` 分量且分数可拆解；greenfield 功能点上章程 owned(planned) 的仓能进入候选（复现示例功能专项 case：sample_service_service 不再因能力树无节点被淘汰）；brownfield 上命中章程禁区的候选被降权，LLM 保留它必须给显式理由。
