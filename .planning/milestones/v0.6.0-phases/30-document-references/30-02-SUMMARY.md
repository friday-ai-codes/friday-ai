...
- `server/delivery/services/document_service.py`：
  ...
  - **`derive_feishu_tenant(canonical_url)`**：`urlparse().hostname` 取首段子域作租户 slug（`example.feishu.cn` → "guanghe"、`acme.larksuite.com` → "acme"）；非飞书/lark 域、无 host、无意义首段（`feishu.cn`/`www`）→ ""。
