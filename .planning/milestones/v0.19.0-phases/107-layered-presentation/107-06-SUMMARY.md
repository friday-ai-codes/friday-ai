...
coverage:
  ...
  - id: D12
    ...
    human_judgment: false
  - id: D13
    description: "生产存量卡死会话（CONTEXT 记录会话 sess-example-01 等）首次上线的实际影响面——需运维先跑 --dry-run 看清将被批量推进的会话，其中可能有已被人工绕道处理过的轮"
    verification: []
    human_judgment: true
...
**运维首次上线建议（非阻塞）**：先在生产执行 `python manage.py expire_pending_clarifications --dry-run` 看清存量卡死会话的影响面（CONTEXT 记录会话 `sess-example-01` 有 2 条），必要时用 `--limit` / `--session-id` 分批处置。
