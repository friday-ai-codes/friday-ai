...
| T-05-1 两个门一起收窄 + 落库走投影 service + **归属判定下移进 service** + 拒绝留痕 | 109-05 | 5 | SPINE-02 | T-109-05-01/02/03/04/06/07 | schema 层收窄（非 prompt）；无来源 fail-closed 并留痕；`aproject`/`arebind` 必填 `actor_user_id`，工具与端点共享同一道门；不碰 `mcp_tools/`。**预期红窗恰为两处**：签名 fixture 与 `test_coding_tools.py` 的 8 个旧签名用例，由 T-05-2 收口，不得回退 schema。🔴 `tests/test_plan_projection_service.py` **不在**红窗内 —— 109-03 T2 建立的 5 组用例（`conversation`/`idempotent`/`concurrent`/`new_version_keeps_old`/`traceability`）的 `actor_user_id` 迁移（补参 + 造数 owner 与 actor 对齐）**属 T-05-1 本身**，该文件须在本 task 内即全绿；不得给 `actor_user_id` 补默认值、不得统一传 `"system"` 绕过归属对齐 | integration | `cd server && uv run pytest tests/mcp_tools/ tests/test_plan_projection_service.py tests/test_plan_projection_api.py -x` | ✅ 存在 | ⬜ pending |
...
| T-08-1 `isUnresearched` 允许清单 + 草稿横幅 + 常驻徽标 | 109-08 | 7 | RELY-01 | T-109-08-02/03/04 | 未知取值保守标注且不上屏；零 `v-html`。既有 fixture 将集体渲染横幅与徽标（`Badge` 被 stub 且无 `ml-auto` 断言 ⇒ 预期仍绿），`TechPlanCard.spec.ts` 已列入 T-08-1 的 `<files>` 作为「若变红则有权就地修」的明文授权；修法限收窄选择器或补 `provenance: 'orchestrated'`，不得加「无 `provenance` 视为可信」豁免 | unit（前端） | `cd web && pnpm vitest run src/components/chat/__tests__/TechPlanCard.spec.ts` | 🟡 加用例 | ⬜ pending |
| T-08-2 确认弹层 + ack 三路径透传 + `code` 分支兜底 | 109-08 | 7 | RELY-01 | T-109-08-01/05/06 | `acknowledge_unresearched: true` 只能由用户勾选产生；按 `code` 不按 `detail`。🔴 **闸门必然打红三条既有用例**（`TechPlanCard.spec.ts:501` 创建态确认 / `:519` 分支模板透传 / `:547` 单仓重试 —— 既有 fixture 不传 `provenance` ⇒ `isUnresearched` 恒 true、点击被弹层拦截）：**处置属 T-08-2 本身**（该 spec 已列入其 `<files>`），逐条补 `provenance: 'orchestrated'` 挂载参数、用例名与断言逐字保留，该文件须在本 task 内即全绿；不得让 `ensureUnresearchedAcknowledged()` 默认返回 `true`、不得给 `isUnresearched` 加「无 `provenance` 视为可信」豁免 | unit（前端） | `cd web && pnpm vitest run src/components/chat/__tests__/TechPlanCard.spec.ts` | 🟡 加用例 | ⬜ pending |
