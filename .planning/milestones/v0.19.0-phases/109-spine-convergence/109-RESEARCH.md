...
|----------|-------------|------------------|
| **Stored data** | `coding_plans` 表存量行（生产实例含徒手创作产物，其中包括事故会话 `sess-example-01` 绕道产生的那份）；`delivery_artifact_version` 存量 technical_plan 版本 | **代码改动 + 迁移 default，不需要数据迁移**：新列 `provenance default="draft"` 恰好正确描述存量（全是徒手产物）；`source_artifact_version_id` 存量为 NULL 正确（无投影来源）。⚠️ 不要写把存量刷成 `orchestrated` 的 data migration |
