...
overrides_applied: 0
human_verification:
  - test: "在生产实例 friday.example.com 执行 `cd server && uv run python manage.py measure_repo_index_stats --json --top 20 --verify-cosine`，把输出回填 105-MEASUREMENTS.md §1 延迟表与 §2 N_r 分布占位表"
  ...
  - test: "从生产会话 sess-example-01 导出「示例功能专项」需求原文与真实 Stage 0 命中数值，替换 golden_main.json 中 gk-001 的合成版本，然后 GENERATE_GOLDEN=1 重建 baseline 并 review 逐例 diff"
...
**Test:** 在 friday.example.com 执行 `uv run python manage.py measure_repo_index_stats --json --top 20 --verify-cosine`，回填 105-MEASUREMENTS.md 的 N_r 分布表与 dense 查询延迟数字。
...
**Test:** 导出会话 sess-example-01 的需求原文与 Stage 0 命中数值，替换 gk-001 合成版本，`GENERATE_GOLDEN=1` 重建 baseline 并 review diff。
