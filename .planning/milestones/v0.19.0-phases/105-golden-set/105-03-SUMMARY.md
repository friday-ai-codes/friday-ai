...
- **编排解锁（RELY-04）**：Stage 1 三种失联情形（网关 400 / 连接错误 / 超时）与两条静默 None 路径（provider_missing / unparsable_llm_output）下，route() 均产出按分数 margin 确定性推导的分级；margin 达标即 `auto_selected=True`——直接解开「失联 → 恒 low → auto_selected 恒 false → 编排卡死」事故链（会话 sess-example-01）
