# Requirements Archive: v0.8.0 多仓串行编码 → 融合 PR
...
- **REPLAN-01**: 编码中全自动 replan/回溯（前端卡住自动唤起后端重调研改方案）——最目标能力，v0.8 用 HITL-01「抛 question 给人」过渡，全自动留 backlog
...
| Feature | Reason |
|---------|--------|
| 编码中全自动 replan/回溯 | 最目标能力，v0.8 用 HITL-01「抛 question 给人」过渡，全自动留 backlog（避免范围爆炸，DOMAIN/ROADMAP-vNext 显式非目标） |
