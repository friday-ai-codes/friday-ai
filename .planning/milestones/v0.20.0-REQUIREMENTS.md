...
- [x] **GATE-01**（**满足 @ Phase 116 + 同步点 2 收尾两步**）：workflow / chat / MCP / feature list 全入口统一走蓝图编排；MCP 入口支持异步澄清协议（返回 pending、可作答、可续取结果），不再跳过澄清
  ...
  - ✅ **同步点 2 收尾 · 第一步（三道消费方接缝 + 终态映射，2026-08-02）**：116 交付的是「入口能建蓝图会话」，缺的是**蓝图与其消费方之间的翻译**。三处静默失败已修复 —— G1 workflow 入口（`waiting_clarification` 用旧链 `Clarification` 判据 ⇒ 每一次澄清都把工作流判死）、G3 MCP 入口（主载荷读 blueprint/v1 不存在的 `execution_plan` 顶层键 ⇒ `repository_tasks` 恒 `[]`，结构合法而语义为空）、G4 feature_list 入口（待答问题取旧链子题模型 ⇒ 永久 `researching` + 空问题列表）。⭐ **终态映射同批改成人审 HITL 挂起**：`pending_review` ⇒ `waiting_event` / `kind="human_review"` 且**刻意不产出 `plan` 载荷**（下游读的就是它）—— 这就是原顺延理由里那条「翻默认 = 让编码代理拿未经人审的蓝图去写代码」的正面解决。详见 `.planning/SYNC-POINT-2-CLOSURE.md` §1–§9。
  - ✅ **同步点 2 收尾 · 第二步（触点 + 翻默认 + 退役，2026-08-02）**：原 ⏭ 清单四项**全部交付** —— ① `DEFAULT_ENTRY_SWITCH` 四键翻到 `technical_blueprint`（`blueprint_entry_switch.py:72-77`），开关机制与 per-entry override 一字未动，运维仍可改一个设置值单入口回退；② 旧 `technical_plan`「不再是任何入口默认」收口，退役标记进 `ProcessDefinition.config`（`builtin_processes.py:1241/1257`，零迁移、程序可查），⛔ 保留注册（在途会话与回滚 override 都要它）、六个冻结文件一行不改；③ `TechPlanCard` / `NodeDataTab` / `ArtifactTimeline` 三处触点按 `schema_version` 判别 blueprint/v1，如实呈现形态 + 11 态状态 + 导向 `/knowledge/blueprints/:id`，v0 逐像素不变；④ workflow 终态人审 HITL 挂起已在第一步交付。commit `789a1c0a` / `39b84961` / `e3184cef`，详见 `SYNC-POINT-2-CLOSURE.md` §11–§18。
  - ⏭ **两处刻意收窄（登记，非缺口）**：`map_merged_plan_to_coding_plan`（chat 侧 v0 投影映射器）**未改** —— 从蓝图版本投影出的 `CodingPlan` 正文与 `affected_files` 仍为空，本次只让它**不再静默**（`TechPlanCard` 如实说明形态并导向查看器，用例显式断言 `affected_files == []` 并写明理由）；要让它有内容需在 chat 侧接 `blueprint_execution.derive_execution_plan`，属独立工作项。`aresolve_entry_process_type` 的**未知 entry** 分支仍回旧链 —— 它不是入口、没有声明默认值，不构成「某个入口的默认」，与退役不冲突。
...
| GATE-01 | Phase 116（实现路径 + 开关）+ 同步点 2 收尾第一步（三道消费方接缝 + 终态映射）+ 第二步（触点 + 翻默认 + 退役） | **Complete**：⭐ **116** —— 蓝图 intake + `build_engine_for_session` 按 `process_type` 分派 engine 与 driver + 六个续驱点改造 + 四个入口各自的蓝图实现路径与 per-entry 开关 `SettingKeys.BLUEPRINT_ENTRY_SWITCH` + feature list 映射 + chat 两条断链修复 + 旧链残余流量可观测（116-02/03）；MCP 异步澄清协议全量（116-06）。⭐ **收尾第一步** —— G1 / G3 / G4 三道消费方接缝修复 + `pending_review` 改人审 HITL 挂起（**不产出 `plan` 载荷**，RELY-01 的真闸）。⭐ **收尾第二步** —— 四个开关键默认翻 `technical_blueprint`（机制与 per-entry 回滚一字未动）+ 旧 process 退役标记进 `ProcessDefinition.config`（零迁移、程序可查、保留注册）+ `TechPlanCard` / `NodeDataTab` / `ArtifactTimeline` 三处触点按 `schema_version` 判别（v0 逐像素不变）。⏭ **两处刻意收窄（登记，非缺口）**：chat 侧 `map_merged_plan_to_coding_plan` 未接派生器（蓝图投影内容仍空，但**不再静默**）；未知 entry 的 fail-soft 仍回旧链（它不是入口，不构成默认）。证据：`.planning/SYNC-POINT-2-CLOSURE.md`。 |
