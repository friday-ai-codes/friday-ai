# Requirements Archive: v0.22.0 代码智能图分析升级（对标 GitNexus）

**Archived:** 2026-08-11
**Status:** tech_debt（26 Complete / 1 Partial（IMPACT-03）/ 0 Missing）
**Audit:** [v0.22.0-MILESTONE-AUDIT.md](./v0.22.0-MILESTONE-AUDIT.md)

下一里程碑立项时重新生成 `.planning/REQUIREMENTS.md`。
...
- [x] **IMPACT-03** ⚠️ PARTIAL: impact 可穿仓库边界（沿 `CrossRepoApiCall` 边），跨仓结果标注 `cross_repo: true` 与独立置信档——内核与合成测绿；本环境无跨仓样本且 LSP 默认关，生产可用性未复验
...
| IMPACT-03 | Phase 122 | Partial（合成测绿；生产 CrossRepo=0 + LSP 默认关，真样本未复验） |
