...
## Milestone Summary — v0.14.0 可观测性与日志治理

**Status:** ✅ SHIPPED 2026-06-24
**Phases:** 71–75（5 phases / 20 plans）
**里程碑审计:** passed — 34/34 需求满足、integration_ok（数据链 contextvars→日志→指标→查询→告警→大盘端到端自洽）。详见 [v0.14.0-MILESTONE-AUDIT.md](./v0.14.0-MILESTONE-AUDIT.md)。

### Key Accomplishments

- **CTX（用户上下文贯穿）**：请求级 `RequestLogContextMiddleware`（最外层）+ `common/log_context.py` bind/clear + DRF 认证后 rebind mixin，自动注入 `user_id(无则 system)/request_id/source/trace_id`；后台任务（durable / `background_runner` / workflow scheduler / 飞书 webhook / apscheduler）经 `bind_task_context` 显式传播 `initiated_by_user_id`，跨线程/durable worker 正确继承。
- **LOG（系统日志治理）**：`SystemLogEntry`（migration 0009）队列化落库（`deque(maxlen=5000)` + daemon `bulk_create` + dropped/write_failed/written/queued 四计数，全 `except` 吞掉绝不反噬）；倒序 + 多维筛选 + 全文查询、调用下钻（MCP/对话复用 Interaction Ledger）、`category`(caller/sampling)+`component` 分类与事件目录、运行时配置热更新（级别/堆栈阈值/采样/保留）、webhook 原始统一留痕（`InboundWebhookEvent`：飞书/git push/容器回调/通用 workflow）、按条件清理 + 保留策略定时清理。
- **RATE（吞吐与速率）**：`RequestMetric`（migration 0010）每请求一行 + 33 处入口埋点（health/observability/poll 打 `synthetic` 隔离不污染）；扩展 `ModelUsageRecord`（`call_source`/`ttft_ms`/`upstream_status_code`，恰 22 类 `call_source`），补全容器侧 token 链 `task`→回调→`arecord_llm_usage(container_callback)`；`GaugeSample`（migration 0011）apscheduler ~45s 周期采样并发/排队趋势。
...
- **SNAP（当前快照）**：`snapshot_service` 五源 best-effort 聚合——host(psutil CPU/内存/协程/线程/后台) / DB(pg_stat_activity+max_connections+psycopg pool+PgBouncer opt-in) / Redis(cache·channels·llm 三路 INFO) / Qdrant(ping + 60s 缓存 + 8s 长超时) / 并发排队（provider 槽位·durable todo·doing·runner·RAG）。
- **QUERY（查询 API）**：时序查询 `MetricsQueryView`（Postgres `percentile_cont` 精确分位 + SQLite degrade，全白名单防注入）+ 快照 `MetricsSnapshotView` 五源聚合。
- **ALERT（告警引擎与通知）**：`SystemAlertRule`（migration 0012，独立 `system_alert_rules` 不套 workflow `AlertRule`）运行时 CRUD + `alert_evaluator` 9 类 metric 分派；`AlertEvent`（P0/P1/P2 + 中文标题 + 机器可读规则信息 + 持续时长 + firing/resolved + `UniqueConstraint(rule,target_key) condition=firing` 去重）；`alert_notifier` 邮件通道（Django SMTP + `SystemSetting` 收件人/开关，回写 `email_sent`）复用飞书/webhook 三通道并存。
- **UI（运维大盘）**：`pages/admin/observability/index.vue` 上半区（复合健康分 + 实时速率卡 + 6 信息卡 + 时间范围 + 上游 429·529 单列 + 时长·TTFT P99）+ `SnapshotRow`（内联阈值变色）+ `TrendCharts`（吞吐/错误三口径/时长/并发排队）+ `AlertEventsTable`/`AlertRulesPanel`/`AlertRuleFormDialog`/`AlertEventDetailSheet` + `SystemLogTable`（4 计数 + 倒序 + 服务端多维筛选）+ `LogDrilldownSheet` + `RuntimeLogConfigForm`；前端 `useQuery` 消费真实后端端点（无 mock）。
...
- 第一性原理：量级低、人触发 → **原始事件行 + Postgres `percentile_cont` 聚合**（精确分位），**不自研**进程内直方图/聚合器/多级 rollup 引擎；最大化复用已有 append-only 表（`ModelUsageRecord`/`RetrievalTrace` 仅补字段扩覆盖面）。
- **指标/留痕/日志三分**：指标=精简事件表，留痕=Interaction Ledger，日志=`SystemLogEntry`；三者用 `request_id/run_id/conversation_id` 关联，不互相复制；`labels` jsonb 禁放用户输入原文（取受控枚举防基数失控）。
- **告警另起系统级模型**：`workflows.AlertRule` 强绑工作流，不复用；新建 `SystemAlertRule` + `AlertEvent`，仅共享通知分发。
- **脱敏在全部入库路径前置**：log_sink/logging/webhook_recorder/ledger/drilldown/alert_notifier/space_tools 均经 `redact_credentials`/`redact_secrets_in_text`/`redact_for_ledger`，CI `test_credential_leak_protection` 守护不破。
- **快照按需不长存**：CPU/内存/DB/Redis/Qdrant 用户明确"只看当前"，不写长时序；只 `GaugeSample` 采并发/队列。

### Accepted / Deferred（非 gap，运行期才能终态确认）

- **SQLite `percentile_cont` 降级**：分位精确值仅 Postgres 可执行；本地 SQLite 走 MAX/AVG 兜底并标 `degraded=true`（方案 §A.4 预声明的设计边界，生产为 Postgres）。
- **运行期 UAT（~10 项）**：真实 SMTP 投递 / 真实指标越线驱动 firing→resolved / PG `pg_stat_activity` 连接分布 / 真实 Redis INFO 命中率 / 真实 Qdrant 枚举与缓存 / 浏览器亮暗双主题·四档断点响应式·快照阈值变色·实时刷新联动·下钻脱敏——代码路径均被 mock/transaction 测试覆盖，仅需真机肉眼/真实外部服务终态确认。
- **既有 structlog 跨模块测试污染**：pre-existing，非本里程碑引入；规范命令以 `-p no:randomly` 固定序运行全绿（269 后端 passed + 22 前端 passed + vue-tsc exit 0）。

### Technical Debt Incurred

- 无新增结构性技术债；Phase 71（LOG-06 分组件级别/堆栈阈值、LOG-07 通用 workflow webhook 留痕）、Phase 72（test-isolation）、Phase 75（429/529 单列、P99、5 个服务端日志维度）的初验 gap 均已在各自 phase 内闭合并补测。后续可观测性增量（Prometheus/OTLP 导出、跨进程分布式 tracing、告警降噪、Sentry、日志冷存储）已列 v2 Future Requirements（OBSX-01~06）。
