...
key-decisions:
  ...
  - "相位门的全量测试改在 SQLite 上跑：生产 Postgres 在 10.0.0.10（每查询 ~6ms RTT），跑到 25% 已耗时 75 分钟、预计 10 小时，不可行"
...
- `real` 行的 `add_nodes 1932ms` 含**跨网取数**（生产库在 10.0.0.10，每查询约 6ms RTT），
...
`cd server && uv run pytest` 首次在**生产 PostgreSQL**（`10.0.0.10:15432`，TCP 连接实测
