...
| 文件 | 规则 | 内容 | status |
|---|---|---| --- |
| `server/tests/mcp_tools/test_delivery_knowledge_tools.py:9` | F401 | `asgiref.sync.sync_to_async` imported but unused | resolved |
| `server/tests/mcp_tools/test_find_related_chunks.py:1` | I001 | Import block is un-sorted or un-formatted | resolved |
...
| 文件 | 规则 | 内容 | status |
|---|---|---| --- |
| `server/services/branch_search.py:109` | F841 | `base_collection` 赋值后未使用 | resolved |
| `server/services/project_context_packer.py:415` | I001 | 函数体内 import 块未排序 | resolved |
