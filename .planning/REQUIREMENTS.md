...
- [ ] **KMOD-01**: 系统以统一实体模型存储四类交付知识实体——需求/缺陷（work_item）、技术方案（tech_plan）、代码变更（code_change：diff/commit/MR）、文档（document：PRD/技术方案文档），实体携带稳定业务引用（`source_kind` + `source_id`）与来源（feishu/chat/mcp/workflow）及事件时间
- [ ] **KMOD-02**: 实体间关系以 bi-temporal 边存储（`valid_at`/`invalid_at` + `created_at`/`expired_at` 四时间戳），失效采用置位而非删除，历史可审计
- [ ] **KMOD-03**: 同一实体的多次修改形成版本链（supersedes），旧版本保留且可按版本号回溯
