...
> ⛔ **v0.20.0 技术方案蓝图（35 条 SCHEMA·LIFE·CHARTER·FLOW·CLAR·BUS·VIEW·GATE）已随里程碑归档到**
> [`milestones/v0.20.0-REQUIREMENTS.md`](./milestones/v0.20.0-REQUIREMENTS.md)（2026-08-02，34/35 满足 · GATE-01 PARTIAL）。
> 本文件因此**只保留仍在建的 v0.19.0 段** —— ⛔ 未按常规做法整份删除，理由是 v0.19.0 在
> `ROADMAP.md` 中仍是在建里程碑且其 24 条需求是它唯一的定义处，整份删除会让 ROADMAP 的
> `[requirements]` 链接悬空。下一里程碑立项（`$gsd-new-milestone`）时在本文件之上追加新段。
> ⚠️ 与 `milestone/v0.19.0-plan-trust` 合并时，本段以**对方分支为准**（STATE.md「并行开发」口径）。
