...
class TaskScheduler:
 ...
 """停止接收新任务（不可恢复）。"""
 self._accepting.clear
 log.info("scheduler_stop_accepting")
 async def wait_all_done(self, timeout: float = 300.0) -> None:
 """等待所有活跃任务完成，超时后返回。"""
 if not self._active_tasks:
 return
 log.info("waiting_active_tasks", count=len(self._active_tasks))
 tasks = list(self._active_tasks.values)
 _done, pending = await asyncio.wait(tasks, timeout=timeout)
 if pending:
 log.warning("force_shutdown", pending=len(pending))
 @property
