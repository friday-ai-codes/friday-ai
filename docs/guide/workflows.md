...
**核心概念：** 触发器接收事件 → 节点链依次处理 → 输出执行结果:: tip 前置阅读
如果你还没有部署 Friday，请先阅读 [快速开始指南](/guide/quick-start) 完成安装和基础配置。::
## 创建工作流
1. 进入目标项目页面
2. 点击左侧导航栏的「工作流」标签
3. 点击「创建工作流」按钮
...
| `auto_transition_status` | 方案生成后自动将工作项流转到指定状态 |:: tip 推荐配置
...
| `chat_id` | 飞书群 ID，用于发送结果通知和分支确认 |:: tip 执行模式
...
| `timeout_seconds` | 超时时间（秒），超时后工作流失败 |:: tip 典型用法
...
4. 点击「执行」开始运行
### 飞书事件触发运行
1. 确保飞书 Webhook 已正确配置（参考 [快速开始指南](/guide/quick-start)）
...
## 附录：节点类型速查表
以下表格列出 Friday 全部 26 种节点类型。`server_local` 表示在 Server 端执行，`runner_dispatched` 表示需要 Runner 执行。
| node_type | 显示名称 | 分类 | 执行模式 | 阻塞 |
|-----------|----------|------|----------|------|
| `feishu_event_trigger` | 飞书事件触发 | 触发器节点 | server_local | |
| `manual_trigger` | 手动触发 | 触发器节点 | server_local | |
| `webhook_trigger` | Webhook 触发 | 触发器节点 | server_local | |
| `ai_code_review` | AI 代码审查 | AI 节点 | runner_dispatched | 是 |
| `ai_coding` | AI 编码执行 | AI 节点 | runner_dispatched | 是 |
| `ai_coding_dispatcher` | AI 编码指派器 | AI 节点 | server_local | |
| `ai_plan_approval` | 方案审批 | AI 节点 | server_local | 是 |
| `ai_plan_generation` | AI 方案生成 | AI 节点 | server_local | 是 |
| `ai_prompt` | AI Prompt | AI 节点 | server_local | |
| `ai_variable_extractor` | AI 变量提取 | AI 节点 | server_local | |
| `context_retrieval` | 召回上下文 | AI 节点 | server_local | |
| `condition` | 条件分支 | 控制流节点 | server_local | |
| `delay` | 延迟 | 控制流节点 | server_local | |
| `human_approval` | 人工审批 | 控制流节点 | server_local | 是 |
| `join` | 并行汇合 | 控制流节点 | server_local | |
| `parallel` | 并行分支 | 控制流节点 | server_local | |
| `wait_feishu_field` | 等待飞书字段 | 控制流节点 | server_local | 是 |
| `create_branch` | 创建分支 | 操作节点 | server_local | |
| `create_pr` | 创建 PR | 操作节点 | server_local | |
| `fetch_project_info` | 获取项目信息 | 操作节点 | server_local | |
| `merge_pr` | 合并 PR | 操作节点 | server_local | |
| `variable_extractor` | 变量提取 | 操作节点 | server_local | |
| `fetch_work_item` | 获取工作项详情 | 集成节点 | server_local | |
| `http_request` | HTTP 请求 | 集成节点 | server_local | |
| `mcp_deploy` | MCP 部署 | 集成节点 | server_local | |
| `notify_feishu` | 飞书通知 | 集成节点 | server_local | |
