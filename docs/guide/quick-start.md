...
::: details LLM API 错误：`LLM API 错误: 401` 或 `未配置 Anthropic API Key`
未配置或配置了无效的 AI Provider Key。
...
::: details Schema 验证失败：`技术方案验证失败: ...`
AI 生成的技术方案不符合预期的 JSON Schema 结构。
...
::: details 飞书回填失败：`飞书回填失败: ...` 或 `FeishuAPIError`
飞书应用权限不足或字段配置错误。
...
::: details 工作流卡住：状态长时间显示「运行中」或「等待事件」
某个节点执行失败或等待条件未满足。
...
::: details 编码任务创建失败：`所有任务创建失败` 或 `缺少 workflow_execution 上下文`
技术方案的执行计划中任务配置不完整。
