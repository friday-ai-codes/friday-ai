...
# worker 打 OOM。
#
# 预算算术（常数已于 2026-08-09 由 Plan 121-10 在**生产库**最大仓上复校，见
# services/code_graph/cache.py 的常数注释）：NODE_COST=800 字节、EDGE_COST=680 字节，
# 生产实测**准入口径**边:节点 = 3.40:1（RESEARCH 原假设 3:1，实测略高）
# → 单图约 n × (800 + 3.4×680) = n × 3112 字节。据此：
#   - CODE_GRAPH_MAX_GRAPH_BYTES = 256MB → 单仓约 **8.6 万**符号即触顶，超过则不进
#     缓存、改走「按需子图」降级路径（返回值带 degraded 标记，由上层工具透出）。
#     ⚠️ 复校前这条写的是「约 11 万符号」——常数上调 + 边:节点比实测偏高，两头都往
#     收紧的方向走。当前生产最大仓 3 万符号，离触顶还远，这次收紧不影响任何现有仓。
