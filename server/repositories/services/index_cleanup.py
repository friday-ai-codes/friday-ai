...
- ``ChunkEdge`` 必须在 ``ChunkRegistry`` 之前删除（per context contract）：当下
  ``ChunkEdge.source_chunk_id / target_chunk_id`` 是 UUID 字段非 DB FK（per
  ``code_relations.models.ChunkEdge`` 注释 contract），但保此序方便日后改 FK 时
  零破坏。
...
# Qdrant collection 删除单步上限（per work item）：collection delete 一般 < 5s，
...
async def _cleanup_vector_artifacts(repo_id: str) -> dict[str, Any]:
    ...
    每步独立异常隔离（per contract），单步失败降级为 0 / False。
    """
    ...
    # ChunkEdge 必须先于 ChunkRegistry — 当前两表无 DB FK，仅语义顺序（contract）
...
async def _cleanup_graph_artifacts(repo_id: str) -> dict[str, int]:
    ...
    每步独立异常隔离（per contract），单步失败降级为 0。
    """
...
async def cleanup_index(
    ...
    *,
    ...
    Returns:
        ...
        可以放心返回 204（per context contract）。``graph_artifacts_cleared`` 按
...
async def _delete_chunk_registries_raw(repo_id: str) -> int:
    """整仓 cleanup：用 `_raw_delete` 跳过 pre_delete signal 噪音（work item）。
...
async def _delete_count(model: Any, repo_id: str, *, label: str) -> int:
    ...
    单步异常吞掉 + log warning + 返回 0（per contract 异常隔离）。
    """
