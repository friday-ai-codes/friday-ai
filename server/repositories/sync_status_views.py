...
class SyncStatusView(APIView):
    """GET /api/repositories/{id}/sync-status/ — 同步状态查询（work item）。"""
    ...
    async def get(self, request: Request, repository_id: uuid.UUID) -> Response:
        ...
        # 最近 5 条历史（contract recent_history[5]）
        recent_history = [
            {
                "id": str(h.id),
        ...
        # contract 倒计时数据源：DjangoJob.next_run_time
        next_sync_at = None
        try:
