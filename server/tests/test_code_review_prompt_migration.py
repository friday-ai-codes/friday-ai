"""Phase / Phase: code_review prompt 迁移字节级等价测试。
Phase Wave（ Task 2） 迁移后：
...
- ** 强化**：``expected_sha`` 本次固化值禁止修改；若 fixture 变量或 Prompt 源漂移导致
 hash 变化，**修 fixture**（固定 execution_id / node_id / mr_ids）或修回 Prompt 路径，
 **不得**更新 expected_sha。
T- 威胁缓解：禁止通过"更新 expected hash 掩盖 Prompt 漂移"路径规避本测试。
"""
...
# ---------------------------------------------------------------------------
# Phase：_CapturingFake seam 注入 + 字节级 hash 守护
# ---------------------------------------------------------------------------
...
def _make_fixed_context -> ExecutionContext:
 """ 守护：固定 execution_id / node_id / mr_ids，使 system prompt 稳定。
...
def capturing_code_review_fake(monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
 """Phase：_CapturingFake seam 注入（与 fake_chat_model_factory 同路径覆盖）。
...
async def test_code_review_system_prompt_byte_equal(
 ...
 """ 强化 / T-：AICodeReviewNode.execute 传入 LLM 的 system prompt
 ...
 **禁止通过更新 expected_sha 修复本测试。**
 ...
 # 强化：expected_sha 固定为 REVIEW_SYSTEM_PROMPT 自身 sha256
 ...
 f" 守护：禁止更新 expected_sha；请检查 render_prompt / Prompt DB seed "
 f"/ SystemMessage 包装路径。"
 )
 # 同时断言字节级字符串相等（更强的语义；若 sha 相等字符串必然相等）
 assert system_content == REVIEW_SYSTEM_PROMPT, (
 "system_content 应与 REVIEW_SYSTEM_PROMPT 字节级相等（ 守护）"
 )
