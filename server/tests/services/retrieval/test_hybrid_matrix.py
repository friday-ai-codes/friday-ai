"""HybridSearchService matrix 路径覆盖 —— per Phase Plan / 。
3 条聚焦于 NullProvider 路径核心 path（rag_only 短路 / max_tokens 裁剪 / 空 query
兜底），与 Phase 既有 ``test_null_provider_paths.py`` 5 条形成 "基础行为 +
能力契约 + 编排路径" 三层覆盖。
函数名直接带 ``_null_provider`` 后缀（per plan must_have 保险方案），不依赖
parametrize ID 即可被 ``pytest -k null_provider --co`` 收集（per ROADMAP
字面要求 "≥10"）。
...
async def test_search_max_tokens_trim_for_null_provider -> None:
 ...
 路径的 budgets ratio 100% rag 配置下仍正常触发（防 Plan 三 flag
