"""``ENABLE_GRAPHRAG_ENRICHMENT`` flag + caller 参数四组合行为锁（per Phase Plan）。
 三 flag 语义中"读出侧" flag 的入口守卫测试。Phase 落 ``enable_graph_enrichment``
caller 参数；Phase 在 ``HybridSearchService.search`` 入口**追加** settings 读取，
两者 AND 合并：任一为 False → 强制 ``_search_rag_only`` 路径（即使 Provider 是
GraphCapableProvider），byte-equivalent 兑现 Phase NullProvider 路径。
...
async def test_enrichment_default_true_graph_capable_active -> None:
 ...
 "HybridSearchResult 必须暴露 hop1_neighbors 字段（per Phase）"
 )
 assert result.hop1_neighbors ==, (
 "items 无 related_chunks → 一跳邻居应为空"
 )
# ---------------------------------------------------------------------------
# Test 2：settings=False 强制 rag_only（Phase 新行为，本 plan 入口守卫验收）
# ---------------------------------------------------------------------------
async def test_enrichment_settings_false_forces_rag_only -> None:
 ...
 Phase Plan 核心交付：HybridSearchService.search 入口读 settings + 与
 ...
 "rag_only 路径应保 Phase markdown section 原貌"
 )
# ---------------------------------------------------------------------------
# Test 3：caller=False 强制 rag_only（Phase 既有行为回归保护）
# ---------------------------------------------------------------------------
async def test_enrichment_caller_false_forces_rag_only -> None:
 """``enable_graph_enrichment=False`` (caller 参数) 强制 rag_only。
 Phase 既有行为回归保护：caller 不需要二跳扩散时主动短路，与
 Phase settings flag 独立（两者 AND 合并）。
 """
 items = [_l3_item("src/auth/login.py", "def login(req): ...")]
 snapshot = _make_l3_snapshot(items)
 with _patch_search_rag(snapshot), _patch_local_lookup_symbols:
