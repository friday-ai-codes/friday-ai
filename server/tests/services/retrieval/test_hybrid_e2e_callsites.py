"""Phase Plan Task 2 — agent / workflow / chat 三路径 E2E callsite 契约。
6 条核心断言验证 ROADMAP §Phase "agent / workflow / chat 三路径调用
...
def test_no_layered_search_direct_import_in_callsites -> None:
 ...
 （per ROADMAP §Phase "agent/workflow/chat 三路径全部通过
 HybridSearchService"）。
 允许命中：
