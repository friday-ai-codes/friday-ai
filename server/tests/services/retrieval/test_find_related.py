"""find_related Python API 单测（per Phase Plan Task 2）。
兑现 第三条 "MAX_HOPS=2 硬上限" 在 find_related API 入口（防 LLM 通过
Phase MCP tool 传 hops=10 引发指数级 ORM 扩散）。
...
14. ``test_hybrid_search_service_has_find_related_method`` —— HybridSearchService.find_related thin
 wrapper 暴露（per Plan Phase MCP tool 直接调用入口）
"""
...
async def test_hybrid_search_service_has_find_related_method(repository) -> None:
 ...
 Phase MCP tool 直接调用入口（per Plan success_criteria："Phase MCP
