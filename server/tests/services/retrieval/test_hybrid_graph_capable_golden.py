"""Phase Plan Task 1 — graph_capable 路径 golden snapshot 二轮 baseline。
10 条 fixture × 字节级 `final_context` 比对，锁定 Plan 落地的 GraphRAG 编排器
（`HybridSearchService._search_graph_capable`）输出契约。
设计与 Phase `tests/codegraph/test_layered_search_golden.py` 同 idiom：
...
def _split_fixture(text: str) -> tuple[str, str]:
 """切分 `{body}\n\n# tokens=...` 返回 (body, metadata_line)，与 Phase idiom 一致。"""
