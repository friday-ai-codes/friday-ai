"""`NeighborMetadata` + `HybridSearchResult` 行为测试（per Phase / ）。
覆盖 4 条 assertion：
...
4. RagSearchResult 兼容路径：to_rag_result 返回值确为 RagSearchResult 实例
 （Plan 选 to_rag_result 显式转换路径而非继承，per Plan deviation -b）
"""
...
def test_neighbor_metadata_allows_nullable_line_fields -> None:
 """ fallback：line_start / line_end 必须接受 None（历史 chunk 未回填）。"""
 nm = NeighborMetadata(
