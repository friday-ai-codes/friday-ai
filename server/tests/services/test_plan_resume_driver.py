"""adrive_plan_session_to_pause_or_terminal 共享续驱 helper 单测（RESUME-01 步骤 1）。

覆盖四条路径：
- 终态返回（DONE / advance 一步到 DONE）→ 返回 session，终态不再 advance。
- researching 在途短路 → 立即返回原 session，engine.advance 未被调用。
- clarifying 在途短路（BLOCKER 守护，保护澄清 HITL）→ 立即返回原 session，advance 未被调用。
- step 上限 fail → 超过 max_steps 后经 session_service.transition(session, "fail") 标记失败返回。

pytest-socket 禁网——全部 IO 在 ORM（真实 PlanSession/RepoResearchTask/Clarification）+
engine MagicMock 边界。
"""
...
from services.plan_orchestration import adrive_plan_session_to_pause_or_terminal
...
async def _make_session(status: PlanSessionStatus) -> PlanSession:
...
async def test_terminal_returns_without_advance() -> None:
    ...
    session = await _make_session(PlanSessionStatus.DONE)
    engine = _engine()

    result = await adrive_plan_session_to_pause_or_terminal(engine, session)
...
async def test_terminal_advance_one_step_to_done() -> None:
    """advance 一步把 MERGING → DONE → 返回该 session，advance 仅调用一次。"""
    session = await _make_session(PlanSessionStatus.MERGING)
    engine = _engine()

    async def _advance(_s: PlanSession) -> None:
        await PlanSession.objects.filter(id=session.id).aupdate(
    ...
    assert result.status == PlanSessionStatus.DONE
    assert engine.advance.await_count == 1


@pytest.mark.asyncio
async def test_researching_pending_short_circuit() -> None:
    """researching 且有 RUNNING 调研任务（在途）→ 立即返回，advance 未调用。"""
...
async def test_clarifying_pending_short_circuit() -> None:
    """clarifying 且有未答 Clarification（answered_at 为空）→ 立即返回，advance 未调用（保护 HITL）。"""
    session = await _make_session(PlanSessionStatus.CLARIFYING)
...
async def test_step_limit_transitions_to_fail() -> None:
    ...
    session = await _make_session(PlanSessionStatus.ROUTING)
    engine = _engine()

    result = await adrive_plan_session_to_pause_or_terminal(engine, session, max_steps=3)
