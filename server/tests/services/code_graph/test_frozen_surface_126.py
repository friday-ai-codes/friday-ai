"""Phase 126 冻结面守卫（D-16 / T-126-06）。

本相位新增内核不得 import ``repo_router_v2``；
本相位提交不得改写 ``mcp/`` submodule（允许 ``server/mcp_tools/views.py`` adapter）。
文件尚未落地时 path.is_file() continue，不 fail。
"""
...
_NO_ROUTER_IMPORT = (
    "services/code_graph/process_trace.py",
    "services/code_graph/rename_preview.py",
    "services/process_enqueue.py",
)
...
def test_phase_126_does_not_touch_repo_router_v2() -> None:
    """静态：process/rename/enqueue 模块不 import ``repo_router_v2``；可选 git 守卫。

    （Req: D-16, 威胁: T-126-06）
    """
    violations: list[str] = []
    ...
    assert bad == [], f"126-* commits touched frozen paths: {bad}"


def test_phase_126_does_not_modify_mcp_submodule() -> None:
    ...
    assert bad == [], f"126-* commits touched mcp/ submodule: {bad}"
