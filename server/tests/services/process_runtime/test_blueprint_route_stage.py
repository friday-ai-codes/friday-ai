...
_GREENFIELD_POINTS = [
    _point("fp_02", "专项学习页", "greenfield", "新增示例功能专项学习页，展示专项课程内容。")
]
...
async def test_normal_path_orders_by_total_and_emits_event() -> None:
    ...
    high = await _make_repo("sample_course_service")
    low = await _make_repo("sample_practice_service")
    router = _router(
        [
            _rv2_candidate(str(low.id), "sample_practice_service", 0.30),
            _rv2_candidate(str(high.id), "sample_course_service", 0.90, confidence="high"),
        ]
    )
...
async def test_breakdown_total_equals_component_sum_end_to_end() -> None:
    ...
    session = await _make_session(_spec(_GREENFIELD_POINTS))
    repo = await _make_repo("sample_course_service")
    router = _router([_rv2_candidate(str(repo.id), "sample_course_service", 0.7)])
...
async def test_charter_planned_team_owner_remains_candidate() -> None:
    ...
    """
    session = await _make_session(_spec(_GREENFIELD_POINTS))
    routed = await _make_repo("sample_course_service")
...
async def test_charter_component_fully_explains_ranking_difference() -> None:
    ...
    session = await _make_session(_spec(_GREENFIELD_POINTS))
    owned = await _make_repo("sample_course_service")
    plain = await _make_repo("sample_practice_service")
    await _make_charter(owned, owned_domains=[{"domain": "专项学习页", "status": "implemented"}])
    router = _router(
        [
            _rv2_candidate(str(owned.id), "sample_course_service", 0.5),
            _rv2_candidate(str(plain.id), "sample_practice_service", 0.5),
        ]
    )
...
async def test_boundary_hit_candidate_is_penalized_not_dropped() -> None:
    ...
    control = await _make_repo("sample_practice_service")
    await _make_charter(blocked, boundaries=[{"rule": "不承接专项学习页与课程权益鉴权"}])
    router = _router(
        [
            _rv2_candidate(str(blocked.id), "study-plan", 0.9),
            _rv2_candidate(str(control.id), "sample_practice_service", 0.9),
        ]
    )
...
async def test_maintenance_only_candidate_gets_extra_penalty() -> None:
    ...
    active = await _make_repo("sample_course_service")
    ...
    router = _router(
        [
            ...
            _rv2_candidate(str(active.id), "sample_course_service", 0.6),
        ]
    )
...
async def test_routing_contract_keys_present() -> None:
    ...
    router = _router(
        [
            _rv2_candidate(str(high.id), "sample_course_service", 0.9, confidence="high"),
            _rv2_candidate(str(low.id), "sample_practice_service", 0.2, confidence="low"),
        ]
    )
...
async def test_route_confidence_and_charter_do_not_decide_role() -> None:
    ...
    session = await _make_session(_spec(_GREENFIELD_POINTS))
    high_conf = await _make_repo("sample_course_service")
    ...
    router = _router(
        [
            _rv2_candidate(str(high_conf.id), "sample_course_service", 0.9, confidence="high"),
            _rv2_candidate(str(charter_owned.id), "sample_service_service", 0.4, confidence="low"),
            _rv2_candidate(str(plain.id), "sample_practice_service", 0.4, confidence="low"),
        ]
    )
    ...
    session = await ConvergenceSession.objects.acreate(
        ...
        stage_state={
            "decomposition": {
                ...
                "extra_evidence": [{"source": "technical_doc", "text": "调用 sample_course_service"}],
            }
        },
    )
...
async def test_intent_recorded_from_feature_points() -> None:
    ...
    repo = await _make_repo("sample_course_service")
    router = _router([_rv2_candidate(str(repo.id), "sample_course_service", 0.5)])
...
async def test_history_unavailable_is_explicit_not_silent_zero() -> None:
    ...
    session = await _make_session(_spec(_GREENFIELD_POINTS))
    repo = await _make_repo("sample_course_service")
    router = _router([_rv2_candidate(str(repo.id), "sample_course_service", 0.6)])
...
async def test_v1_fallback_router_version_is_visible_in_evidence() -> None:
    ...
    repo = await _make_repo("sample_course_service")
    router = _router(
        [_rv2_candidate(str(repo.id), "sample_course_service", 0.6, matched_node_paths=[])],
