"""process_runtime 入口无关 ``ask_clarification`` helper 守护测试（CLARIFY-03，90-04）。

**这是 ``services.process_runtime`` 的编排 helper，不是 chat agent tool**。仓内另有同名
...
导入路径与之**显式区分**，断言对象为 ``from services.process_runtime import ask_clarification``。
...
from services.process_runtime import ask_clarification
...
def test_ask_clarification_is_process_runtime_not_chat_tool() -> None:
    """命名撞车防护：本 helper 模块路径与 chat tool 区分，是 process_runtime 资产。"""
    assert ask_clarification.__module__ == "services.process_runtime.ask_clarification"
