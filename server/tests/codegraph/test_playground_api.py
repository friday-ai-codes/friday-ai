"""PlaygroundSearchView API 测试 —— work item 覆盖。
...
def test_serialize_neighbor_falls_back_when_partial_mock(admin_client):
    """work item 后端兜底：partial mock weight=None / reason=None / hop=None →
...
def test_max_tokens_invalid_returns_400(admin_client):
    """contract：max_tokens 传入非整数字符串 → 400 Bad Request 而非 500 Internal Server Error。"""
    response = admin_client.post(
        PLAYGROUND_URL,
