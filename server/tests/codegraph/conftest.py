...
Phase Plan（golden snapshot baseline）追加：
- `GOLDEN_QUERIES_REGISTRY`：20 条 parametrize 入口常量（per ）
- `golden_mock_environment` fixture：确定性 mock 五个外部依赖（per ）
"""
...
def repo_file_indexes(repo_for_summary):
 ...
 return indexes
# =============================================================================
# Phase Plan: Golden Snapshot Baseline fixtures
#
# 锁定 LayeredSearchService.search 的 final_context 现状行为，作为后续 RAG
# 解耦阶段的"零漂移"门禁（Pitfall 4 per ROADMAP §Phase Success Criteria #2）。
#
# 设计要点（per / / ）：
