...
保证套件可收集、不报 collection error；/04 实现落地后这些断言自动生效（RED→GREEN）。
覆盖需求：
-：创建返回一次性明文（friday_pat_ 前缀）
-：list/任何字段绝不含明文
-：有效 token 认证放行，无 scope 分权（ single token）
-：吊销/过期 token 被拒并写 DENIED run
"""
...
def test_valid_token_passes(
 ...
 """：有效 token 认证放行（无 scope 校验， single token）。"""
...
def test_revoked_expired_denied_and_logged(
 ...
 """：吊销 token 被拒，并写一条 DENIED run（fingerprint 入库，明文不入）。"""
 auth_mod = pytest.importorskip("access_tokens.authentication")
 ...
 # denial 记录里绝不含明文（raw_request 已脱敏，）
