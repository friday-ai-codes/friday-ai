"""Phase：Context window 预算策略测试。
覆盖 CONTEXT 全部子项：
- `_CONTEXT_SAFETY_BUFFER = 800` 硬编码、禁暴露节点配置
- 预算公式 ``max_input - max_output - 800``
- strict_error 抛 ContextWindowExceededError（错误消息含 5 关键字段）
- auto_trim 策略 + trim_messages(strategy='last', include_system=True)
 + BUDGET_WARNING event data 六字段锁（warning_type / trimmed_count /
 budget / original_tokens / model / session_id）
- auto_trim 不改变 messages 时不发 BUDGET_WARNING
...
def test_safety_buffer_is_800 -> None:
 """：_CONTEXT_SAFETY_BUFFER 硬编码 800，禁暴露节点配置。"""
 assert _CONTEXT_SAFETY_BUFFER == 800
def test_context_safety_buffer_not_configurable -> None:
 """：LangChainRunnerConfig 无 context_safety_buffer 字段（禁暴露配置）。"""
...
async def test_strict_error_raises(
 ...
 """：strict_error 策略 + 超预算 → ContextWindowExceededError。"""
...
async def test_strict_error_message_format(
 ...
 """：ContextWindowExceededError 消息必须含 5 关键字段。"""
 caps = _make_caps(max_input_tokens=100, max_output_tokens=50)
 config = _make_config(
 resolved_stub, caps=caps, context_strategy="strict_error"
 )
 ...
 msg = str(exc)
 # 5 关键字段均在错误消息中出现（便于前端 Phase 降级引导）
...
async def test_auto_trim_yields_warning(
 ...
 """：auto_trim 策略 + 超预算 → 发射 BUDGET_WARNING event。"""
 caps = _make_caps(max_input_tokens=100, max_output_tokens=50)
 config = _make_config(
...
async def test_auto_trim_warning_fields(
 ...
 """：BUDGET_WARNING.data 字段锁（前端 SSE 合约冻结）。
...
async def test_auto_trim_preserves_system_message(
 ...
 """：auto_trim 使用 `include_system=True`，SystemMessage 必保留。
...
async def test_auto_trim_no_effect_when_under_budget(
 ...
 """：当 current <= budget 时 auto_trim 不触发，不发 BUDGET_WARNING。"""
...
# ============================================================================
#：预算公式边界 + max_output_tokens 覆盖
# ============================================================================
def test_budget_formula_exact(
 resolved_stub: ResolvedProviderConfig,
) -> None:
 """：预算公式 budget = max_input - max_output - 800。
...
def test_max_output_exceeds_budget_strict(
 ...
 """：config.max_output_tokens 覆盖 caps.max_output_tokens 参与预算计算。
...
async def test_last_trim_meta_initialized_empty(
 ...
 """ 集成：__init__ 初始化 _last_trim_meta 为 {}，短消息场景下不变。"""
