"""Phase 契约：SSE 事件流跨 5 Provider 字段名一致性（20 contract tests）。
参数化维度：
...
断言字段名 keys 跨 Provider 完全一致（CONTEXT data 字段锁定表）。
参考：
- CONTEXT 字段锁
...
async def test_text_delta_fields_cross_provider(
 ...
 """ /：TEXT_DELTA.data 字段跨 5 Provider 一致。
...
async def test_tool_use_start_fields_cross_provider(
 ...
 """ /：TOOL_USE_START.data 字段跨 5 Provider 一致。
...
async def test_tool_use_result_fields_cross_provider(
 ...
 """ /：TOOL_USE_RESULT.data 字段跨 5 Provider 一致。
...
async def test_message_complete_fields_cross_provider(
 ...
 """ /：MESSAGE_COMPLETE.data 字段跨 5 Provider 一致。
...
async def test_fields_names_identical_across_5_providers(
 ...
 """ 铁律：同一事件类型，5 Provider 发出的 data keys 集合**完全**相等。
