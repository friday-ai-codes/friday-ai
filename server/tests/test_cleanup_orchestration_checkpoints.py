"""Phase 测试 — cleanup_orchestration_checkpoints management command。
覆盖 VALIDATION.md 锁定的 test 名：
- test_normal_cleanup
- test_dry_run
...
def test_normal_cleanup(
 ...
 """ 正常清理：COMPLETED + ERROR + updated_at<now-7d 都被删除，adelete_thread 被调用。"""
 out = StringIO
...
def test_dry_run(
 ...
 """ dry-run：不真删，输出 '[dry-run] would delete ...'。"""
 out = StringIO
 call_command(
...
def test_status_filter_excludes_running(
 ...
 """ status filter：10 天前但 status=RUNNING 的运行不被清理（T- mitigation）。"""
...
def test_empty_no_crash(db: None, mock_saver: AsyncMock) -> None:
 """ 边界：0 条符合条件时正常完成，adelete_thread 调用 0 次。"""
 out = StringIO
 call_command("cleanup_orchestration_checkpoints", "--days=7", stdout=out)
 mock_saver.adelete_thread.assert_not_awaited
 # 成功输出中含 "0 runs"
...
def test_adelete_thread_failure_does_not_stop_batch(
 ...
 """ 异常隔离：单 thread 删除失败不阻塞整批（T- mitigation）。"""
 # 第一次抛错，第二次成功
...
def test_batch_size_boundary_500(
 ...
 """ batch_size=500 边界：501 条 thread_id 应被分 2 批 ORM delete，
