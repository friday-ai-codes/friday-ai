"""Phase Plan — 增量索引 3 callsite `adelete_for_files` 孤儿清理测试。
覆盖 GRAPH-：
...
- 全量索引路径（`run_full_index`，CONTEXT 决议）**不加** delete hook，
 Phase GRAPH- 落 `build_graph_for_repository` 时再统一处理。
...
# ---------------------------------------------------------------------------
# 端到端：直接调 adelete_for_files 验证孤儿确实被删除（GRAPH- ）
# ---------------------------------------------------------------------------
