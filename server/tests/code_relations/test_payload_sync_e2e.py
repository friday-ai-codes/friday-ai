"""Phase payload sync E2E（per ）。
100 chunks + 50 edges 走 `_run_all_builders_and_sync_payload` 全链路；
断言 batch_set_payload 被调**恰好一次**（per / ）+ payload 满足
top-20 + 5KB 约束（per ）+ 50 ChunkEdge 入库（per ）。
...
async def test_e2e_100_chunks_50_edges_single_batch_set_payload(repository) -> None:
 ...
 断言矩阵：
 - mock_batch.call_count == 1（per ）
 - len(updates_arg) == 50（前 50 有边的 chunk）
 - 每 update payload "related_chunks" 长度 ≤ 20（per top-N）
 - 每 update payload bytes < 5 KB（per size cap）
 - DB ChunkEdge.objects.acount == 50（per 入库）
 """
 ...
 f"batch_set_payload 应仅调一次（per ），实际 {mock_batch.call_count} 次"
 )
