"""CoChangedEdgeBuilder Pitfall RSS perf gate（per Phase / ）。
...
class _LightStdout:
 ...
 ``call_args_list`` 造成 GB 级假阳性内存（per measurement 隔离）。
 """
 __slots__ = ("_iter",)
...
async def test_co_changed_100k_commits_rss_under_1gb(tmp_path) -> None:
 """100k commits 流式入 builder，tracemalloc peak < 1 GB（Pitfall RSS gate ）。"""
 ...
 f"{peak / 1024**2:.1f} MB, required < {_RSS_LIMIT_BYTES / 1024**2:.0f} MB (per )"
 )
