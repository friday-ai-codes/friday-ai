"""ImportEdgeBuilder 测试（per initial implementation contract）。"""
...
async def test_resolve_dot_dot_parent_import(repository) -> None:
    """work item 回归：``from ..utils import X`` (target_module=``..utils``) 应解析
...
async def test_resolve_single_dot_same_package(repository) -> None:
    """work item 回归：``from .helper import Y`` (target_module=``.helper``) 应解析
...
async def test_vue_target_module_resolution(repository) -> None:
    """initial implementation：CANDIDATE_EXTENSIONS 含 .vue 后，target_module=components.Button
