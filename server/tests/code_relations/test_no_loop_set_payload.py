"""Pitfall 2 防御 grep gate（per Phase）。
禁止在 server/code_relations/ 内出现「for ...: ... set_payload(...)」循环单点
set_payload 模式；payload sync 必须走 QdrantService.batch_set_payload（Plan
交付）+ 底层 client.batch_update_points + SetPayloadOperation。
ripgrep 在 macOS / Linux CI 均可用；项目 251 Plan test_migration.py 已确立
