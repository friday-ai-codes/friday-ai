...
1. test_marks_running_before_dispatch
   ...
   写为 RUNNING（同步路径，per context contract）。
...
3. test_completion_marks_failed_on_exception
   ...
   写为 "failed"，payload_synced_at 保 None（per contract 失败语义）。
...
async def _drain_background_tasks() -> None:
    """循环 drain `_BACKGROUND_TASKS` 直到为空（work item race-fix 配套）。
...
async def test_multi_task_completion_marks_completed_only_after_last(
    ...
    """work item regression：enqueue spawn 多个 task 时，每个都注册 callback；
    仅最后一个 done 才写 COMPLETED。

    旧实现 `max(new_tasks, key=id)` 只给一个 task 加 callback；若 enqueue
    内部 spawn N>1 个 task（contract 列出的合理演进），剩余 N-1 个完成时不会
...
async def test_no_task_spawned_keeps_running_not_failed(repository) -> None:
    """work item regression：enqueue 合法不 spawn task（dedup 等）→ 保 RUNNING，不写 FAILED。"""
    history = await _make_history(repository)
    dirty = [uuid.uuid4()]
...
def test_enqueue_edge_build_no_await_in_body() -> None:
    """work item contract：`enqueue_edge_build` 函数体内禁止 await。
    ...
    assert not re.search(r"\bawait\b", body_text), (
        "enqueue_edge_build 函数体内禁止 await（work item 契约）；改动需同步更新 "
