...
async def test_dual_write_no_integrity_error(repository) -> None:
 """：base + feature 双写同三元组不抛 IntegrityError，两行并存。"""
 source = uuid.uuid4
 target = uuid.uuid4
 base_edge = ChunkEdge(
...
async def test_feature_edge_branch_name(repository) -> None:
 """：feature 路径构建的 ChunkEdge.branch_name 正确且只读本分支行。"""
...
async def test_enqueue_threads_branch(repository) -> None:
 """：enqueue_edge_build_for_history → build 链透传 branch_name。"""
