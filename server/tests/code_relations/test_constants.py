"""`code_relations.constants` 暴露 Phase / Phase 编排器共享常量。
覆盖 assertion：
Phase（既有）：
...
Phase Plan（新增）：
- CO_CHANGED_WINDOW_COMMITS == 2000（per，CoChangedEdgeBuilder 滑窗）
- SEMANTIC_SCORE_THRESHOLD == 0.85（per Phase，SemanticEdgeBuilder Qdrant 阈值）
...
# ---------------------------------------------------------------------------
# Phase Plan：新增 CO_CHANGED_WINDOW_COMMITS / SEMANTIC_SCORE_THRESHOLD
# ---------------------------------------------------------------------------
def test_co_changed_window_commits_literal_value -> None:
 """``CO_CHANGED_WINDOW_COMMITS`` 字面值锁定 == 2000（per Phase）。"""
...
def test_semantic_score_threshold_literal_value -> None:
 """``SEMANTIC_SCORE_THRESHOLD`` 字面值锁定 == 0.85（per Phase 防漂移）。"""
...
def test_phase_256_constants_not_env_derived -> None:
 ...
 与 Phase 三常量同款防御：扫描源码，断言 RHS 是纯字面赋值，
...
def test_existing_constants_unchanged_regression_guard -> None:
 """既有常量 MAX_NEIGHBORS_PER_CHUNK / MAX_HOPS 未被改动（Phase 回归保护）。
 Phase Plan ``<action>`` 明确"不修改既有常量"——本测试在新增常量
 PR 引入意外副作用时锁定既有值。
 """
