...
- batch_set_payload 调**恰好一次**（per contract，不是 6 次 per builder）
...
async def test_top_level_exception_logged_not_silent(repository) -> None:
    """work item 回归：bulk_insert_edges 抛 RuntimeError → 顶层 try/except 捕获 +
...
async def test_create_task_strong_reference_held(repository) -> None:
    """work item 回归：enqueue_edge_build spawn 的 task 被 _BACKGROUND_TASKS 强持有，
