...
2. test_handler_schedules_reconcile_with_distinct_sources
   ...
   sources 为去重后的所有 source_chunk_id（per contract dirty source 收集）。
...
4. test_handler_exception_does_not_block_delete
   ...
   ChunkRegistry 行仍被成功删除（per contract 异常隔离）。
5. test_reconcile_not_triggered_on_rollback
   transaction.atomic 回滚时 `_schedule_reconcile` 不被调用（per contract
...
def _reset_pending_state() -> None:
    """每条用例前后清空 thread-local pending dict（work item burst accumulator）。
...
def test_handler_deletes_inbound_chunk_edges(repository) -> None:
    """删 target chunk → 指向它的 2 条 ChunkEdge 同步被清理（孤儿边自动清理 per contract）。"""
...
def test_burst_delete_collapses_to_single_schedule(repository) -> None:
    """work item regression：QuerySet.delete() 删 N 个 chunk → 仅 1 次 schedule_reconcile。
...
def test_handler_exception_does_not_block_delete(repository) -> None:
    """handler 内 ORM 抛错 → catch + log warning + ChunkRegistry 删除仍成功（per contract）。"""
