...
保证套件可收集、不报 collection error；/03 实现落地后这些断言自动生效（RED→GREEN）。
覆盖需求：
-：create_interaction_run 同步创建并必成功（可追踪）
-：8 类 event_type 事件级入库
-：tool call 记录 + parent_event 父子 trace
-：model usage 记录
- best-effort：子事件写入失败降级不抛
-：redact_for_ledger 脱敏（friday_pat_ / sk-ant- / nested dict）
"""
...
def test_run_created_synchronously -> None:
 """：create_interaction_run 同步创建 InteractionRun 并落库。"""
...
def test_tool_call_record -> None:
 """：tool call 记录 + parent_event 父子 trace 关联。"""
 ledger = pytest.importorskip("interactions.ledger")
