...
class TestGitHubWebhook:
 """GitHub Webhook 接口测试。"""
 def test_github_webhook_pr_merged(self, api_client, urls):
 """测试 GitHub PR 合并 webhook。"""
 response = api_client.post(
 urls.github_webhook,
 {
 "action": "closed",
 ...
 assert response.status_code == status.HTTP_200_OK
 assert response.data["status"] == "accepted"
 def test_github_webhook_pr_closed_not_merged(self, api_client, urls):
 """测试 GitHub PR 关闭但未合并。"""
 response = api_client.post(
 urls.github_webhook,
 {
 ...
 assert response.status_code == status.HTTP_200_OK
 def test_github_webhook_invalid_json(self, api_client, urls):
 """测试 GitHub webhook 无效 JSON。"""
 response = api_client.post(
 urls.github_webhook,
 "invalid json",
