...
Threat mitigation:
- security mitigation-01 / security mitigation-02：静默 fallthrough 到系统凭证（认证混淆）→ 保存前拒绝
- security mitigation-03：空白字符串 ``"   "`` 退化形式走 .strip() 覆盖
- security mitigation-05：攻击者构造 curl PATCH 绕过前端 → DRF serializer 链路拦截
"""
