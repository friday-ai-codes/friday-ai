"""Phase Wave（ Task 1）：AIPlanGenerationNode 测试 mock 迁移。
迁移前（Phase / Wave 之前）：通过 ``unittest.mock.patch`` 拦截老 SDK 子工厂 runner 整个替掉；
迁移后：统一走 conftest `fake_chat_model_factory` 注入
...
# ============================================================================
# tool_calls 回放测试（Plan §3 新增 / 继承受益验证）
# ============================================================================
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_plan_generation_tool_loop_with_fake_model(
 ...
 """ +：AIPlanGenerationNode 多轮工具调用回放（verify_plan / send_plan_card）。
 回放序列：
 ...
 assert result.output["plan"] == VALID_TECHNICAL_PLAN
# ============================================================================
# sub_step 三态 emit 序列守护（Plan §4 / ROADMAP ）
# ============================================================================
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_plan_generation_sub_step_emit_order(
 ...
 """ / ROADMAP：sub_step analyze / generate_plan / review 三态 emit 序列守护。
 断言：
