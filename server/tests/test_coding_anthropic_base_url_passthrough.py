...
覆盖：
- ROADMAP Phase（passthrough 成功路由）
- CONTEXT 纠偏命名（env_FRIDAY_TASK_CLAUDE_API_KEY / env_FRIDAY_TASK_CLAUDE_BASE_URL）
- CONTEXT （URL scheme 白名单 + host 非空）
- CONTEXT （空 base_url 不注入 metadata 键）
- Threat model T-（api_key 不明文入日志；依赖 redact_credentials processor 自动脱敏）
- Threat model T-（metadata 键名错误导致 Runner 静默丢弃）
- Threat model T-（非法 scheme 注入容器）
- Threat model T-（ProviderMissingError 时节点错误可观测）
"""
...
# =========================================================================
# Task 1: _validate_anthropic_base_url parametrize（ / T- 缓解）
# =========================================================================
@pytest.mark.parametrize(
 ...
 # 空输入： 行为 — 返回空字符串（调用方不注入 metadata 键）
 ("", "", False),
 # 非法 scheme：T- 缓解
...
def test_validate_anthropic_base_url(
 ...
 """ 锁定：scheme 白名单 + 非空 + trim 三项最小校验；T- 缓解。"""
