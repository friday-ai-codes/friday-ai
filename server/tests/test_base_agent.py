"""Phase Wave（ Task 3）：AIAgentBaseNode 单元测试。
覆盖范围（Plan Task 3 <behavior> 11 场景）：
...
async def test_execute_tools_empty_when_hook_returns_none(
 ...
 （非 _tool_registry 全量）。覆盖 （CONTEXT）→ 分歧 B 锁定。
 """
 mock_aresolve_ok(source="system", provider_type="anthropic")
 fake_chat_model_factory(responses=["done"])
...
async def test_execute_logs_custom_api_flag(
 ...
 """Test 11 / Pitfall #8 / T- 守护：use_custom_api=True 场景 structlog
 写 agent_node_start event 必须含 custom_api=True 字段，且不得泄漏
 api_key / api_base_url 原值。
 说明：项目 structlog JSON → stdout（developer-notes.md 规定），不经 stdlib logging
 ...
 assert "sk-test-placeholder" not in combined, (
 "api_key 明文不得出现在 structlog 输出（T-）"
 )
