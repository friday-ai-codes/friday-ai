"""Phase Plan：Provider 凭证 16 用例权限矩阵集成测试（ 硬性契约）。
参考 CONTEXT / + Plan 实际交付的 ProviderCredentialPermission + get_queryset +
...
- project_b_admin（ProjectRole.ADMIN）：project_b 读写；跨项目 retrieve/PATCH 404
 关键断言：跨项目 retrieve/PATCH/DELETE → **404**（非 403），防"凭证存在"事实泄漏
...
def test_provider_credential_permission_matrix(
 ...
 与 / 契约一致；失败时输出 user + method + URL + body 调试信息。
 """
