"""Phase Wave（ Task 2）：节点级 provider_credential_id 四层优先级 +
cross_project scope 拒绝集成测试（ / + 安全硬化）。
覆盖范围（Plan Task 2 <behavior> 5 场景）：
...
- Test 4 test_custom_api_bypasses_provider_credential_id
 ...
 未被调用）。分歧 A 锁定。
- Test 5 test_cross_project_credential_rejected（ 核心守护）
 ...
 T- disposition=mitigate 真正落地（不推给 Phase/230）。
Fixture 策略（ / conftest 复用）：
- `fake_chat_model_factory` / `make_minimal_context` 引用 conftest，不重复定义。
- `cred_layers` fixture 预置 3 层凭证 + 2 Project，支持所有测试场景。
 修复：Project 用 `feishu_project_key` 字段（实证存在）。
 修复：用 `encrypt_value(json.dumps({"api_key": ...}))` 生成合法 Fernet
...
def cred_layers(db) -> dict[str, Any]:
 ...
 # 修复：encrypt_value 生成合法 Fernet 密文，不用占位字符串
...
async def test_project_level_overrides_system(
 ...
 实际 schema 未落地限制（Phase/229 字段预留）。
 """
 fake = fake_chat_model_factory(responses=["done"])
 captured = _capture_resolved(monkeypatch, fake)
 project_a = cred_layers["project_a"]
...
# ============================================================================
# Test 5： 核心守护 —— 跨 project scope 拒绝
# ============================================================================
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_cross_project_credential_rejected(
 ...
 """Test 5： 核心守护 —— project_b 节点指定 project_a 的
