...
- derive_feishu_tenant 纯函数派生（example.feishu.cn → guanghe；非飞书域 / 无意义首段 → ""）。
...
PRD_URL = f"https://example.feishu.cn/docx/{DOC_TOKEN}"
...
def test_derive_feishu_tenant_from_doc_url() -> None:
    ...
    assert derive_feishu_tenant("https://example.feishu.cn/docx/abc") == "guanghe"
...
async def test_tech_plan_records_tech_doc_facet() -> None:
    ...
    await DocumentService().upsert_from_feishu(
        ...
        content="技术方案正文",
        canonical_url="https://example.feishu.cn/docx/TechDocToken99",
    )
