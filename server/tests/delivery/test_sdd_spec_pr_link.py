...
- SDD 仓回填：(artifact_version_id, repository_id) 命中 SddSpec → append {pr_url,
...
- 非 SDD no-op：(artifact_version_id, repository_id) 无 SddSpec → 直接返回（无写入、无异常）。
...
async def _make_artifact_version() -> ArtifactVersion:
    ...
    return await ArtifactVersion.objects.acreate(artifact=plan, version_no=1, content={}, content_hash="h")
...
async def _make_spec(
    *, status: str = SddSpecStatus.APPROVED, repo=None, pv=None
    ...
    pv = pv or await _make_artifact_version()
    return await SddSpec.objects.acreate(repository=repo, artifact_version=pv, status=status)
...
async def test_link_appends_pr_and_marks_implemented_when_approved() -> None:
    ...
    pv = await _make_artifact_version()
...
async def test_link_idempotent_same_pr_url() -> None:
    ...
    await service.link_implementation_pr(
        artifact_version_id=str(pv.id), repository_id=str(repo.id), pr_url=url
    )
...
async def test_link_distinct_pr_urls_append_both() -> None:
    ...
    await service.link_implementation_pr(
        artifact_version_id=str(pv.id), repository_id=str(repo.id),
