...
- 首次 create_draft → 经 DocumentService.create_internal_spec 落 Document(sdd_spec) +
  建 SddSpec(status=draft, change_kind) 连 document/work_item/plan_version_id/repository。
- 重产同 (plan_version_id, repository)：返回既有 SddSpec，不新增 SddSpec 行，且不新增
...
async def _make_plan_version_id() -> str:
    """建一个 canonical TechnicalPlan + v1，返回 PlanVersion.id（标量）。"""
    plan = await TechnicalPlan.objects.acreate(origin=TechnicalPlanOrigin.CHAT)
    from delivery.models import PlanVersion

    pv = await PlanVersion.objects.acreate(plan=plan, version=1, content={}, content_hash="h")
    return str(pv.id)
...
async def test_first_create_draft_lands_document_and_sdd_spec() -> None:
    ...
    pv_id = await _make_plan_version_id()
...
async def test_recreate_same_key_is_idempotent_no_new_rows() -> None:
    """重产同 (plan_version_id, repository)：返回既有 SddSpec，不新增 SddSpec/Document/Version。"""
    repo = await _make_repo()
    pv_id = await _make_plan_version_id()
    service = SddSpecService()

    spec1 = await service.create_draft(
        plan_version_id=pv_id, repository=repo, work_item=None, content="正文 v1"
    )
    spec2 = await service.create_draft(
        plan_version_id=pv_id, repository=repo, work_item=None, content="正文 v2 不同"
    )
...
async def test_work_item_none_is_allowed() -> None:
    ...
    spec = await SddSpecService().create_draft(
        plan_version_id=pv_id, repository=repo, work_item=None, content="正文"
    )
