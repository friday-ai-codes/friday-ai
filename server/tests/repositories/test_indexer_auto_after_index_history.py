"""Phase Plan — indexer auto_after_index 路径写 GraphBuildHistory 集成测试。
覆盖 GRAPH-..05 中 ``auto_after_index`` trigger 路径：
...
- **Section B 功能集成断言** —— 在测试模块内定义一个 ``_run_wrapper`` helper，
 ...
 跑 helper 验证 DB 行的字段口径与 不变量（向量轨 INDEXED 不受影响）。
...
- **Section D update_graph_progress smoke**（T-）—— 源码检查至少 1 处
...
def test_callsite_gating_precedes_history_create(method_name: str) -> None:
 ...
 —— 跳过场景不应创建空 history 行（CONTEXT Q4 / 不变量）。
 """
 method = getattr(IndexerService, method_name)
 src = inspect.getsource(method)
...
# 包裹模板，mock _extract_and_write_graph 后验证 DB 字段口径和 不变量。
...
async def test_auto_after_index_creates_completed_history(
 ...
 # 不变量：indexer 主流程不受影响——repo.index_status 保持 INDEXED
...
# ---------------------------------------------------------------------------
# Section D：update_graph_progress helper（T- smoke）
# ---------------------------------------------------------------------------
def test_at_least_one_callsite_invokes_update_graph_progress -> None:
 """T- acceptance：至少 1 处 callsite 在 ``IndexStage.BUILDING_GRAPH``
 之后调 ``update_graph_progress(...)``（验证 Plan stub helper 通路）。
 """
 import services.indexer as indexer_module
 src = inspect.getsource(indexer_module)
 update_calls = [
 ...
 f"update_graph_progress 调用预期 1-2 处（T- CONTEXT 决议），"
 f"实际 {len(update_calls)} 处：\n" + "\n".join(update_calls)
 )
 # 既有 BUILDING_GRAPH 调用保留（Phase 才彻底删）
...
async def test_update_graph_progress_called_during_auto_after_index(
 ...
 + ``total > 0``（Plan stub helper 通路 smoke）。
 走单个 IndexerService 入口方法的 BUILDING_GRAPH 段，断言 update_graph_progress
 被触发——本测试覆盖 T- 切换点（callsite #1 run_full_index 或
...
async def test_failure_error_message_truncated_to_1000_chars(
 ...
 """``error_message`` 严格 ≤ 1000 字符（与 Plan service 同口径）。"""
 settings.ENABLE_CODEGRAPH = True
 indexer = IndexerService(str(repo.id))
 huge = "X" * 5000
 with patch.object(
