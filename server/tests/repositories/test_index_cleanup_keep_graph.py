"""Phase Plan：cleanup_index keep_graph 双路径单测。
覆盖（GRAPH-..03）：
...
3. ``test_report_field_position``：``CleanupReport`` 末位字段必须为
 ...
 调用方字段顺序漂移，per CONTEXT / Plan 关键约束）。
...
5. ``test_vector_artifacts_order``：mock ``_delete_count`` + ``_delete_chunk_registries_raw``
 ...
 （Phase 不变量：ChunkEdge 必须先于 ChunkRegistry）。
6. ``test_index_delete_view_keep_graph_query`` (T- 端到端)：``DELETE
...
async def test_vector_artifacts_order -> None:
 """断言 _cleanup_vector_artifacts 内部子步骤调用顺序（Phase 不变量）。"""
