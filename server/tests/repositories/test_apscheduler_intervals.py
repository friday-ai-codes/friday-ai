...
不需要启动 APScheduler，只检查命令代码中的触发器参数（ 修复后改用 settings 常量）。
"""
import pathlib
def test_poll_interval_is_2_hours -> None:
 """静态 AST 解析确认 poll_repository_updates 使用 SYNC_INTERVAL_SECONDS，不依赖运行时调度器。"""
 src = pathlib.Path("agents/management/commands/runapscheduler.py").read_text
 assert "settings.SYNC_INTERVAL_SECONDS" in src, (
 "runapscheduler.py 必须使用 IntervalTrigger(seconds=settings.SYNC_INTERVAL_SECONDS)"
 )
