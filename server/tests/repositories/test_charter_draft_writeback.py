...
def _draft(domain: str = "功能/流程优化", rule: str = "不承接课程权益鉴权") -> dict:
    return {
...
async def test_updates_ai_draft_in_place_without_bumping_version() -> None:
    ...
    assert {item["domain"] for item in charter.owned_domains} == {"功能/流程优化", "题库能力"}
...
async def test_human_confirmed_charter_only_receives_draft_content() -> None:
    ...
    assert [item["domain"] for item in fresh.draft_content["owned_domains"]] == ["功能/流程优化"]
...
async def test_human_confirmed_draft_content_merges_by_key() -> None:
    ...
    assert sorted(domains) == ["功能/流程优化", "题库能力"], "同 domain 重复提交不得堆积"
