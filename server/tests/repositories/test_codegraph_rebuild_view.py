"""Phase Plan / GRAPH- + GRAPH-：codegraph REST 三件套端到端测试。
覆盖的端点：
...
- ROADMAP 反向独立锁：``graph_build_status=RUNNING`` 不阻塞 ``POST /index/``
...
def test_index_creation_not_blocked_by_running_graph(
 ...
 """ROADMAP Phase 反向语义验证：向量轨锁与图谱轨锁独立。
 ...
 assert response.status_code != 409, (
 f"图谱锁误参与了向量轨判定（ 违例）：status={response.status_code}, "
