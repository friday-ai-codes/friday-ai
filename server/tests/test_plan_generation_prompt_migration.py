"""Phase + Phase Wave（ Task 2）：plan_generation prompt 迁移。
Phase 原 6 测试验证 Prompt Center render_prompt 三态行为 + `.replace` 字节级等价，
...
捕获 ``SystemMessage.content`` 做字节级 sha256 断言。
关键守护（ 强化）：
...
class TestPlanGenerationMigration:
 ...
 assert result == expected
 # ================================================================
 # Phase Wave（ Task 2）：`_CapturingFake` + execute
 # 链路字节级 hash 守护（ / / Pattern 8）
 # ================================================================
 @pytest.mark.asyncio
 ...
 """Phase：``_CapturingFake`` 捕获 ``SystemMessage.content`` 做字节级 sha256 断言。
 强化：
 ...
 **kwargs: Any,
 ...
 # 3. 固定 execution_id / node_id 使 session_id 稳定（ 强化）
 ...
 f" 守护：禁止更新 expected hash；若语义确变，修 Prompt 路径"
