...
class TestBuildSystemPrompt:
 ...
 prompt = _build_system_prompt("MyProject", "proj-1", role="developer")
 assert "代码" in prompt
 assert "技术" in prompt
 assert "架构" in prompt
 def test_pm_role_contains_management_keywords(self):
 """pm 角色包含管理相关关键词。"""
 prompt = _build_system_prompt("MyProject", "proj-1", role="pm")
 assert "进度" in prompt
 assert "风险" in prompt
 assert "优先级" in prompt
 def test_designer_role_contains_ux_keywords(self):
 """designer 角色包含交互设计相关关键词。"""
 prompt = _build_system_prompt("MyProject", "proj-1", role="designer")
 assert "交互" in prompt
 assert "视觉" in prompt
 assert "用户" in prompt
 def test_qa_role_contains_testing_keywords(self):
 """qa 角色包含测试相关关键词。"""
 prompt = _build_system_prompt("MyProject", "proj-1", role="qa")
 assert "测试" in prompt
 assert "边界" in prompt
 assert "缺陷" in prompt
 def test_general_role_is_balanced(self):
 """general 角色平衡各维度。"""
 prompt = _build_system_prompt("MyProject", "proj-1", role="general")
 assert "全能" in prompt
 assert "灵活" in prompt
 def test_default_role_is_developer(self):
 """默认角色为 developer。"""
 prompt_default = _build_system_prompt("MyProject", "proj-1")
 prompt_dev = _build_system_prompt("MyProject", "proj-1", role="developer")
 assert prompt_default == prompt_dev
 def test_invalid_role_falls_back_to_general(self):
 """无效角色回退到 general。"""
 prompt_invalid = _build_system_prompt("MyProject", "proj-1", role="unknown_role")
 prompt_general = _build_system_prompt("MyProject", "proj-1", role="general")
 ...
 """不同角色的 prompt 内容不同。"""
 prompts = {
 role: _build_system_prompt("P", "proj-1", role=role) for role in ROLE_PROMPTS
 }
 # 两两不同
