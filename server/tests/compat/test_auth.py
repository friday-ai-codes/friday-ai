...
async def test_optional_bearer_allowany() -> None:
    """OPENAI_COMPAT_API_KEYS 为空 → AllowAny，无 Authorization header 也能访问（work item）。"""
    with override_settings(OPENAI_COMPAT_API_KEYS=[]):
        client = AsyncClient()
...
async def test_optional_bearer_deny() -> None:
    """OPENAI_COMPAT_API_KEYS=["secret"] 时，无 Authorization header → 403（work item）。"""
    with override_settings(OPENAI_COMPAT_API_KEYS=["secret"]):
        client = AsyncClient()
        response = await client.get("/v1/models/")
...
async def test_optional_bearer_allow() -> None:
    """OPENAI_COMPAT_API_KEYS=["secret"]，Authorization: Bearer secret → 200（work item）。
