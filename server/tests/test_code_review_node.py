...
合并后 checkpoint 新增 3 项契约：
- per-MR Runner session_id 独立性（contract）
- resolved 循环外一次解析（contract）
- sub_step 三子步骤 emit 序列（contract / success criterion）

mock 迁移（contract）：
- 删除老 Runner mock.patch 调用点（见 checkpoint Summary）
- 替换为 conftest ``fake_chat_model_factory`` seam + ``mock_aresolve_ok`` +
  ``make_minimal_context`` 4 公共 fixture（task 落地；work item 弥补）
"""
...
class TestAICodeReviewNode:
    """AICodeReviewNode 独立执行行为测试（contract 迁移后）。"""
...
# ---------------------------------------------------------------------------
# work item 合并后新增：per-MR 独立性 / resolved 复用 / sub_step 三态序列
# ---------------------------------------------------------------------------
...
async def test_code_review_per_mr_independent_session_id(
    ...
    """contract：3 MR 场景 -> 3 个 Runner 实例，session_id 各不同且均以 ``review-`` 开头。"""
...
async def test_code_review_resolved_reused_across_mrs(
    ...
    """contract：resolved 在循环外一次解析 —— _resolve_api_key_and_model 调用 1 次 for 3 MR。"""
    ...
    assert resolve_call_count == 1, (
        f"_resolve_api_key_and_model 应循环外一次解析，实际 {resolve_call_count} 次（contract 违反）"
    )
...
async def test_code_review_sub_step_emit_order(
    ...
    """contract / success criterion：fetch_diff / ai_review / send_notification 三子步骤按序 emit。
