"""Phase Plan 单元测试：agents.llm_factory.build_chat_model。
覆盖 Requirements：
- 单一工厂入口 + 5 Provider 前缀映射
- thinking 分派（仅 Anthropic）
- reasoning 分派（OpenAI o 系列 + Gemini 2.5，capabilities + 正则双检测）
- max_output_tokens 校验
- timeout 透传
- api_key SecretStr 脱敏（威胁 T-/02）
...
def _make_resolved(
 ...
 """构造 ResolvedProviderConfig，简化测试模板（Phase dataclass 定义）。"""
 default_base_urls = {
...
def test_openai_responses_vs_openai_chat_distinction -> None:
 """ /：use_responses_api=True 区分 Responses vs Chat。"""
...
def test_output_version_v1_locked -> None:
 """：显式传 output_version='v1'（不依赖 default，抗 partner 升级静默改 default）。"""
...
def test_base_url_passthrough -> None:
 """：resolved.base_url 非空时透传到底层 kwargs。"""
...
def test_organization_id_passthrough -> None:
 """：resolved.extra['organization_id'] → kwargs['organization']。"""
 resolved = _make_resolved(
...
def test_thinking_requires_both_capability_and_tokens -> None:
 """：supports_thinking=True 但 max_thinking_tokens=None 时不注入（严格两条件同真）。"""
 resolved = _make_resolved(ProviderType.ANTHROPIC)
 caps = _make_caps(supports_thinking=True)
 model = build_chat_model(
 resolved,
...
def test_no_api_key_leak_in_exception -> None:
 """ + T-：max_output_tokens 超限异常消息不含 api_key 明文。"""
 plaintext_key = "sk-test-placeholder"
 resolved = _make_resolved(ProviderType.ANTHROPIC, api_key=plaintext_key)
 caps = _make_caps(max_output_tokens=8192)
 with pytest.raises(ValueError) as exc_info:
