"""Phase Wave：LangChainAgentRunner AgentSession + ToolCallLog 持久化守护测试。
 / 审计结论：Phase 交付的 langchain_runner.py 未实现持久化；
 补齐后本测试文件守护回归。
测试覆盖（Plan Task 2 Behaviors 1-6）：
...
Fixture 策略（Q6 决策弥补 ）：
本文件内 `_patch_model` + `_make_fake` 为本文件私有 helper；~08 所有新测试将
统一引用 抽到 conftest.py 的 `fake_chat_model_factory` / `mock_aresolve_ok`
...
User = get_user_model
# ============================================================================
# Helpers（本文件私有；Q6/ conftest 抽取见 Task 3）
# ============================================================================
def _patch_model(monkeypatch: pytest.MonkeyPatch, fake: BaseChatModel) -> None:
 """注入 FakeChatModel 到 runner._build_model 调用点（与 Phase core 测试一致）。"""
 monkeypatch.setattr(
 ...
 *,
 ...
 """最小 LangChainRunnerConfig（agent_session 可 None 测试 静默路径）。"""
...
async def test_add_usage_called_after_turn_no_tools(
 ...
 """ Behavior 1：单 turn 无工具，usage 正确写入 AgentSession.metadata。
...
async def test_multiple_turns_accumulate_usage(
 ...
 """ Behavior 1（累加）：两 turn usage={input:50, output:30} 各一次 → 累计 100/60。
...
async def test_tool_call_log_created_per_tool_use(
 ...
 """ Behavior 2：工具 echo turn 1 触发 + turn 2 结束 → ToolCallLog 1 条。
...
async def test_tool_call_log_iteration_increments(
 ...
 """ Behavior 2（iteration 递增）：两工具调用 → iteration 1, 2。
...
# ============================================================================
# Test 5：agent_session=None 静默跳过
# ============================================================================
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_agent_session_none_silent_skip(
 ...
 """ Behavior 3：agent_session=None 时 _persist_usage / _log_tool_call 静默跳过。
...
# ============================================================================
# Test 6：DB 异常走 logger.exception 兜底（T- Tampering mitigation）
# ============================================================================
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_db_exception_does_not_crash_runner(
 ...
 因此用 capsys 而非 caplog 捕获日志（developer-notes.md 规定）。
 """
