"""Phase Plan Task 1: _parse_summary_json 和脱敏测试。
...
class TestSanitizeSummary:
 ...
 脱敏在容器侧完成（Plan）。此测试确认 passthrough 行为。
 """
 from subagent.api.callbacks import _parse_summary_json
 ...
 def test_GITHUB_TOKEN_PLACEHOLDER(self) -> None:
 """JSON 中含 ghp_ key 的字段正常解析（脱敏责任在容器侧）。"""
 from subagent.api.callbacks import _parse_summary_json
 import json
 data = {"overview": "Token: GITHUB_TOKEN_PLACEHOLDER"}
 raw = json.dumps(data)
 result = _parse_summary_json(raw)
 parsed = json.loads(result)
 assert parsed["overview"] == "Token: GITHUB_TOKEN_PLACEHOLDER"
