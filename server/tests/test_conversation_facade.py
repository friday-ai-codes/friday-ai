...
- 通过 graph.astream 驱动会话
- 从 graph state 读取结果并调用 finalize_conversation 落库
- 不直接创建 SDKAgentRunner
- 事件顺序由 graph StreamWriter → Queue → yield 保持一致
"""
...
class TestConversationFacade:
 ...
 """,: graph 事件通过 Queue 桥接到 facade yield。"""
