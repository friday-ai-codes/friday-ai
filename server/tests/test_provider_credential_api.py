"""contract / contract / contract: CRUD + toggle + refresh-models 行为测试 + W4 脱敏链路断言。

覆盖契约：
- contract：Pydantic dispatch 校验 + Read Serializer 不回显 encrypted_config
- contract：scope=system / scope=project 过滤
- contract：toggle-active 切 is_active + aresolve 下次解析跳过
- contract：refresh-models 写回 available_models + 上游异常脱敏链路
- W4（security mitigation-05）：mock 上游异常含 `sk-ant-realkey...` 时，HTTP body + DB 字段
...
# ======================================================================
# contract：Create Serializer Pydantic dispatch 校验
# ======================================================================
...
def test_create_credential_pydantic_dispatch_valid_anthropic(
    ...
    resp = client.post(
        ...
        data={
            ...
            "config": {
                "api_key": "sk-ant-validkey1234567890abcdef",
    ...
    assert b"sk-ant-validkey1234567890abcdef" not in resp.content, (
    ...
    assert config.get("api_key") == "sk-ant-validkey1234567890abcdef", (
...
def test_retrieve_system_credential_non_superuser_hides_secret_plaintext(
    ...
    assert b"sk-ant-sys-default-testing" not in resp.content, (
...
# ======================================================================
# contract：toggle-active + aresolve_or_error 跳过验证
# ======================================================================
...
def test_toggle_active_skips_in_aresolve(
    ...
    # aresolve_or_error（contract Result 模式）应返回 ProviderMissingError；
...
# ======================================================================
# contract：refresh-models 写回 available_models（成功路径）
# ======================================================================
...
# ======================================================================
# W4 修复：refresh-models 上游异常脱敏链路（security mitigation-05 核心）
# ======================================================================
...
def test_refresh_models_upstream_error_redacted(
    ...
    """W4 + security mitigation-05：上游错误经 redact_secrets_in_text 脱敏后入库 + 不泄漏到响应。
    ...
    client = _client_for(system_admin_user)
    leaking_key = "sk-ant-realkey1234567890abcdef"
    upstream_exc_msg = (
...
# ======================================================================
# list/retrieve scope 过滤（contract）
# ======================================================================
...
# ======================================================================
# UAT 第 3 项 hotfix follow-up（follow-up）：scope=any 联合查询
# ======================================================================
...
def test_list_scope_filter_any(
    ...
    UAT 第 3 项 hotfix follow-up（follow-up）：chat 路径
