...
class TestManualTriggerFullFlow(TransactionTestCase):
 """ Test 1: 手动触发索引 → 全量索引 → Qdrant 写入 → 搜索返回结果。"""
 def setUp(self) -> None:
...
class TestWebhookTriggerFlow(TransactionTestCase):
 """ Test 2: Webhook 触发 → trigger_auto_index → IndexHistory 状态跟踪。"""
 def setUp(self) -> None:
 self.repo = Repository.objects.create(
...
class TestIncrementalIndexFlow(TransactionTestCase):
 """ Test 3: 增量索引验证——仅变更文件被重新索引，FileIndex 正确更新。"""
 def setUp(self) -> None:
 self.repo = Repository.objects.create(
 name="incr-test-repo",
...
class TestIndexStatusProgress(TransactionTestCase):
 """ Test 4: IndexStatusView 返回 overall_progress 且值在 0-100 之间。"""
 def setUp(self) -> None:
 self.repo = Repository.objects.create(
