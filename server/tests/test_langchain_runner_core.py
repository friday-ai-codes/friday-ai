"""Phase Plan Task 2：LangChainAgentRunner 核心 ReAct + interrupt + tool flow 测试。
覆盖 Requirement 映射（project-docs/phases/work-item/work-item.md）：
...
Plan 会抽离到 tests/helpers/fake_chat_model.py；本 plan 先 inline 最小化 FakeChatModel。
...
# ============================================================================
# FakeChatModel（inline）——本 plan 本 task 临时实现，Plan 抽离
# ============================================================================
class _InlineFakeChatModel(BaseChatModel):
 ...
 参考：RESEARCH Pattern 3 （Plan 会迁入 tests/helpers/fake_chat_model.py）。
 """
...
def test_runner_interface(runner_config: LangChainRunnerConfig) -> None:
 ...
 # Wave stub 空 dict；Wave Plan auto_trim 分支会写入 {trimmed_count, budget, ...}
...
async def test_gemini_missing_tool_id_filled_with_uuid(
 ...
 """ /：tool_call id 缺失时 runner 补 uuid.uuid4.hex[:12]（12 hex）。"""
 from langchain_core.tools import tool
 @tool
...
async def test_tool_exception_yields_failed_result(
 ...
 """：工具抛异常 → TOOL_USE_RESULT(success=False) + ToolMessage(status='error') + 下一轮继续。"""
 from langchain_core.tools import tool
 @tool
...
async def test_interrupt_cancels_stream(
 ...
 """ / Pitfall F：interrupt 触发 CancelledError → result.status='interrupted'。"""
...
async def test_text_delta_fields_complete(
 ...
 """ 预证：TEXT_DELTA.data 至少含 {text, model, session_id} 三字段。"""
...
async def test_thinking_event_emitted_for_reasoning_block(
 ...
 """：content_blocks 含 {type:'reasoning', reasoning:...} → THINKING event。"""
 fake = _InlineFakeChatModel.make(
 responses=[""],
...
async def test_message_complete_contains_usage(
 ...
 """：MESSAGE_COMPLETE.data.usage 至少含 input + output 两字段映射。"""
 fake = _InlineFakeChatModel.make(
 responses=["ok"],
 usage_metadata={
 "input_tokens": 100,
...
async def test_error_event_on_llm_exception(
 ...
 """：LLM astream 抛异常 → yield ERROR event + result.status='error'（不重试）。"""
...
async def test_normalize_prompt_accepts_string_and_list(
 ...
 """：prompt 为 str 或 list[BaseMessage] 都能正常运行。"""
 fake = _InlineFakeChatModel.make(responses=["ok"])
 _patch_model(monkeypatch, fake)
 # str 输入
...
async def test_tool_use_result_fields_on_success(
 ...
 """：TOOL_USE_RESULT(success=True) 含 {tool_call_id, tool_name, result, model, session_id}。"""
 from langchain_core.tools import tool
 @tool
