...
- 凭证解析通过 ``ProviderConfigService.aresolve_or_error``（ Result 模式）
...
class TestShouldGenerateTitle:
 ...
 assert await should_generate_title(str(conversation.id)) is False
# ============================================================================
# generate_title 测试（ 迁移后：FakeChatModel + build_chat_model seam）
# ============================================================================
@pytest.mark.django_db(transaction=True)
class TestGenerateTitle:
 ...
 """：成功生成标题并更新数据库（FakeChatModel seam）。"""
 ...
 """：凭证解析返回 ProviderMissingError 时返回 None。"""
 from chat.title_service import generate_title
