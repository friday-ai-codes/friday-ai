...
class TestWorkflowExecutionTenantMigration:
    ...
    def test_execution_has_project_field(self, execution):
        ...
        assert hasattr(execution, "project")
        assert execution.project is not None
    ...
    def test_execution_project_not_null(self, workflow, user):
        ...
        assert exec_instance.project_id is not None

    @pytest.mark.django_db
    def test_backfill_correctness(self, workflow, user):
        """验证 execution.project == execution.workflow.project。"""
        exec_instance = WorkflowExecution.objects.create(
            workflow=workflow,
            project=workflow.project,
            trigger_type="manual",
        ...
        exec_instance.refresh_from_db()
        assert exec_instance.project_id == exec_instance.workflow.project_id
    ...
    def test_execution_cascade_delete(self, workflow, project, user):
        """删除 Project 后关联的 WorkflowExecution 也被删除。"""
        exec_instance = WorkflowExecution.objects.create(
            workflow=workflow,
            project=project,
            trigger_type="manual",
