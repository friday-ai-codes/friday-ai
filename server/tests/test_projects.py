...
class TestProjectDetail:
 ...
 """测试获取单个项目。"""
 response = authenticated_admin_client.get(urls.project_detail(project.id))
 assert response.status_code == status.HTTP_200_OK
 ...
 """测试获取不存在的项目。"""
 response = authenticated_admin_client.get(urls.project_detail(uuid.uuid4))
 ...
 """测试删除项目。"""
 response = authenticated_admin_client.delete(urls.project_detail(project.id))
