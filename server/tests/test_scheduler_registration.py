...
def test_disabled_no_start -> None:
 """ 测试环境默认 FF_ENABLE_SCHEDULER=False（conftest autouse 守护）。
 对应 T- Availability mitigation：
...
def test_cleanup_orchestration_checkpoints_job_defined -> None:
 """ runapscheduler 模块定义了 cleanup_orchestration_checkpoints_job 函数。"""
...
def test_cleanup_orchestration_checkpoints_job_calls_command(
 ...
 """ job wrapper 通过 django call_command 调用 cleanup_orchestration_checkpoints。"""
 from agents.management.commands import runapscheduler
...
def test_add_job_registration_block_present -> None:
 """ runapscheduler.py 源码含 cleanup_orchestration_checkpoints 的 add_job 注册块。
