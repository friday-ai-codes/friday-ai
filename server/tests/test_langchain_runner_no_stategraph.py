"""静态断言：langchain_runner.py 不得嵌入 LangGraph 子图执行器（work item）。

依据：
- REQUIREMENTS work item "不嵌入 LangGraph 子图执行器（保持朴素 ReAct loop）"
...
若未来需要子图嵌入，必须另起设计评审并重新评估风险缓解。
任何违反 work item 的 commit 会立即 CI fail。
"""
...
def test_no_langgraph_imports() -> None:
    """langchain_runner.py 不得 import 任何 langgraph.* 符号（work item 禁线）。"""
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            ...
            assert mod not in FORBIDDEN_IMPORT_MODULES, (
                f"langchain_runner.py 禁止 from {mod} import（work item + Pitfall 9）"
            )
            # 兜底：也禁止 langgraph.* 全前缀（例如将来 langgraph.new_subpkg）
            assert not mod.startswith("langgraph."), (
                f"langchain_runner.py 禁止 from {mod} import（work item 禁线）"
            )
        elif isinstance(node, ast.Import):
            for alias in node.names:
                assert not alias.name.startswith("langgraph."), (
                    f"langchain_runner.py 禁止 import {alias.name}（work item 禁线）"
                )
                assert alias.name != "langgraph", (
                    "langchain_runner.py 禁止 import langgraph（work item 禁线）"
                )
...
def test_no_forbidden_symbols_in_source() -> None:
    ...
    for sym in FORBIDDEN_SYMBOLS:
        assert sym not in src, (
            f"langchain_runner.py 不得引用 {sym}（work item 禁止嵌 LangGraph 子图执行器）"
        )
