"""Phase + Phase Wave（ Task 3）：variable_extractor 迁移测试。
Phase 原 4 测试验证 Prompt Center render_prompt 三态行为 + `.format` 字节级
等价，**不触及** LLM 调用（迁移前老 httpx 直调 / SDK 客户端均无 mock，
node.execute 链路也不跑）。
Phase Wave补强：新增 `_CapturingFake` seam 经
...
- 禁止条款：本 plan 不得引入硬编码 hash 更不得后续升级中更新已有 hash（ 后
...
# ============================================================================
# Phase Wave（ Task 3）：_CapturingFake seam 字节级守护
# ============================================================================
...
class TestVariableExtractorPromptByteEqual:
 """Phase Wave 补强：execute 路径 HumanMessage.content 字节级等价。
 ...
 """ 强化：fallback 路径下 execute 捕获的 HumanMessage.content
 ...
 # === 强化：动态计算 expected prompt（非硬编码 hash） ===
 ...
 f" 守护：禁止更新 hash；若语义确变，修 render_prompt "
