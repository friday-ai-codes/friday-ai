"""Phase / Pitfall #10 弥补：AIPromptNode 字节级 hash 守护。
作为第 5 个 `_prompt_migration.py` 测试文件完成 ROADMAP "4+1 字节级 hash
100% 通过"的最后一项覆盖（原 Pitfall #10 明确 AIPromptNode 无字节级 hash
覆盖，由本 plan 首次建立基线； 强化：expected hash 本次首建即锁定，
后续 plan 不得改动）。
...
`_prompt_migration.py` 一致；其他 fixture 统一引用 conftest。
"""
...
async def test_rendered_system_and_user_prompt_byte_equal(
 ...
 强化：固定 execution_id + node_id 让 render_template 变量渲染稳定；
 ...
 "Jinja2/render_template 渲染后字节级漂移（ / 强化：hash 首建即锁）"
 )
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
...
async def test_injected_value_not_rerendered_by_langchain(
 ...
 """Test 3 / Pitfall #1 / T- 守护：模板变量值内的 `{{evil}}` 不被 LangChain
 二次 Jinja2 渲染。
