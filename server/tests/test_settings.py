...
class TestSettingsListCreate:
    ...
    def test_list_settings_returns_list(self, authenticated_admin_client, urls):
        """测试列出系统设置返回列表。"""
    ...
    def test_create_setting(self, authenticated_admin_client, urls):
        """测试创建系统设置。"""
        setting_data = {
            "key": "test_setting",
            "value": "test_value",
    ...
    def test_create_duplicate_setting(self, authenticated_admin_client, urls):
        ...
        # 创建第一个
        response1 = authenticated_admin_client.post(urls.settings_list, setting_data, format="json")
        assert response1.status_code == status.HTTP_201_CREATED

        # 尝试创建重复的
        response2 = authenticated_admin_client.post(urls.settings_list, setting_data, format="json")
...
class TestSettingsDetail:
    ...
    def test_get_setting(self, authenticated_admin_client, urls):
        """测试获取单个系统设置。"""
        # 先创建
        setting_data = {
        ...
        create_response = authenticated_admin_client.post(urls.settings_list, setting_data, format="json")
        assert create_response.status_code == status.HTTP_201_CREATED

        # 获取
        response = authenticated_admin_client.get(urls.settings_detail("get_test_setting"))

        assert response.status_code == status.HTTP_200_OK
        assert response.data["key"] == "get_test_setting"
        assert response.data["has_value"] is True

    def test_get_nonexistent_setting(self, authenticated_admin_client, urls):
        """测试获取不存在的系统设置。"""
        response = authenticated_admin_client.get(urls.settings_detail("nonexistent_key"))

        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_update_setting(self, authenticated_admin_client, urls):
        """测试更新系统设置。"""
        # 先创建
        setting_data = {
    ...
    def test_delete_setting(self, authenticated_admin_client, urls):
        """测试删除系统设置。"""
        # 先创建
        setting_data = {
        ...
        # 删除
        response = authenticated_admin_client.delete(urls.settings_detail("delete_test_setting"))

        assert response.status_code == status.HTTP_204_NO_CONTENT

        # 确认已删除
        get_response = authenticated_admin_client.get(urls.settings_detail("delete_test_setting"))
        assert get_response.status_code == status.HTTP_404_NOT_FOUND

    def test_delete_nonexistent_setting(self, authenticated_admin_client, urls):
        """测试删除不存在的系统设置。"""
...
class TestSettingsPlainText:
    ...
    def test_setting_value_visible_in_list(self, authenticated_admin_client, urls):
        ...
        assert setting["value"] == "sk-test-1234567890abcdef"
