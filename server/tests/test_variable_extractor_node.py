...
- Test 3 test_temperature_0_3_bound_to_chat_model
  contract 守护：chat_model.bind(temperature=0.3) 实际捕获；kwargs["temperature"]==0.3。
...
- Test 7 test_httpx_not_called
  success criterion 守护：variable_extractor 不再走 httpx.AsyncClient 直调（monkeypatch
  httpx.AsyncClient 为 forbidden raise）。
- Test 8 test_aget_claude_config_only_for_model_fallback
  contract 守护：aget_claude_config 仅在 config_model="" 时被调用拿 model；api_key /
  base_url 走 aresolve_or_error 产出。

Fixture 策略（contract / Pattern 9 / Q6 决策 / work item）：
- checkpoint-03 已抽取 conftest.py 公共 fixture（fake_chat_model_factory /
  mock_aresolve_ok / mock_aresolve_missing / make_minimal_context），本文件统一引用，
  **禁止**重复定义。checkpoint 静态扫描拦截。
"""
...
async def test_temperature_0_3_bound_to_chat_model(
    ...
    """Test 3：contract 守护 —— chat_model.bind(temperature=0.3) 实际捕获。"""
...
# ============================================================================
# Test 7: httpx 未被调用（success criterion 守护）
# ============================================================================
...
async def test_httpx_not_called(
    ...
    """Test 7：success criterion 守护 —— variable_extractor 不走 httpx.AsyncClient 直调。"""
    ...
    class _ForbiddenClient:
        def __init__(self, *a: Any, **kw: Any) -> None:
            calls.append((a, kw))
            raise AssertionError("不应创建 httpx.AsyncClient（work item 违反）")
    ...
    assert calls == [], "httpx.AsyncClient 被调用（success criterion 违反）"
