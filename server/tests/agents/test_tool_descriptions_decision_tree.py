...
def test_find_related_code_description_has_decision_tree -> None:
 """``find_related_code`` description 含 4 关键决策树锚点（per CONTEXT ）。
 ...
 "排他子句保护 LLM 误用，per CONTEXT "
 )
 assert "Decision tree:" in desc, (
 "missing 'Decision tree:' header —— per CONTEXT "
 )
