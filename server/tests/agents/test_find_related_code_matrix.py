...
函数名带 ``_null_provider`` 后缀（per ROADMAP 字面要求 ``pytest -k
...
async def test_find_related_returns_empty_when_no_edges_for_null_provider -> None:
 ...
 ``services.retrieval.find_related.find_related``——Phase Plan deviation
...
async def test_find_related_with_chunk_edge_data_for_null_provider -> None:
 ...
 边 1:1 对齐；
 - ``reason`` 非空（Phase Plan ``explain_neighbor`` 模板生成）；
 ...
 assert result[0].reason, "reason 必须非空（per Phase Plan explain_neighbor 模板）"
