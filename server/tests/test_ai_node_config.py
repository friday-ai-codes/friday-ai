"""Phase Wave（ Task 3）：AI 节点 config_schema JSON Schema 验证测试。
覆盖范围（Plan Task 3 <action> 6 测试函数）：
...
- test_max_budget_usd_preserved_in_all_three_nodes
 守护：max_budget_usd 字段在 3 节点保留
 Task 1 已把 provider_credential_id 追加到 3 节点 config_schema；
本 Plan Task 3 在 JSON Schema 层面守护字段契约不被后续 plan 误删。
"""
...
# ============================================================================
# Test 6： 守护 —— max_budget_usd 在 3 节点保留
# ============================================================================
def test_max_budget_usd_preserved_in_all_three_nodes -> None:
 """ 守护：max_budget_usd 字段在 3 节点 config_schema 保留不变。
 该字段 Phase 激活（成本追踪 Dashboard），禁止本 phase 删除。
 """
 from workflows.nodes.ai.base_agent import AIAgentBaseNode
 from workflows.nodes.ai.prompt import AIPromptNode
 from workflows.nodes.ai.variable_extractor import AIVariableExtractorNode
 ...
 f"{cls.__name__} 缺少 max_budget_usd 字段（ 违反）"
 )
...
def test_variable_extractor_has_no_max_output_tokens -> None:
 """Anti-pattern H / 守护：AIVariableExtractorNode 不含 max_output_tokens 字段。
 字段矩阵明确：max_output_tokens 在 AIAgentBaseNode / AIPromptNode 加；
