"""Phase Plan：PROVIDER_REGISTRY 5 Provider 契约测试 + Pydantic credential_schema 脱敏测试。
覆盖 Requirement: （5 Provider 元数据形状契约）。
威胁参考: T-（凭证 repr 泄漏）/ T-（ValidationError 回显明文）。
本文件 Phase Plan 完全重写：
- 旧版断言 `len(ProviderType) == 1`（Phase 遗留），在 5 Provider 扩展下自然失效。
...
def test_registry_langchain_prefix_populated -> None:
 """Test 3：5 个 entry 的 langchain_prefix 与 Phase init_chat_model 预期对齐。"""
 from services.provider_config import PROVIDER_REGISTRY, ProviderType
 expected = {
...
def test_credential_schema_secretstr_repr -> None:
 """Test 4 (T-)：SecretStr 字段在 repr 时显示 **********，不泄漏 api_key 明文。"""
 from services.provider_config import AnthropicCredentialSchema
 c = AnthropicCredentialSchema(api_key="sk-test-placeholder") # type: ignore[arg-type]
