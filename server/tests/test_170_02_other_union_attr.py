...
class TestOtherFilesUnionAttrFix:
 ...
 "runners/views.py",
 "--show-error-codes"
 ], capture_output=True, text=True, cwd="/Users/example/Projects/open-source/friday-ai/server")
 # 检查是否有 union-attr 错误
 has_union_attr_errors = "union-attr" in result.stdout or "union-attr" in result.stderr
 # 检查文件中是否还有 type: ignore[union-attr] 注释
 with open("/Users/example/Projects/open-source/friday-ai/server/runners/views.py", "r") as f:
 content = f.read
 ...
 has_union_attr_errors = "union-attr" in result.stdout or "union-attr" in result.stderr
 # 检查文件中是否还有 type: ignore[union-attr] 注释
 with open("/Users/example/Projects/open-source/friday-ai/server/chat/title_service.py", "r") as f:
 content = f.read
 has_union_attr_ignores = "type: ignore[union-attr]" in content
