"""Phase Wave：静态守护测试门禁。
覆盖：
- ** httpx 清零**：`server/workflows/nodes/ai/*.py` 无
 `async with httpx.AsyncClient` / `import httpx` 出现
- ** 禁区冻结**（coding.py / coding_dispatcher.py / sdk/runner.py）：
...
- ** 完成**：workflow 节点测试文件无 `mock.patch("anthropic.AsyncAnthropic")`
- ** 三节点 config_schema 新字段守护**：provider_credential_id 存在
- ** API tier scope 校验存在性守护**：3 节点均含 `cred.scope == "project"`
 相关校验代码
- ** 守护 base_ref 从 VALIDATION.md frontmatter.phase_start_sha
 ...
 **不使用 HEAD~10 硬编码**（HEAD~10 是非确定性 —— 取决于运行时 commit 数）。
 首 task 必须写入 phase_start_sha；本测试读取后做 git diff。
 """
 ...
 f"实际：{sha!r}。 首 task 必须写入；"
 f"禁止 HEAD~10 硬编码 base_ref（ 决策）。"
 )
 return sha
...
def _workflow_node_test_files -> list[pathlib.Path]:
 """列出 workflow 节点测试文件（扫描 老 mock 残留时用）。
 定位规则：
 ...
 return results
# ---------------------------------------------------------------------------
# Test 1 — httpx 清零（ / / Anti-pattern F）
# ---------------------------------------------------------------------------
def test_no_httpx_client_in_ai_nodes -> None:
 """ / Anti-pattern F 守护：AI 节点无 `async with httpx.AsyncClient`。"""
 ...
 f" 违反：AI 节点出现 `async with httpx.AsyncClient`（ 未完成）\n"
...
def test_no_httpx_import_in_prompt_nodes -> None:
 """prompt.py / variable_extractor.py 无 `import httpx`（ 辅助守护）。"""
 ...
 offenders.append((str(path), i))
 assert not offenders, (
 f" 违反：prompt.py / variable_extractor.py 仍 import httpx\n"
 f"offenders: {offenders}"
 )
# ---------------------------------------------------------------------------
...
def test_no_chat_prompt_template_in_ai_nodes -> None:
 """ / Anti-pattern A 守护：AI 节点禁 LangChain ChatPromptTemplate。
 ...
 f"( / 要求纯 SystemMessage+HumanMessage 双包装)\n"
 f"offenders: {offenders}"
 )
# ---------------------------------------------------------------------------
...
def test_no_sdk_runner_import_in_ai_nodes -> None:
 """ 主干守护：base_agent / prompt / variable_extractor / code_review 无
 `SDKAgentRunner` 或 `agents.sdk.runner` 引用。
 **排除 coding.py**（AICodingNode 容器分发路径保留 SDKAgentRunner； 冻结）。
 """
 offenders: list[tuple[str, int, str]] =
 ...
 offenders.append((str(py), i, line.strip))
 assert not offenders, (
 f"Anti-pattern G 违反：AI 节点出现 SDKAgentRunner（ 主干回滚）\n"
 f"offenders: {offenders}"
 )
# ---------------------------------------------------------------------------
# Test 10 — 守护：workflow 节点测试文件无 `anthropic.AsyncAnthropic` mock
# ---------------------------------------------------------------------------
def test_no_anthropic_asyncanthropic_mock_in_workflow_tests -> None:
 """ 守护：workflow 节点测试文件完全迁移到 FakeChatModel。
 **排除 chat 路径测试 / e2e 测试**（chat_runner / sdk_runner / title_service 仍可
 通过 anthropic mock 驱动； 锁定 chat 路径不迁移）。
 """
 ...
 offenders.append((str(py), i, line.strip))
 assert not offenders, (
 f" 违反：workflow 节点测试仍 mock `anthropic.AsyncAnthropic`\n"
...
def test_provider_credential_id_in_all_three_configs -> None:
 ...
 missing.append(name)
 assert not missing, (
 f" 违反：以下节点缺少 `provider_credential_id` 字段："
 f"{missing}（ 方案 A 要求 Task 1 一次性同步 3 节点 schema）"
 )
# ---------------------------------------------------------------------------
# Test 13 — 守护：3 节点均含 API tier scope 校验
# ---------------------------------------------------------------------------
def test_api_tier_scope_check_exists_in_all_three_nodes -> None:
 """ 守护：3 节点均含 API tier scope 校验代码（跨 project 拒绝）。
 匹配模式覆盖三种可能实现：
 ...
 missing.append(name)
 assert not missing, (
 f" 违反：以下节点缺少 API tier scope 校验代码"
 f"（T- disposition=mitigate 未落地）：{missing}；"
 f"应在 Task 1 加 scope 校验或抽 helper"
 )
# ---------------------------------------------------------------------------
# Test 14 — phase_start_sha 合法性守护
# ---------------------------------------------------------------------------
def test_phase_start_sha_is_valid -> None:
 """ 守护：VALIDATION.md frontmatter.phase_start_sha 合法。
 双重校验：
