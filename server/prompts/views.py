...
Phase Task 2 产出。所有 view 基于 `adrf.views.APIView` 手写
...
RBAC 切换。
错误处理：`_handle_prompt_error` 把 Plan 的 exception 体系映射到 HTTP：
...
# ============================================================================
# Exception → HTTP mapping（ 校验结果与 LLM 渲染故障的 HTTP 出口）
# ============================================================================
def _handle_prompt_error(exc: PromptError) -> Response:
 """把 Plan prompt exception 统一映射到 HTTP Response。"""
...
class PromptListView(APIView):
 ...
 "首版 body 通过 `append_version` 服务层写入并自动激活。"
 ),
...
class PromptDetailView(APIView):
 ...
 description=(
 "title / description 原地更新。body 变更走 `append_version`，"
 "字节级相等时幂等跳过。"
 ),
 ...
 summary="删除 Prompt",
 description=(
 "is_builtin=True 且 scope=system 的 Prompt 禁止删除（T- mitigate）；"
...
class PromptRenderPreviewView(APIView):
 ...
 description=(
 "调用 `render_prompt` 服务层，对传入 variables 做 声明校验 + "
 " 三步清洗 + Jinja2 sandbox 渲染。变量缺失返回 422。"
 ),
...
class PromptActivateVersionView(APIView):
 ...
 description=(
 "pointer flip only，不创建新版本。仅写权限用户可调用："
 "系统级 → superuser，项目级 → 项目 ADMIN。T- mitigate：复合查询 "
