...
决策依据（work-item.md）：
- 无缓存：每次调用直连 DB
- StrictUndefined + 声明前置校验
- 变量三步清洗（长度截断 / {{}} 转义 / XML tag 包裹）
- append-only 版本化 + 幂等字节级比对
- declared_variables 运行时 regex 计算
- PROMPT_CENTER_DISABLED_KEYS 环境变量灰度开关
"""
...
def _get_disabled_keys -> set[str]:
 """从 os.environ 读 PROMPT_CENTER_DISABLED_KEYS 灰度开关。
 ...
 *,
 ...
 """变量值三步清洗，仅用于 DB 渲染路径，不用于 fallback。"""
 ...
 *,
 ...
 """渲染提示词的统一入口（Phase 对外 API）。
 流程：
 1. 读 PROMPT_CENTER_DISABLED_KEYS— 命中直接 fallback
 2. 查 active PromptVersion（ 无缓存）
 3. 未命中 → fallback（走 _render_fallback，跳过 XML 清洗）
 4. 命中 → 声明校验→ 变量清洗→ Jinja2 sandbox 渲染
 ...
 if not fallback:
 # 仅当 slug 在 BUILTIN_SLUGS 内却 DB 无记录 → 系统 bug（Phase seed 应补齐）
...
def _append_version_sync(
 ...
 幂等保护：若 new_body == active_version.body 字节级相等，跳过创建。
 """
...
def compute_version_diff(v1: PromptVersion, v2: PromptVersion) -> str:
 ...
 前端 Phase 用 jsdiff 再次 render；后端只返回原始 diff text。
 """
 lines = list(
 unified_diff(
