"""implementation / 229: Provider 凭证相关 URL。

挂载于 friday/urls.py 的 /api/providers/ 前缀下。

路由结构：
- types/                                → ProviderTypesView（implementation schema-driven 数据源）
- credentials/                          → ProviderCredentialViewSet（implementation CRUD）
- credentials/<uuid>/                   → retrieve / update / partial_update / destroy
- credentials/<uuid>/toggle-active/     → @action toggle_active（implementation contract）
- credentials/<uuid>/refresh-models/    → @action refresh_models（implementation contract）
- credentials/<uuid>/test-connection/   → ProviderCredentialTestConnectionView（implementation 就位）
...
urlpatterns = [
    ...
    # implementation contract / contract / contract：ViewSet CRUD + @action toggle-active / refresh-models
    *router.urls,
]
