"""Provider 凭证 DRF 权限类（Phase 层 1）。
 双层防御的 request / object 级权限：
...
类比范本：`server/workflows/api/permissions.py:WorkflowPermission`
差异点：ProviderCredential 非 FK 到 Project（model 刻意用 UUIDField），
