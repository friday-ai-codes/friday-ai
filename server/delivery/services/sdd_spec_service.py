...
幂等（D-49-3，兼顾不留孤儿 Document）：先按 ``(artifact_version_id, repository)`` 短路探测既有
...
async 安全：用 ``artifact_version_id`` 标量 + ``repository`` / ``work_item`` 实例，不裸访问
...
class SddSpecService:
    ...
    async def link_implementation_pr(
        self, *, artifact_version_id: Any, repository_id: Any, pr_url: str
    ) -> None:
        """回填 spec→实现 PR 关联（spec→PR 唯一写入入口，INV-6，D-52-2）。

        按 ``(artifact_version_id, repository_id)`` 命中 SddSpec：追加 PR ref（按 ``pr_url``
        ...
        Args:
            artifact_version_id: 来源 ArtifactVersion.id 标量（幂等键之一，async 安全用标量）。
    ...
    def _link_implementation_pr(
        ...
        with transaction.atomic():
            spec = (
                ...
                .filter(artifact_version_id=artifact_version_id, repository_id=repository_id)
                .first()
            )
            if spec is None:
    ...
    async def create_draft(
        ...
        *,
        ...
        Args:
            artifact_version_id: 来源 ArtifactVersion.id 标量（幂等键之一）。
        ...
        existing = await SddSpec.objects.filter(
            artifact_version_id=artifact_version_id, repository=repository
        ).afirst()
