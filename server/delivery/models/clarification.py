...
编排（任意 ``process_type``）在不清晰时发 ``Clarification`` 挂起等用户，回答后由各
process 自行决定后续推进（不再在模型层耦合任何 process 专属重跑面）。
...
- **process-agnostic**：P3 起移除原 ``affected_partials``（M2M→RepoResearchTask）硬绑定，
  使澄清对任意 ``process_type`` 通用、不再耦合"多仓调研"。澄清回答后的影响面由各 process
  在其自身 stage / 节点逻辑里决定（如 technical_plan 经 merge 回退 / 重研究判定）。
"""
...
class Clarification(models.Model):
    """HITL 澄清问答（§6 字段；process-agnostic 轮次容器）。"""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)

    # 归属一次 ConvergenceSession 编排（任意 process_type）；删 session 级联删其澄清
