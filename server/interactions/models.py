...
覆盖外部 MCP/Skill 入口的全量交互留痕（ 全量事件级入库）：
- ``InteractionRun``：顶层运行实例，对外 trace 锚点。
- ``InteractionEvent``：事件流，8 类 event_type + 父子 trace。
- ``ToolCallRecord``：tool call 明细。
- ``ModelUsageRecord``：model usage 明细。
**append-only 约束**：四表只增不改，重试记录为新事件 / 新记录，
不覆盖历史；模型层不提供更新语义。
**脱敏契约**：``token_fingerprint`` 仅存 hash_token（sha256 hex），
绝不存明文；``raw_request`` / ``payload`` / tool ``input``·``output`` / model prompt
在写库前必须已经过 ``redact_for_ledger``（由 的写入 helper 保证）。
...
class InteractionRun(models.Model):
 ...
 ERROR = "error", "错误"
 # DENIED：token 吊销/过期/无效时记录的拒绝运行（供 ）。
 DENIED = "denied", "已拒绝"
 id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
 # 对外暴露的 trace id，与内部 pk 解耦。
 run_id = models.UUIDField(default=uuid.uuid4, unique=True, db_index=True)
 # = hash_token(token)（sha256 hex，64 字符）；绝不存明文。
 ...
 # 原始请求快照（写入前已 redacted，由 helper 保证）。
