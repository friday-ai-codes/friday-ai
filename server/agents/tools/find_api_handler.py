"""``find_api_handler`` agent tool —— per Phase Plan 。
给定 HTTP URL + method，在指定仓库的 Endpoint 表中查找匹配的后端 handler symbol。
实现要点：
- **URL 归一化**：复用 Phase ``path_normalizer.normalize_url_path``，将查询 URL 与
 Endpoint.url_path 统一归一化为:param 风格再对比。
- **查询策略**：先按 ``http_method + repository_id`` 过滤 Endpoint，再 Python 侧逐条
 normalize(url_path) 对比——单仓 endpoint 数量有限（Phase 实测 285+），Python 侧对比可接受。
 ...
 *,
 ...
 return ToolResult(
 success=False,
 error="repository_id is required (per Phase)",
 )
