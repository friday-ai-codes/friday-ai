...
- **结构化错误**：``repository_id`` 缺失 / Pydantic ``ValidationError`` / 起点
  ...
  runtime（per work-item contract）。
...
"""Tool description 决策树文本（per work-item contract / plan）。
...
"""JSON Schema 镜像 ``FindRelatedCodeInput`` 字段（per task must_have）。

``required: ["repository_id"]`` 与 tool 函数运行时校验对齐：``chunk_id`` 跨 repo
不唯一，必须显式限定单 repo 查询（per work-item contract）。Pydantic schema 仍保持
``repository_id: str | None`` 留给未来 inferred-from-context 扩展，但 LLM 调用方
读 JSON Schema 应一次性看到必填语义（per work-item work item）。
...
async def find_related_code(
    ...
    Args:
        ...
        repository_id: 目标仓库 UUID（必填，per work-item contract）。
...
async def _find_related_code_impl(
    *,
    ...
    if validated.chunk_id is not None:
        ...
        # 不抛 AttributeError（per work-item contract）。
        # work item 防御 Protocol 实现漂移：getattr(..., default) 仅在属性不存在时
        ...
        if not sym_file_path:
            # work item 防御 Protocol 实现漂移：第三方 Provider 若返回 dict 缺
