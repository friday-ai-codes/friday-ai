"""ToolDefinition → LangChain StructuredTool 桥接模块（Phase）。
...
def _schema_type_to_python(prop: dict[str, Any]) -> Any:
 """JSON Schema type → Python 类型（chat_runner.py 一字不改 copy）。
...
def _build_args_schema(tool_def: ToolDefinition, hidden_fields: set[str]) -> type[BaseModel]:
 """ToolDefinition.parameters → pydantic BaseModel（chat_runner.py 一字不改 copy）。
 ...
 **kwargs: Any,
 ...
 # 字段，因此不存在"kwargs 覆盖 injected"的越权路径（T-）。
