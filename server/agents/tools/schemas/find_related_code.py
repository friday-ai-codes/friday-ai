...
``apps.ready()`` 之前可独立 import，避免 app loading 顺序耦合（for schema safety
...
"""``downstream`` = 沿边正向；``upstream`` = 反向；``both`` = 双向合并（per work-item contract）。"""
...
class FindRelatedCodeInput(BaseModel):
    ...
    file_path: str | None = Field(
        ...
        description=(
            ...
            "首 chunk 作为起点（per work-item contract）。与 chunk_id / symbol_name 三选一。"
        ),
    )
    ...
    repository_id: str | None = Field(
        ...
        description=(
            ...
            "tool 函数报错 'repository_id is required'（per work-item contract）。"
        ),
    )
    ...
    direction: Direction = Field(
        ...
        description=(
            ...
            "limit/2 去重合并）。默认 both（per work-item contract）。"
        ),
    )
    limit: int = Field(
        default=20,
        ...
        description=(
            ...
            "（per work-item contract 跨 hops 行为）。"
        ),
    )
...
class FindRelatedCodeOutput(BaseModel):
    ...
    让 Agent 区分"查到了但无邻居"与"工具失败"两种空结果（per work-item contract）。
    """
