"""API 相关 MCP tool 输入/输出契约 —— per Phase Plan。
三个 tool 的 schema 集中维护，防止字段漂移：
- ``FindApiHandlerInput / Output``：给定 URL + method 找后端 handler symbol
- ``FindApiCallersInput / Output``：给定 handler 名找所有前端调用方
- ``ListEndpointsInput / Output``：列仓库所有 API 端点
规范同 Phase find_related_code：
