"""``search_repository_code`` agent tool 输入/输出契约 —— per contract / contract / contract。
...
- ``SearchRepositoryCodeOutput``：``final_context`` / ``tokens`` /
  ...
  ``RagSearchResult`` 字段语义（per contract）。
...
class SearchRepositoryCodeOutput(BaseModel):
    """Agent tool ``search_repository_code`` 输出契约（per contract）。
