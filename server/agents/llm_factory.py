"""Phase 核心工厂入口 —— build_chat_model thin wrapper。
...
api_key 用 pydantic.SecretStr 包装防泄漏（T-/02 缓解）。
职责边界（明确不做的事）：
- 不调 LLM（仅构造对象；实测 init_chat_model 构造本身不触网 —— RESEARCH Pitfall I）
- 不做 tool binding（归 Plan Runner：model.bind_tools(tools)）
- 不计成本（归调用方 Phase AIAgentBaseNode + pricing.calculate_cost_v2）
- 不持有状态（纯函数；每次调用构造新 BaseChatModel 实例）
上游调用方：
- Plan LangChainAgentRunner._build_model
- Plan FakeChatModel monkeypatch 正是替换此函数
- Phase AIAgentBaseNode / AIPromptNode / AIVariableExtractorNode
决策依据：
- CONTEXT ~10（签名 / prefix / SecretStr / thinking / reasoning /
 max_output / timeout / base_url / extra / mypy）
- CONTEXT （output_version='v1' 锁死，抗 partner 静默改 default）
- RESEARCH Pattern 1 （权威模板，live venv 实证）
...
# 正则兜底：OpenAI o 系列（o1/o2/.../o9/o4-mini）+ gpt-5（ 第二重检测）
 ...
 *,
 ...
 """单一工厂入口：capabilities 驱动 kwargs 分派。
 Args:
 resolved: Phase 四层解析后的 Provider 配置（含 api_key 明文 / base_url /
 ...
 prefix = PROVIDER_REGISTRY[resolved.provider_type].langchain_prefix #
 kwargs: dict[str, Any] = {
 "api_key": SecretStr(resolved.api_key), # 防泄漏
 "timeout": timeout_seconds, #
 "streaming": streaming,
 "output_version": "v1", # 锁死 v1 content blocks schema
 }
 if resolved.base_url:
 kwargs["base_url"] = resolved.base_url #
 # thinking 分派（，仅 Anthropic 系列支持）
 ...
 # reasoning 分派—— capabilities + 正则双重检测
 ...
 # Provider 特有字段透传（OpenAI organization / Responses API 区分）
