"""Phase：意图分类 + 相关性置信度判定 + 协商 payload 构造。
...
# Phase /：硬编码动词词典 —— **不进 Prompt Center**。
...
#
# v26.0 hotfix #1（2026-05-21）：CONFIDENCE_GAP_MAX 从 0.7 → 0.92。
...
#
# v26.0 hotfix #2（2026-05-21，UAT 复测发现）：单 ratio 仍不够，需引入
...
def evaluate_relev_confidence(
 ...
 判定逻辑：
 - ``relev_output is None`` → ``missing``（工具尚未调用 / Phase 未实施）。
 ...
 # 仅当比例接近 AND 绝对差极小才算真歧义（v26.0 hotfix #2）
 level = "low_confidence"
 else:
