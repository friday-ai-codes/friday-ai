...
（per contract/20/21/22）。

**Pitfall 防御**：必须流式处理 git log stdout，禁止 ``subprocess.check_output``
全量缓冲。100k commits 工况下 RSS < 1 GB（contract）；本实现用 ``dict[(file_a,file_b), int]``
+ ``deque[str]`` 长度 5 per pair 维持低内存。

**contract 决策（planner 裁量结果，方案 A：笛卡尔积 + 0.5 折扣）**：
...
"""metadata.commit_hashes 长度上限（per contract）；deque(maxlen=5) 保留最近 5 个 commit。"""

_LARGE_FILE_THRESHOLD = 50
"""大文件保护阈值（per contract + contract SameFile 对齐）；单文件 chunks > 50 触发限制。"""
...
class CoChangedEdgeBuilder(BaseEdgeBuilder):
    ...
    async def _stream_git_log(
        ...
        per contract/contract：必须用 ``asyncio.create_subprocess_exec`` + readline
        流式处理；禁止 ``subprocess.check_output`` 一次性缓冲全部 stdout。

        work item 修复：并发 task 同步消费 stderr，避免 OS pipe buffer（默认 64KB）
        被 git stderr（LFS warning / gc advice / replace ref 等）写满后阻塞子
        进程 → 主循环只读 stdout 永远等不到 EOF → ``proc.wait()`` 永久挂起。
        work item 修复：returncode != 0 但已采集 partial counter 时，log warning
        ...
        async def _drain_stderr() -> bytes:
            """并发消费 stderr，避免 pipe buffer 满阻塞子进程（work item）。
