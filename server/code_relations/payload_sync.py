...
一次性写入 Qdrant（contract：6 builder 全部完成后**统一**调一次，不是每 builder
独立 sync）。
...
"""5KB 超限时的 top-N 截断阶梯（per contract）。"""
