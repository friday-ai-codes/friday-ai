...
- 调度完（成功）：`graph_build_status = COMPLETED` + edge_count（累计快照口径，
  per context contract）+ payload_synced_at = now()
...
**异常隔离**（per context contract / threat security mitigation）：所有 IndexHistory
...
async def _count_edges_or_none(repository_id: str) -> int | None:
    """累计快照口径，统计失败时返回 None（与"真实为 0"区分，per work item）。
...
async def _handle_completion(
    ...
    `remaining` 是共享计数器（list 包 int 模拟可变 ref，per work item fix）：
...
def _schedule_completion_callback(
    ...
    **work item race-fix**：把新建的 completion task 也注册到 `_BACKGROUND_TASKS`，
...
async def enqueue_edge_build_for_history(
    ...
    *,
    ...
    异常隔离（per context contract）：函数体顶层 try/except 包裹，任何失败仅 log
    ...
    try:
        ...
        # work item fix：`max(new_tasks, key=id)` 用 CPython 内存地址判"最新"是错的
