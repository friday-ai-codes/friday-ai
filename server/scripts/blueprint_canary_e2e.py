...
This creates only synthetic 高阶提效 fixture data in the configured database.  It never
reads Feishu, orchestration, Git, or any configured production credential.  The public MCP
...
def _seed() -> tuple[str, str, str, str]:
    ...
    project_key = f"canary-sample-{uuid.uuid4().hex[:8]}"
    space = Space.objects.create(
        name=f"canary-space-{uuid.uuid4().hex[:8]}", feishu_project_key=project_key
    )
    project = Project.objects.create(
        id=uuid.uuid4(), space=space, name="示例功能专项（isolated canary）"
    )
    ...
    matrix = [
        {
            ...
            "base_branch": "main",
            "planned_branch": f"feat/canary-sample-{repository.name}",
    ...
    plan = McpWorkItemTechnicalPlan.objects.create(
        ...
        work_item_type="story",
        work_item_id=20260826,
        title="示例功能专项技术蓝图（isolated canary）",
    ...
    question = "示例功能专项的学习权益校验应复用哪套既有接口？"
    thread = async_to_sync(BlueprintLifecycleService().open_thread)(
        artifact,
        kind="ai_clarification",
        blocking=True,
        question=question,
        anchor={"block_id": "sample-rights", "section_path": "implementation"},
...
def run(base_url: str) -> None:
    ...
    code, rework = _post(
        ...
        rework_path,
        {
            ...
            "comment": "请明确高阶专项复用 study-course 权益接口的回退策略。",
            "rework_scope": "merge",
        },
    )
