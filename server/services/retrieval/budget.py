...
Phase 升级：扩 cross_repo 第三字段（默认 0.0 向后兼容）。
...
- 默认 rag=0.6 / graph=0.4 / cross_repo=0.0（per ROADMAP；cross_repo=0 backward compat）
...
"""settings.CROSS_REPO_BUDGET_RATIO 缺省值（默认 0.0 = 未启用跨仓预算，per ）。
...
class HybridBudget:
 """token 预算三比例策略类（frozen + slots，不可修改；Phase 扩 cross_repo 字段）。
 Attributes:
 rag: RAG 主线分配比例，默认 0.6（per ROADMAP ）
 graph: 图谱 enrichment 分配比例，默认 0.4
 cross_repo: 跨仓 API 扩散分配比例，默认 0.0（向后兼容；per ）
 向后兼容：
 ...
 """: 强制 ``rag + graph + cross_repo == 1.0`` + 非负——避免静默超 budget。
 Phase 升级：三字段 sum 校验（原双字段 rag+graph，现三字段总和）。
 ...
 Phase: 新增 cross_repo key（默认 0，不影响现有 budgets["rag"] / budgets["graph"] 读取）。
 Raises:
 ...
 """读取 Django settings 构造实例（Phase 扩 CROSS_REPO_BUDGET_RATIO）。
