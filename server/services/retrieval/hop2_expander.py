...
from services.retrieval.hop1_reader import resolve_neighbor_metadata
...
async def fetch_hop2_edges(
 ...
 """单次 ChunkEdge ORM 拉边，返回二跳候选 (source, target, edge_type, weight)。
 Args:
 hop1_chunk_ids: 一跳邻居 chunk_id 集合（作为二跳的 source 全集）。
 repo_ids: 仓库 ID 列表（命中 ``(repository, source_chunk_id)`` 索引）。
 Returns:
 ``list[(source_chunk_id_str, target_chunk_id_str, edge_type, weight)]``，
 按 weight desc 排序，长度 ≤ ``TOP_NEIGHBORS_PER_HOP2=50``：
 - ``hop1_chunk_ids`` 为空 或 ``repo_ids`` 为空 → 立即返回 ````，
 **零 SQL 查询**（fast-path 早返）；
 - 否则单次 ORM ``filter(repository_id__in, source_chunk_id__in).only(4字段)
 .order_by('-weight')[:50]`` + ``async for`` 拉满；
 - structlog ``hop2_edges_fetched`` 事件含 ``repo_count`` / ``source_count``
 / ``edge_count`` / ``capped``（``edge_count == 50`` 视为可能 capped）。
 **零 N+1**：仅一次 SQL（``CaptureQueriesContext`` 守护）。
 ...
 source_chunk_id__in=hop1_chunk_ids,
 )
 ...
 *,
 ...
 repo_ids: 候选仓库 ID 列表。
 reason_fn: ``(edge_type, source_chunk_id) -> str`` reason 生成器，
 复用 Plan ``resolve_neighbor_metadata`` 注入式契约；Plan
 落地真实 ``_explain_neighbor`` 后无缝替换。
 Returns:
 ``list[NeighborMetadata]``：
 - ``hop=2`` 固定标注；
 ...
 **性能**：单次 ChunkEdge ORM（``fetch_hop2_edges``）+ 单次 ChunkRegistry
 ...
 for src, tgt, edge_type, weight in edges:
 if tgt in reject:
 continue
 if tgt == src:
 # 自环边："自环不传递信息"（per D-Deviation 3）
 continue
 by_source.setdefault(src, ).append((tgt, edge_type, weight))
 if not by_source:
 return
