...
（security mitigation-01 mitigation）。底层 ``_REGISTRY`` dict 通过 ``register_provider``
+ ``freeze`` 双重生命周期守卫（security mitigation / contract）独占写入。
"""
...
def _reset_for_tests() -> None:
    """测试用 reset 钩子（仅在 pytest 上下文调用，per contract 不暴露给业务）。"""
    global _FROZEN
    _REGISTRY.clear()
    _FROZEN = False
