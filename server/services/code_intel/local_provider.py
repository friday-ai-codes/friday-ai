...
- ``lookup_symbols`` 对齐 ``LayeredSearchService._l2_symbol_lookup`` 的 ORM 查询
  （iexact 主路径 + icontains 回退），不暴露 Symbol ORM 对象（per security mitigation）。
- ``expand_graph`` 沿用 ``GraphExpansionService.expand`` + LayeredSearchService L4
  的 nodes 同名去重保留最短 depth 语义；``max_hops > 2`` 直接 ValueError（per work item）。
...
class LocalProvider:
    ...
    async def lookup_symbols(
        ...
        *,
        ...
        ``branch_name`` (work item): base/overlay 合并语义同 hop2——base 行
    ...
    async def expand_graph(
        ...
        *,
        ...
        if max_hops > 2:
            raise ValueError(f"max_hops={max_hops} exceeds the supported limit of 2 (per work item)")
        if max_hops < 0:
