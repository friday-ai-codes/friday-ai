"""ModelCapabilities — Provider 模型能力 + 定价查询服务。
...
Phase 承接点: runtime cache + /api/show context_length 主动拉取。
"""
...
class ModelCapabilities:
 ...
 """：Ollama /api/tags 返回的模型清单合并到 credential.available_models。
 本 phase：仅写 available_models 字段（caller 负责 save / aupdate 入库）。
 Phase 可在此基础上增加运行时 cache 与 /api/show context_length 拉取。
 Args:
