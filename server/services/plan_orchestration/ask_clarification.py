...
- **入口无关**：本 helper 只负责「产出结构化澄清请求」这一入口无关能力——**不**驱动
  ...
  ``PlanSessionService.transition`` 转移）。驱动与挂起映射是各入口私有（对齐
...
（``from services.plan_orchestration import ask_clarification``），**绝不**复用 / import /
改动 chat tool 资产。
"""
...
async def ask_clarification(
    ...
    *,
    ...
    Args:
        session: 归属的 ``PlanSession``（透传给 service，service 内 ``sync_to_async``）。
    ...
    """
    # 函数内 lazy import 规避 import 环（plan_orchestration barrel → delivery.services）。
    from delivery.services import ClarificationService
