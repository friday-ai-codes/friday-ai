...
（``plan_orchestration.__init__`` 在加载期 re-export 本模块）。
"""
...
async def agenerate_specs_for_plan(
    ...
    *,
    ...
    继续下一仓。
    """
    from delivery.models import PlanSession, PlanVersion, WorkItem
    from delivery.services import PlanSessionService, SddSpecService
    ...
    if pv is None:
        return []
    merged_plan = pv.content if isinstance(pv.content, dict) else {}

    # 来源 session（emit + requirement + work_item）；async 安全标量
    session = await PlanSession.objects.filter(current_plan_version=plan_version_id).afirst()
    requirement = ""
    work_item = None
