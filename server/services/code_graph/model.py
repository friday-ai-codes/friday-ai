...
# 生产 PostgreSQL 上 **218 个**已索引且有调用边的仓库 / base 分支 / 与 loader 同口径
# 并已交叉验证）：
#   p10=0.0762 / p50=0.1697 / p90=0.2426，全库最高的一个仓也只有 0.5593；
#   per extension：.go 0.194 / .vue 0.188 / .ts 0.146 / .py 0.148 / .tsx 0.104 /
#   .js 0.025（⚠️ 按 ``caller_file`` 后缀近似，本仓无 language 字段）。
#   原值 0.6 会命中 **218/218** 个仓库 —— 阈值**永远触发**，等于信号失效
#   （威胁登记 T-121-长鸣：长鸣的标记与不存在的标记效果相同）。
#   校准为 **0.10**：落在 p10(0.076) 与 p50(0.170) 之间，命中 38/218（17%），
#   既不长鸣也非永不触发。
#
# 🚨 **写给 Phase 122 的硬要求**：本仓生态的解析率常态就在 0.17 量级（218 个仓的
#    中位数）——也就是说，即便一个仓**没有**被判 low_resolution，它也有约 83% 的
#    调用边未解析；全库解析率最高的仓也才 0.56，没有任何一个仓「解析得好」。因此
