"""清理 N 天前已完成/错误的编排 checkpoint（Phase）。
Phase：防 SQLite orchestration_checkpoints.db 无限膨胀。
清理规则：
...
**严格**状态过滤（Security Domain T-）：只删 COMPLETED / ERROR，
...
class Command(BaseCommand):
 help = "清理 N 天前已完成/错误的编排 checkpoint（默认 7 天；Phase）"
