"""Phase Hotfix（230 ）：v8.1 Project.claude_api_key_encrypted 残留预检命令。
...
用途：报告有多少 Project 行的 `claude_api_key_encrypted` 非空 —— 这些行会在
 migration 后字段被删除而历史凭证丢失（- Denial of Service + Tampering）。
典型输出：
...
class Command(BaseCommand):
 """v8.1 Project.claude_api_key_encrypted 残留预检（Phase Hotfix 230-）。"""
 help = (
