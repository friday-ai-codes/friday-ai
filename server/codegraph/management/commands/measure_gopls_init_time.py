"""Phase: 测量 gopls 初始化耗时（ 90s 实战门 / 60s stretch advisory）。
per /
==================
- ** 实战门（硬卡）**：``init_time_ms < 90000``（90s）
- **Stretch advisory**：``init_time_ms < 60000``（60s）；不阻塞 phase 完成
CLI 用例
========:
 python manage.py measure_gopls_init_time \\
 --repo-root=/Users/example/Projects/guanghe/study-course \\
...
class Command(BaseCommand):
 """Phase：gopls 初始化耗时测量命令。"""
 help = "测量 gopls 初始化耗时（：90s 实战门 / 60s stretch advisory）"
