"""Phase: GoplsBackend —— LspBackend 子类实装（4 hook + ClassVar 覆写）+ 工厂闭包。
继承 Phase ``LspBackend`` 的 4 extract_* 模板方法（try/except + tree-sitter
...
_lsp_extract_calls tree-sitter raw symbols + per-symbol references
 全部入 CallData（per：Phase 精化）
_lsp_extract_endpoints return （Go 端点 Phase gin tree-sitter；per ）
============================ ==============================================
per：gopls 复用 Phase ``_SUPERVISORS["gopls"]`` module-level 单例缓存
——与 Phase VolarPool 多实例选择**正好对称**，体现 Phase 双路径设计意图。
per：``make_gopls_backend(language)`` 工厂闭包替换 Phase 占位
...
# LSP SymbolKind → Friday SymbolData.symbol_type 映射（per V2 转换契约 / Pitfall P-）
_SYMBOL_KIND_MAP: Final[dict[int, str]] = {
 ...
 lsp.SymbolKind.TypeParameter.value: "VARIABLE", # Go 泛型（per Pitfall P-）
 lsp.SymbolKind.Property.value: "VARIABLE",
}
class _GoplsLazyBackend(LspBackend):
 """gopls LSP backend；继承 Phase LspBackend + 4 hook 实装。
 Phase 实装 _GoplsLazyBackend 模式：闭包内 lazy 推断 go_mod_root +
 通过 get_or_create_supervisor（module-level 单例）拿 supervisor。
 per：gopls 单实例策略（与 Phase VolarPool 多实例对称）。
 """
 ...
 """workspace/symbol + documentSymbol 混合策略（per / V2）。
 异常路径不 try/except；让 Phase LspBackend.extract_symbols 基类捕获 +
 fallback tree-sitter（per ）。
 """
 ...
 """tree-sitter raw + textDocument/definition 双层（per ）。
 策略：
 ...
 """per-symbol textDocument/references 反向追踪（per / ）。
 per：Phase 简化——全部 reference 入 CallData；
 Phase 精化 cross-file caller 解析。
 策略：
 ...
 """Go 端点 Phase gin tree-sitter 处理；本 phase 直接 return （per ）。"""
 return
# =============================================================================
...
def _convert_workspace_symbol(
 ...
 # 未知 kind → 默认 "FUNCTION"（per Pitfall P-）
 symbol_type = (
...
def _resolve_import_target_path(
 ...
 per Pitfall P- / Go 版本：character=0 在多数 import 行有效。
 """
...
def make_gopls_backend(language: str = "go") -> Callable[[str], ExtractorBackend]:
 """工厂闭包 —— 替换 Phase 占位 ``make_lsp_backend``（gopls 版本）。
 ...
 per：gopls 单实例策略——与 Phase VolarPool 多实例选择正好对称；
 用尽了 Phase 双路径设计意图。
 Args:
 ...
 Note:
 per Pitfall P-：闭包内 _GoplsLazyBackend 沿用 Phase VolarLazyBackend
