"""Phase: Node 22 LTS + vue-language-server + typescript SDK 检测层。
per：启动时一次性 subprocess 检测 + 进程级缓存（不过期；服务重启即重检）。
per：tsdk 三探针顺序（npm root -g / which tsc / monorepo node_modules）+ None fallback。
...
- 检测失败 **不** raise；返 ``NodeCheckResult.available=False``，让 VolarPool
 调用方决定是否走 fallback（per Phase fallback 兜底原则；per ）。
...
def discover_tsdk -> Path | None:
 """typescript SDK lib 路径动态发现（三探针 + None fallback，per ）。
 探针顺序：
