"""Phase: nx sub-project 自动发现层（三探针并集 + tsconfig.json 兜底）。
per：
 ...
 跳过；``!`` 前缀 glob 不支持
per /：薄防御层 —— 读 package.json ``dependencies.vue`` /
``devDependencies.vue`` semver；vue<2.7 时仍纳入 SubProject list（标 vue_version）；
``VolarPool.get`` 入口再防御 raise，触发 Phase LspBackend 基类 fallback tree-sitter。
公开 API
========
- ``discover_sub_projects(repo_root: Path) -> list[SubProject]``
- ``is_vue_27_or_newer(vue_spec: str | None) -> bool``
- ``SubProject`` frozen dataclass（per 让其可作 dict key / OrderedDict 配合）
"""
...
class SubProject:
 """nx sub-project 元数据（per frozen=True 让其可作 dict key）。"""
