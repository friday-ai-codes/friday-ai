"""Phase: LspBackend 抽象基类 —— 5 模板方法 + 5 abstract hook + tree-sitter fallback。
设计要点（per / / Pitfall P14 / / ）：
...
# 4 个 fallback 事件名（与 supervisor.py 一致，per / V5）
...
class LspBackend(abc.ABC):
 ...
 子类（如 Phase ``VolarBackend`` / Phase ``GoplsBackend``）需覆写：
 - ClassVar 字段：``name`` / ``language_ids`` / ``command`` /
 ``initialization_options``（per ）
...
def make_lsp_backend(name: str) -> Callable[[str], LspBackend]:
 ...
 GoplsBackend）在 Phase / 267 各自定义 ``make_volar_backend`` /
