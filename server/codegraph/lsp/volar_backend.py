"""Phase: VolarBackend —— LspBackend 子类实装（4 hook + ClassVar 覆写）+ 工厂闭包。
...
============================ ==============================================
per：``make_volar_backend(language)`` 工厂闭包替换 Phase 占位
...
class VolarBackend(LspBackend):
 """volar (@vue/language-server) LSP backend；继承 Phase LspBackend。"""
 ...
 的完整 cross-file caller 解析在 Plan 集成测试驱动迭代）
...
def _resolve_import_target_path(
 ...
 per Pitfall P-：character=0 在多数 import 行有效；偶失败返 None
 让上层使用原始 specifier。
 """
...
def _convert_references_to_call_data(
 ...
 """``Location`` → ``CallData``；过滤 callee 自身位置（per Pitfall P-）。
 Returns:
...
def make_volar_backend(language: str) -> Callable[[str], ExtractorBackend]:
 """工厂闭包 —— 替换 Phase 占位 ``make_lsp_backend``。
 ...
 Note:
 闭包接收的 ``actual_language`` 与外层 ``language`` 在 Phase BACKEND_REGISTRY
 替换路径下一致；保留参数让 GraphExtractor 可统一调用 ``factory(lang)`` 风格
 （per Phase register_backend 签名约束）。
