"""Phase: GoplsInterfaceExtractor —— Go interface 实现关系抽取模块。
本模块提供 Go interface 实现关系的 LSP 抽取能力（textDocument/implementation）；
独立于 ExtractorBackend Protocol（per Phase），可由 indexer 选择性消费。
per 理由：
...
class InterfaceImplementationData:
 """Go interface 实现关系数据（per Phase /：frozen=True）。
 Fields:
 ...
 *,
 ...
 per Phase：仅处理 symbol_type == "CLASS" 的 symbol（Go interface/struct
 在 Phase SymbolKind 映射中均为 "CLASS"）；per-symbol 调 gopls 的
 ...
 Note:
 本函数复用 Phase get_or_create_supervisor("gopls", ...) 单例缓存；
 与 _GoplsLazyBackend._get_supervisor 使用同一 supervisor 实例（per ）。
 """
