"""Phase: LspSupervisor —— LSP 进程 lifecycle + 健康检查 + crash-loop 防护。
核心职责（per / / / / ）：
...
5. **did_* stub API**：``notify_did_open / change / close`` 仅维护
 ``_open_documents`` 字典 + log debug（per ）；真实 LSP notification 由
 Phase / 267 子类 backend 实装
6. **可观测性**：12 事件名常量字典（per ）+ structlog 字段化所有状态转换
设计约束：
- **不**用 ``async_to_sync`` / ``sync_to_async``（per Pitfall P11；Phase 踩坑）
...
class LspSupervisor:
 ...
 """重启后批量 didOpen 复盘 stub（per ）。
 本 phase 仅 log info；真实 LSP notification 由 Phase / 267 子类实装。
 """
 logger.info(
