"""Phase: VolarPool —— 多 sub-project volar 实例池 + LRU 调度。
per：``max_concurrent=4`` + ``OrderedDict`` LRU 驱逐最久未用
per：模块级单例 ``get_volar_pool`` + ``threading.Lock`` 守门并发 init
per：VolarPool 内部 ``_pool: OrderedDict[Path, LspSupervisor]`` 自管实例
 （**不**复用 Phase ``_SUPERVISORS`` 模块级 cache，避免多实例 collide）
per：所有 supervisor 共享 ``services/background_runner.py`` 全局 loop（同 Phase）
公开 API
========
- ``VolarPool`` 类：``get(sub_project_path, *, vue_version)`` /
 ``shutdown_all(timeout)``
- ``get_volar_pool -> VolarPool`` 模块级单例工厂
4 个新 structlog 事件（per / ）
==========================================
 ...
 *,
 ...
 per Pitfall P-：atexit 阶段 background loop 可能已停，
...
def get_volar_pool -> VolarPool:
 """模块级单例（per ）；lazy 实例化让 settings 加载顺序安全。"""
 global _VOLAR_POOL
