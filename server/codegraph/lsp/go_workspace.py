"""Phase: Go workspace 自动发现 —— go.work 优先 + go.mod 最浅路径策略。
per：gopls 复用 Phase _SUPERVISORS module-level 单实例缓存；
 workspace_root 指向 go.mod 最近父目录（不是文件所在目录）。
per：vendor/ 内嵌 go.mod 不采纳（只扫一级子目录）。
per Pitfall P-：vendor/ 显式跳过，防误识别。
per Pitfall P-：go.work use 路径相对于 go.work 所在目录，需 resolve 绝对化。
公开 API
========
...
def discover_go_workspace(repo_root: Path) -> GoWorkspace | None:
 ...
 # 优先检查 go.work（per / go 1.18+ workspace 模式）
...
def _parse_go_work(go_work: Path, repo_root: Path) -> GoWorkspace | None:
 ...
 per Pitfall P-：use 路径相对于 go.work 所在目录（即 repo_root）。
 """
 try:
