"""Phase: LSP supervisor 集成测试（真实 subprocess + work item roundtrip）。
覆盖 5 路径：
1. supervisor 启动 + 健康检查通过（V4 第一段）
2. friday/crash 触发崩溃后 supervisor 状态非 READY（V4 第二段 + T- 部分）
3. friday/hang 触发 LspTimeoutError（T- / V4）
4. 启动命令失败 N 次后转 DISABLED（V3 / T- crash-loop 防护核心）
