"""Phase: registry 集成测试（apps.ready + EXTRACTOR_BACKENDS 切 volar + fallback）。
per Plan Task 3 acceptance：
...
- 端到端：Vue 2.6- mock VolarPool.get raise → LspBackend 基类 fallback →
...
def test_vue26_fallback_end_to_end_via_lsp_unhealthy_error(tmp_path: Any) -> None:
 """ 端到端：mock supervisor raise LspUnhealthyError →
