"""initial implementation: gopls v0.14+ + Go SDK ≥ 1.20 二进制检测层 + 进程缓存。
...
per work item：检测失败不 raise；返 GoCheckResult.available=False，让 _GoplsLazyBackend
  调用方决定是否走 fallback（per initial implementation fallback 兜底原则）。
...
_MIN_GOPLS_MINOR: Final[int] = 14  # gopls v0.14+（per work item）
...
def check_go_runtime(*, force_refresh: bool = False) -> GoCheckResult:
    ...
    Args:
        force_refresh: 测试用入口；正常路径走缓存（per Pitfall P-checkpoint）。
...
def _probe_gopls() -> tuple[str | None, bool, str]:
    ...
    per Pitfall P-checkpoint：gopls version 输出格式不稳定，使用宽松正则。
    ...
    # 宽松正则匹配 gopls version（per Pitfall P-checkpoint）
