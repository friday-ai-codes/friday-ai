...
def file_neighbors(
    ...
    """文件级双向依赖（work item）：上游=被谁调用/导入，下游=调用/导入了谁。
...
def component_neighbors(
    ...
    """组件级双向依赖（work item）：上游=用本组件的组件，下游=本组件用的组件。
