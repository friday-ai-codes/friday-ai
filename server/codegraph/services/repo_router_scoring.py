...
def select_stage0_pool(
    ...
    *,
    ...
    - **主序** ``primary_slots``（默认 8）：原总分前排原样保留——这是多探针
      修复后实测稳定的 Stage 1 窗口，含 sample_practice_service 等 breadth 偏高但
      确实相关的仓；
    - **预留** ``top_k - primary_slots``：从 ``score - breadth`` 序补进主序
      未覆盖的仓（专精仓 sample_course_service 路径）。

    返回顺序 = 主序 + 预留（不再按总分重排），配合
    ``REPO_ROUTER_STAGE1_MAX_CANDIDATES >= primary_slots + 1``，让 LLM
    先看稳定前排、再看到专精仓。纯去 breadth 重排会把 sample_practice_service
    ...
    # 预留席最多 2：再多会把「总分刚出主序」的假阳性（如 sample_web）塞进
    # Stage 1，挤掉 sample_service_service（实测）。专精仓 sample_course_service 一个席位就够。
    ...
    # 占了预留席只会稀释 Stage 1（sample_web 路径）。只从更靠后的仓里按去
    # breadth 序补位，sample_course_service（总分 #14、去 breadth #10）才能进来。
