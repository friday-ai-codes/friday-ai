"""HtmlExtractor 单元测试 —— 验证 Phase 落地。
per Phase /：PascalCase tag / custom element / id 属性 / 三类 import /
...
class TestHtmlExtractor:
 ...
 """<img src="data:image/..."> 不在 imports（per scheme 守卫）。"""
 bundle = HtmlExtractor.extract(
 "apps/test/public/index.html", html_index_source, html_ctx
 )
 ...
 """<a href="https://example.com"> 不在 imports（per OUT OF SCOPE）。"""
 bundle = HtmlExtractor.extract(
 "apps/test/public/index.html", html_index_source, html_ctx
 )
 targets = [i.target_module for i in bundle.imports]
