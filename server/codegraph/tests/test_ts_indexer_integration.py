"""TS / TSX 真实仓库 study-app 端到端集成测试 —— 覆盖 / 真实仓库验证。
不调 indexer ORM 路径（per Phase CONTEXT decisions 同 Phase 精神
"不扩展到 GraphWriter 全链路"），仅断言 GraphExtractor.extract_all 在
真实 .ts / .tsx 文件上返回非空 bundle 各字段。
环境变量 TS_SAMPLE_REPO 不设时默认指向 `/Users/example/Projects/guanghe/study-app`；
路径不存在时整 TestStudyAppExtraction 类 SKIP（CI / 其他开发机兜底，不阻断 phase）。
TestTSExtractorRegistration 不带 skipif，任意环境必跑（巩固 Plan 双注册）。
"""
...
TS_SAMPLE_REPO = Path(
 os.environ.get(
 "TS_SAMPLE_REPO",
 "/Users/example/Projects/guanghe/study-app",
 )
)
@pytest.fixture
def ts_parser:
...
class TestStudyAppExtraction:
 ...
 # calls 可能 0（纯 interface / type 文件），不强断言（per 宽阈值）
