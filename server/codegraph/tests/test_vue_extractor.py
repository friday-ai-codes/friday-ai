...
class TestVueExtractor:
 ...
 """ /：methods 内 handleClick / onSave 抽 METHOD。"""
 extractor = VueExtractor
 ctx = FileContext(
 file_path="vue2_options.vue", language="vue", repository_id="r1"
 )
 ...
 """ /：template `@click="handleClick"` → TEMPLATE_REF CallData。"""
 extractor = VueExtractor
 ctx = FileContext(
 file_path="vue2_options.vue", language="vue", repository_id="r1"
 )
...
function bar {}
 ...
 """ / Pitfall 1：script 内 onClick 行号还原到原文件视角。"""
 extractor = VueExtractor
 ctx = FileContext(
 file_path="vue27_setup.vue", language="vue", repository_id="r1"
 )
