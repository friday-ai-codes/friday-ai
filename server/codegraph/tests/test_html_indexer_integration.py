"""HTML 真实仓库 study-app 端到端集成测试 —— 覆盖 Phase 真实样本。
不调 indexer ORM 路径（同 Phase / 262 / 263 精神），仅断言 HtmlExtractor.extract
...
HTML_SAMPLE_REPO = Path(
 os.environ.get("STUDY_APP_REPO", "/Users/example/Projects/guanghe/study-app")
)
...
@pytest.mark.skipif(
 not HTML_SAMPLE_REPO.exists,
 reason=f"study-app sample repo not found at {HTML_SAMPLE_REPO}",
)
