"""Vue 真实仓库 study-app 端到端集成测试 —— 覆盖 Phase 集成验证。
不调 indexer ORM 路径（同 Phase / 262 精神），仅断言 VueExtractor.extract
在真实 .vue 文件上返回非空 bundle 各字段。
环境约束：
- 默认采样路径 `/Users/example/Projects/guanghe/study-app`，可通过环境变量
 `VUE_SAMPLE_REPO` 覆盖
