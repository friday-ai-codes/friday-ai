"""work item —— PythonImportResolver：Python import 模块名 → 仓内文件路径。
...
① **目标集合**：从数据库后缀查询改为查内存 ``SymbolIndex._files`` 集合
   （精确等值，checkpoint 路径基准结论已坐实 ``Symbol.file_path`` = 仓相对路径基准）；
...
class PythonImportResolver:
    """Python import 模块名 → 仓内 ``file_path`` 的语言专属解析器（work item）。"""
    ...
    def resolve_module(
        ...
        # ② 相对导入：按前导点数计回溯层级（PEP 328 / work item，禁用 lstrip("./")）。
    ...
    def _lookup(self, candidate: str) -> str | None:
        ...
        checkpoint 路径基准结论已坐实 ``Symbol.file_path`` = 仓相对路径，正常 ``has_file`` 精确等值即
        命中；锚定兜底是防御性双保险——仅当仓内 ``file_path`` 含统一前缀目录（与精确口径
        不一致）时生效。锚定用 ``f"/{candidate}"`` 而非裸 ``candidate`` endswith，防
        ``auth.py`` 误匹配 ``oauth.py``（work item）。
        """
