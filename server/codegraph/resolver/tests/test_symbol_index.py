"""work item SymbolIndex 单测 + RESEARCH Open Question Q1 路径基准坐实。
...
async def test_path_basis_alignment(test_repository) -> None:
    ...
    结论（供 checkpoint 引用）：基准 = 仓相对路径（GraphWriter 直接落 bundle.file_path /
    SymbolData.file_path，无 repo 子目录前缀改写）。因此 ``has_file`` 用**精确等值**口径
    即可命中；``/`` + endswith 锚定兜底仅在未来发现前缀不一致时才需要（留给 checkpoint）。
    """
