...
def extract_calls(tree: Any, ctx: "FileContext") -> "list[CallData]":
    ...
    caller_key[1] 用稳定字面 sentinel "<module>"（work item）。
...
def _extract_one_call(wn: Any, ctx: "FileContext") -> "CallData | None":
    ...
    模块级调用（无 ancestor_function）caller_key[1] 用 "<module>"（work item），
    ...
    # ★ JSX 元素分支（work item / work item）：仅大写组件抽为 call_type=JSX
