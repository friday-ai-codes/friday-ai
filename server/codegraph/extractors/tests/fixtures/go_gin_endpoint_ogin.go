...
import (
	...
	"gitlab.example.com/backend/yc/pkg/server/ogin"
)
// StartHTTP 模拟 study-course 风格的 *ogin.Server 路由注册（用于 测试）
func StartHTTP(server *ogin.Server) {
	// Use 注册全局 middleware ——：不写入 endpoint 表
	server.Use(metricsMiddleware)
	server.Use(tracingMiddleware)
	// ogin.G* 参数验证 middleware：测试 metadata 提取
