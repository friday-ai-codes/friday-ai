...
Step 0: 扫描 axios.{METHOD}() 调用 → 识别所在 export function 为 LowLevelHelper（work item）
Step 1: 找调用 LowLevelHelper 的 export function → ApiWrapper，提取 URL+method（work item）
Step 2: volar textDocument/references 反向追踪 ApiWrapper → ApiCallSite（work item）

Vue SFC 支持：通过 vue_sfc_splitter 拆 script block 后用 TS parser 解析（work item）
"""
...
def discover_low_level_helpers(
    ...
    Returns:
        LowLevelHelper 函数名列表（per work item）
    """
    helpers: list[str] = []
    root = tree.root_node
...
def discover_api_wrappers(
    ...
    - url_path_pattern: strip_base_url 后的路径（work item）
    - _jsdoc_text: 紧前方 JSDoc 注释（Plan-04 JSDoc 富集消费）
    ...
    Returns:
        ApiWrapperData 列表（per work item）
    """
    if not helper_names:
        return []
...
def resolve_wrappers_for_repository(
    ...
    # JSDoc 富集（work item，Plan-04 enrich_wrapper_metadata 调用）
