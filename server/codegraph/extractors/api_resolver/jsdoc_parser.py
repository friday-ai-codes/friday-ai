...
per: 解析 @description / @author / @date / yapi URL pattern。
yapi URL pattern: https://yapi.example.com/project/{pid}/interface/api/{iid}
"""
...
def parse_jsdoc(comment_text: str | None) -> dict[str, Any] | None:
 ...
 pid = int(m.group(1))
 iid = int(m.group(2))
 yapi_url = f"https://yapi.example.com/project/{pid}/interface/api/{iid}"
