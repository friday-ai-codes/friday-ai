...
per initial implementation / work item：HTML 分支抽 <link href> / <script src> / <img src>，
...
# initial implementation：HTML 抽 ImportData 的标签 + 对应 attribute 名
...
def _parse_import_statement_ts(
    ...
    """解析 TS / TSX 的 import_statement 节点（per work item / work item / work item）。
    ...
    - type-only 导入 `import type { T } from 'mod'` —— 与运行时同等记录（work item）
...
def _parse_export_statement_ts(
    ...
    """解析 TS / TSX 的 export_statement 重导出形态（per work item）。
...
def _extract_html_imports(
    ...
    per initial implementation / work item：仅抽 <link href> / <script src> / <img src>；
