"""Python 专用抽取器 —— 直接实现 LanguageExtractor，不依赖 registry 模块。

固定使用 TreeSitterBackend("python")，避免与 registry.py 的循环导入。
"""
...
class PythonExtractor:
    ...
    直接组合 TreeSitterBackend 实现 extract() 单一入口，
    类型名独立以便后续阶段引入替代 backend 时可在此覆写。
    """
