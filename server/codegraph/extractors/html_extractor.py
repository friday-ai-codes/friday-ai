...
per Phase /：与 ts_extractor.py / go_extractor.py 字面对称，
**不**沿用 Phase SFC pre-splitter 模式（HTML 是单 grammar 文件，无须中间层）。
calls / endpoints 显式 —— per /：HTML 无 callable / endpoint 语义；
inline `<script>` / `<style>` 段不递归解析（per / 留 Phase volar）。
