...
# contract patch compat：``from codegraph.services import layered_search`` 间接 import，
...
def _serialize_neighbor(neighbor: Any) -> dict[str, Any]:
    ...
    work item 兜底（防 partial mock weight=None 让前端 ``null.toFixed`` 抛 TypeError）：
...
def _serialize_layer(layer: LayerResult) -> dict[str, Any]:
    ...
    关键差异 5（security mitigation）：L4 items 包含 Symbol ORM 对象，
    ...
    # 手动序列化，不暴露完整 ORM 对象（security mitigation 信息披露防护）
...
class PlaygroundSearchView(APIView):
    ...
    五层检索测试面板（仅 admin 可访问，security mitigation 权限防护）。
    请求体：
    ...
    async def post(self, request: Any) -> Response:
        ...
        # contract: int(...) 失败时返回 400 Bad Request 而非 500（用户传入非整数字符串）
        try:
