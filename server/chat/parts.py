"""Chat assistant message ``parts`` 数据模型与 builder（workflow update）。
...
设计决策：数据契约、schema versioning、collector 状态机与 legacy hydration
行为保持一致。
"""
...
# 同步升 version 并保留旧分支（additive 策略，见 schema versioning contract）。
...
# 前端 ``MessagePart`` 联合类型必须与此处保持同源（contract 输出）。
Part = Annotated[
...
class PartsCollector:
    ...
    def to_message_payload(self) -> dict[str, Any]:
        ...
        - ``content`` = 所有 text part ``.text`` 拼接（向后兼容字段，contract
          强同源契约）。
