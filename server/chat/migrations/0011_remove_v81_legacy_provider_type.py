"""Phase Plan /：硬删 Conversation.provider_type 字段。
forwards:
...
def forwards_noop(apps, schema_editor):
 """Phase Plan：Conversation.provider_type 已在 Phase/229 UI cutover 后
 不再被写入。本函数作为 RunPython 的 forwards 钩子保留结构（便于测试观察），
 不做实际数据转换。
 """
 # 历史数据：由于 Conversation 无 metadata JSONField，provider_type 值硬删即丢失。
 # 调用方（前端 / ProviderConfigService）已在 Plan 切换到 provider_credential_id FK。
 return
