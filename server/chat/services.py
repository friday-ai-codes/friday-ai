...
def _load_project_credential_sync(project_id: int) -> tuple[str, str]:
    """initial implementation plan：同步读项目级默认 ProviderCredential(anthropic)。
...
async def _aload_project_credential_async(project_id: int) -> tuple[str, str]:
    """initial implementation plan：异步读项目级默认 ProviderCredential(anthropic)。"""
    from system.models import ProviderCredential
    from services.provider_config import (
        PROVIDER_REGISTRY,
        ProviderType,
...
def _load_system_credential_sync() -> tuple[str, str]:
    """initial implementation plan：同步读系统级默认 ProviderCredential(anthropic)。"""
    from system.models import ProviderCredential
    from services.provider_config import (
        PROVIDER_REGISTRY,
        ProviderType,
...
def get_chat_service(
    ...
    initial implementation plan（contract/contract）：从 ProviderCredential 读取凭证
