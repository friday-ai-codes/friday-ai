...
#
# 验证范围:
#   work item: Runner 注册 + 心跳保持
#   work item: Docker 容器隔离执行 + 结果回报
...
# per contract: Docker Compose 本地全栈启动
...
log_step "注册 Runner (per contract)"
# per contract: 手动流程验证 -- friday-runner register
...
log_step "启动 Runner 并验证心跳 (per contract)"
...
if [ "$HEARTBEAT_STATUS" = "online" ]; then
  log_pass "3 个心跳后 Runner 仍在线 (work item 心跳验证通过)"
else
  log_fail "心跳后 Runner 状态异常: $HEARTBEAT_STATUS"
fi

# per contract: 验证断连恢复
log_step "验证断连恢复 (per contract)"
kill "$RUNNER_PID" 2>/dev/null || true
wait "$RUNNER_PID" 2>/dev/null || true
...
if [ "$RECOVER_STATUS" = "online" ]; then
  log_pass "断连恢复: Runner 重新上线 (work item 断连恢复验证通过)"
else
  log_fail "断连恢复失败: $RECOVER_STATUS (预期 online)"
fi

# ============================================================================
# 阶段 3: work item -- 容器隔离执行 (per contract, contract, contract)
# ============================================================================

log_step "创建真实任务并分发到 Runner (per contract)"
...
if echo "$DISPATCH_RESULT" | grep -q "dispatched"; then
  log_pass "任务已通过 TaskDispatcher 分发到 Runner (per contract)"
else
  log_fail "任务分发失败: $DISPATCH_RESULT"
fi

log_step "验证容器隔离执行 (per contract)"
...
if [ "$CONTAINER_FOUND" = true ]; then
  # 检查容器日志是否可见 (per contract: 执行日志可在 Server 端查看)
  log_info "获取容器执行日志..."
  CONTAINER_LOGS=$(docker logs "$CONTAINER_ID" 2>&1 | head -20)
  if [ -n "$CONTAINER_LOGS" ]; then
    log_pass "容器日志可获取 (per contract 容器隔离验证通过)"
    echo "$CONTAINER_LOGS" | head -5
  else
    log_info "容器日志为空（容器可能还在执行中或已完成）"
  fi
else
  log_fail "60 秒内未检测到 friday-task 容器启动 (per contract)"
fi

log_step "验证结果回报 (per contract)"
...
" 2>/dev/null)
  ...
  if [ "$ASSIGNMENT_STATUS" = "completed" ]; then
    ...
    log_pass "任务结果回报成功: status=completed (per contract 结果回报验证通过)"
    break
  elif [ "$ASSIGNMENT_STATUS" = "failed" ]; then
    RESULT_REPORTED=true
    log_info "任务执行失败但结果已回报: status=failed (结果回报机制正常)"
    # E2E 测试中使用假 repo，失败是预期的；关键是结果能回报
    log_pass "任务结果回报机制正常: status=failed (per contract 结果回报验证通过)"
    break
...
if [ "$RESULT_REPORTED" = false ]; then
  log_fail "120 秒内未收到任务结果回报 (per contract)"
  # 输出 Runner 日志帮助调试
