#!/usr/bin/env bash
# implementation work CI 防回归脚本：禁止 server/chat/ 与 server/workflows/nodes/ai/
...
# 白名单（implementation work contract 双轨保留的 fallback 常量定义行 + work item 边界豁免）:
...
if [[ -n "$MATCHES_1" ]]; then
    ...
    if [[ -n "$FILTERED_1" ]]; then
        ...
        echo "$FILTERED_1"
        echo ""
        echo "implementation work 要求所有 prompt 走 render_prompt(slug, ..., fallback=ORIGINAL_CONST)"
...
if [[ $FAIL -eq 0 ]]; then
    ...
    echo ""
    echo "❌ check_prompt_hardcoding FAILED — 上列位置需迁移到 Prompt Center（implementation work）"
    exit 1
fi
